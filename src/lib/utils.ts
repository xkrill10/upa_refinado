...
export function formatWords(text: string) {
  ...
  return workingText
    ...
    .map((word, index) => {
      ...
      if (index === 0) {
        return word.charAt(0).toUpperCase() + (isAllCaps ? word.slice(1).toLowerCase() : word.slice(1));
      }
...
export function formatPatientNameLGPD(name: string) {
  if (!name || name.toUpperCase().includes("NÃO IDENTIFICADO") || name.toUpperCase().includes("PRÉ-CADASTRO") || name.toUpperCase().includes("DESCONHECIDO")) {
    return "PACIENTE";
  }
  ...
  let index = 1;
  while (index < parts.length && skipWords.includes(parts[index].toUpperCase())) {
    index++;
  }
