...
export default function MyWorkspace() {
  ...
  const [finishingPatient, setFinishingPatient] = useState<Patient | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (!room || !doctor) {
      toast.error("Acesso negado", {
        description: "Por favor, assuma um consultório primeiro.",
      });
  ...
  useEffect(() => {
    if (prevPatientsRef.current.length > 0) {
      patients.forEach((newPatient) => {
        const oldPatient = prevPatientsRef.current.find(
        ...
        if (oldPatient) {
          newPatient.exams?.forEach((newExam) => {
            ...
            if (
              oldExam &&
              oldExam.status !== "completed" &&
              newExam.status === "completed"
            ) {
              // Verifica se o paciente está associado a este médico
              if (
                newPatient.sector === activeRoom ||
                newPatient.subStatus === "reaval"
              ) {
                playDing();
  ...
  const isPediatric =
    activeRoom.toUpperCase().includes("PEDIÁTRICO") ||
    activeRoom.toUpperCase().includes("PEDIATRIA");
  ...
  const relevantPatients = patients.filter((p) => {
    ...
    if (isEmergencyRoom) {
      ...
      if (
        activeRoom.toUpperCase().includes("PEDIATRIA") ||
        activeRoom.toUpperCase().includes("PEDIÁTRIC")
      ) {
  ...
  const myAttendingPatients = relevantPatients
    .filter((p) => p.status === "attending" && p.sector === activeRoom)
    .sort(
      (a, b) =>
        new Date(a.arrivalTime).getTime() - new Date(b.arrivalTime).getTime(),
    );

  const waitingPatients = relevantPatients
    .filter(
      (p) =>
        p.status === "waiting" &&
        p.risk &&
    ...
    <div className="flex-1 w-full flex flex-col min-h-[calc(100vh-6rem)] relative overflow-hidden bg-transparent">
      ...
      <div className="flex-1 p-6 space-y-8 overflow-y-auto max-w-[1400px] mx-auto w-full pb-24">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" /> Dentro do
              Consultório
            </h2>
              ...
              {myAttendingPatients.length === 0 ? (
                ...
                >
                  <div
                    className={cn(
                      ...
                      isBlueTheme
                        ? "bg-[#006699]/10 text-[#006699] dark:bg-sky-500/10 dark:text-sky-400"
                ...
                myAttendingPatients.map((patient) => {
                  const allExamsCompleted =
                    ...
                    patient.exams.filter((e) => e.status === "completed")
                      .length === patient.exams.length;
                  return (
                    <motion.div
                      key={patient.id}
                      ...
                      <Card
                        className={cn(
                          ...
                          allExamsCompleted
                            ...
                            : isBlueTheme
                              ? "border border-white/50 dark:border-white/10 hover:shadow-[0_8px_32px_0_rgba(0,102,153,0.15)]"
                              : "border border-white/50 dark:border-white/10 hover:shadow-[0_8px_32px_0_rgba(249,115,22,0.15)] dark:hover:shadow-[0_8px_32px_0_rgba(249,115,22,0.2)]",
                        )}
                      >
                        {allExamsCompleted && (
                        ...
                        <CardContent className="p-0 flex flex-col md:flex-row items-stretch">
                          ...
                          <div className="p-5 flex flex-col justify-center gap-3 border-b md:border-b-0 md:border-r border-slate-200/50 dark:border-slate-800/50 flex-1 md:max-w-[250px]">
                            ...
                            >
                              <User className="h-3.5 w-3.5 mr-1.5" /> Ver
                              Prontuário
                            </Button>
                          </div>
                          ...
                          <div className="p-5 flex-1 min-w-[300px] flex flex-col justify-center gap-3 bg-slate-50/50 dark:bg-slate-900/20">
                            ...
                            >
                              <Stethoscope className="h-4 w-4 mr-2" /> Iniciar
                              Atendimento
                            </Button>
                            {!isFastTrack && (
        ...
        <section className="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <Users className="h-4 w-4 text-primary" /> Sala de Espera
              (Aguardando)
            </h2>
            <div className="bg-slate-200/50 dark:bg-slate-800 px-3 py-1 rounded-full text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">
            ...
            {waitingPatients.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {waitingPatients.map((patient) => {
                  ...
                  const isMyReaval =
                    patient.subStatus === "reaval" &&
                    patient.sector === activeRoom;
                  const allExamsCompleted =
                    patient.exams &&
                    patient.exams.length > 0 &&
                    patient.exams.filter((e) => e.status === "completed")
                      ...
                      <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          <div
                            className={cn(
                              ...
                              patient.risk === "emergency"
                                ...
                                : patient.risk === "very-urgent"
                                  ? "bg-orange-500 text-white animate-pulse shadow-[0_0_15px_rgba(249,115,22,0.5)] ring-2 ring-orange-400 ring-offset-2 ring-offset-white dark:ring-offset-slate-900"
                                  : patient.risk === "urgent"
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">
                              ...
                              • {patient.ticket || "GERAL"} • Aguardando há{" "}
                              {Math.floor(
                                (new Date().getTime() -
                        ...
                        <Button
                          className={cn(
                            ...
                            isBlueTheme
                              ? "bg-[#006699] hover:bg-[#005580] text-white"
                              : "bg-orange-500 hover:bg-orange-600 text-white",
                          )}
                          onClick={() => handleCall(patient)}
                        >
                          <Megaphone className="h-3.5 w-3.5 mr-2" /> Chamar
                          Próximo
                        </Button>
                      </div>
                    </div>
      ...
      <Dialog
        open={!!finishingPatient}
        onOpenChange={(open) => !open && setFinishingPatient(null)}
      >
        <DialogContent className="sm:max-w-[425px] border-emerald-100 dark:border-emerald-900/30 shadow-2xl rounded-3xl p-6 glass-card-premium bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-center font-medium text-slate-600 dark:text-slate-400 mt-3 text-sm">
              Você está prestes a finalizar o atendimento de{" "}
              <strong className="text-slate-800 dark:text-slate-200">
                {finishingPatient?.name}
              </strong>
              .
              <br />
              <br />
