...
import {
  usePrescriptions,
  AprazamentoHour as ContextAprazamentoHour,
...
const getPatientAvatar = (
  ...
  if (age !== undefined) {
    const isMale =
      ...
      name.toLowerCase().includes("pedro");
    const isFemale =
      gender?.toUpperCase() === "F" ||
      name.toLowerCase().includes("maria") ||
      name.toLowerCase().includes("helena");
...
export default function NursingCheck() {
  ...
  const [patientData, setPatientData] = useState<
    Record<string, PatientCheckData>
  >(() => {
  ...
  const [viewingPrescription, setViewingPrescription] =
    useState<ExtendedPrescriptionItem | null>(null);
    ...
    return [
      {
        ...
        reportedAt: "2026-05-23 10:45",
        nurseName: "Enf. Ricardo Braga",
        observation:
          "Paciente relatou cefaleia súbita e intensa. Administrado repouso.",
      },
    ];
  });
  ...
  const currentSheet = selectedPatient
    ? getPatientCheckSheet(selectedPatient.id)
    : { prescriptions: [] };
  ...
  const totalPrescribed = prescriptions.reduce(
    (acc, curr) => acc + (curr?.hours?.length || 0),
    0,
  );
  const totalChecked = prescriptions.reduce(
    (acc, curr) =>
      acc + (curr?.hours?.filter((h) => h.status === "checked")?.length || 0),
    0,
  );
  const totalDelayed = prescriptions.reduce(
    (acc, curr) =>
      acc + (curr?.hours?.filter((h) => h.status === "delayed")?.length || 0),
    0,
  );
  const totalRefused = prescriptions.reduce(
    (acc, curr) =>
      acc + (curr?.hours?.filter((h) => h.status === "refused")?.length || 0),
    0,
  );
  const totalPending = prescriptions.reduce(
    (acc, curr) =>
      acc + (curr?.hours?.filter((h) => h.status === "pending")?.length || 0),
    0,
  );
    ...
    const updatedPrescriptions = prescriptions.map((p) => {
      if (p.id === prescriptionId) {
        ...
        updatedHours[hourIdx] = {
          ...
          nurseName: coSignatureSuccess
            ? `${nurseName} (Dupla Checagem: ${coSigningNurse})`
            : nurseName,
          vitalSigns:
            vitals.bp || vitals.hr || vitals.temp || vitals.glycemia
              ? { ...vitals }
              : undefined,
  ...
  const handleUndoCheck = () => {
    ...
    toast.success(
      `Administração de ${viewingCheckDetails.medName} revertida com sucesso.`,
    );
  ...
  const handleReportOccurrence = () => {
    ...
    toast.warning(
      `Ocorrência de ${newOccurrence.category} reportada no leito de ${selectedPatient.name}!`,
    );
  ...
  const handleSuspendPrescription = (prescriptionId?: string) => {
    ...
    if (targetPresc?.orderId) {
      ...
      toast.error(
        "Suspensão de medicamento do contexto não está implementada nesta view, precisa de permissão médica.",
      );
    } else {
      const updatedData = {
        ...patientData,
  ...
  const handleAddExtraDose = (prescriptionId?: string) => {
    ...
    const currentHourStr = new Date().toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    let isContextPrescription = false;
  ...
  const calculateHours = (startHour: string, frequency: string): string[] => {
    ...
    else if (frequency === "Contínuo" || frequency === "Dose única")
      return [startHour];

    for (let i = 0; i < 24; i += interval) {
      const nextH = (startH + i) % 24;
      hours.push(
        `${String(nextH).padStart(2, "0")}:${String(startM).padStart(2, "0")}`,
      );
    }
  ...
  const addPrescription = (e: React.FormEvent) => {
    ...
    // Check allergy!
    const isAllergic = selectedPatient.allergies
      ?.toLowerCase()
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <Badge
              variant="outline"
              className="text-[9px] font-black uppercase tracking-widest text-rose-500 border-rose-500/20"
            >
              Beira do Leito
            </Badge>
          </div>
          <h1 className="text-3xl font-black tracking-tight text-foreground uppercase mission-control-title">
            Checagem de Enfermagem
          </h1>
          <p className="text-muted-foreground font-medium mt-1">
            Aprazamento eletrônico e controle de segurança de medicações
            aplicadas.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          },
          {
            ...
            icon: Activity,
            value: selectedPatient?.triaged ? "0" : "5",
          ...
          },
          {
            ...
            value: totalDelayed,
            color:
              totalDelayed > 0 ? "text-amber-500" : "text-muted-foreground/40",
            bg:
              totalDelayed > 0 ? "bg-amber-500/10 animate-pulse" : "bg-muted/5",
          },
        ].map((stat, i) => (
          <Card
            key={i}
            className="p-4 glass-card-premium border-white/40 dark:border-white/10 shadow-lg relative overflow-hidden flex flex-col justify-between h-28 group hover:-translate-y-1 transition-all duration-300"
          >
            <div className="flex justify-between items-start">
              ...
              <div
                className={`h-8 w-8 rounded-xl ${stat.bg} ${stat.color} flex items-center justify-center shrink-0`}
              >
              ...
              <div className="flex flex-col gap-4">
                ...
                <Select value={filterSector} onValueChange={setFilterSector}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Amarela" className="font-bold text-xs">
                      OBS. AMARELA
                    </SelectItem>
                    <SelectItem value="Verde" className="font-bold text-xs">
                      OBS. VERDE
                    </SelectItem>
                    <SelectItem
                      value="Sala de Medicação"
                      className="font-bold text-xs"
                    >
                      SALA DE MEDICAÇÃO
                    </SelectItem>
          ...
          {selectedPatient ? (
            <Card className="glass-card-premium border-white/40 dark:border-white/10 shadow-xl overflow-hidden rounded-xl">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      >
                        {selectedPatient.computedSector} •{" "}
                        {selectedPatient.computedRoom}
                      </Badge>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  {prescriptions.map((p) => {
                    const hasAllergyConflict = selectedPatient.allergies
                      ?.toLowerCase()
                      .includes(p.medication.toLowerCase());
                    return (
                      <div
                        key={p.id}
                        className={cn(
                      ...
                      >
                        ...
                        <div
                          onClick={() =>
                            setViewingPrescription({ ...p, hasAllergyConflict })
                          }
                  ...
                  {prescriptions.length === 0 && (
                    <div className="p-12 border-2 border-dashed border-white/10 rounded-3xl text-center text-muted-foreground flex flex-col items-center">
                      ...
                      <p className="font-bold text-sm">
                        Nenhuma medicação aprazada para este paciente.
                      </p>
                      <p className="text-xs opacity-75 mt-1">
                        Utilize o botão superior para simular aprazamentos e
                        gerar os horários.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="pt-6 border-t border-border/50 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-black uppercase tracking-wider">
                          Feed de Ocorrências Clínicas Beira-Leito
                        </span>
                      </div>
                    ...
                    <div className="space-y-3">
                      {occurrences
                        ...
                        .map((o) => (
                          <div
                            key={o.id}
                            className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-1.5 animate-in fade-in duration-300"
                          >
                            <div className="flex items-center justify-between">
                      ...
                      {occurrences.filter(
                        ...
                        <p className="text-center py-6 text-xs text-muted-foreground font-medium bg-muted/25 rounded-2xl border border-dashed border-border/55">
                          Sem intercorrências registradas neste leito para o
                          plantão atual.
                        </p>
                      )}
                    </div>
            ...
            <Card className="p-12 border-2 border-dashed rounded-3xl text-center text-muted-foreground flex flex-col items-center justify-center min-h-[400px] glass-card">
              ...
              <p className="text-sm max-w-sm mt-2">
                Selecione um paciente ativo da lista lateral para ver sua ficha
                clínica de checagem beira-leito.
              </p>
            </Card>
          )}
        </div>
              ...
              <form onSubmit={addPrescription}>
                ...
                <div className="p-8 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      Medicação Comercial / Princípio Ativo{" "}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={wizardData.route}
                        onValueChange={(v) =>
                          setWizardData((prev) => ({ ...prev, route: v }))
                        }
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setWizardData((prev) => ({ ...prev, frequency: v }))
                        }
                      >
                        <SelectTrigger className="h-11 border-primary/20">
                          <SelectValue />
                        ...
                        <SelectContent>
                          <SelectItem value="4/4h">
                            A cada 4 horas (4/4h)
                          </SelectItem>
                          <SelectItem value="6/6h">
                            A cada 6 horas (6/6h)
                          </SelectItem>
                          <SelectItem value="8/8h">
                            A cada 8 horas (8/8h)
                          </SelectItem>
                          <SelectItem value="12/12h">
                            A cada 12 horas (12/12h)
                          </SelectItem>
            ...
            >
              ...
              <p className="text-sm font-bold text-red-100 leading-relaxed max-w-sm">
                ...
                <span className="text-white underline">
                  {allergyAlert.patientName}
                </span>{" "}
                possui alergia grave catalogada a{" "}
          ...
          <>
            ...
            >
              <div className="p-8 bg-gradient-to-br from-rose-500 to-rose-600 dark:from-rose-600/90 dark:to-rose-700/90 text-white relative border-b border-rose-450 dark:border-rose-950/20">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tight leading-none truncate text-white">
                  {activeCheckItem.medName}
                </h3>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-2 bg-muted/40 p-1.5 rounded-2xl border">
                  ...
                  <button
                    ...
                    className={cn(
                      "py-3 rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all",
                      checkAction === "refuse"
                        ? "bg-white dark:bg-slate-900 shadow-md text-red-500"
                        : "text-muted-foreground hover:text-foreground",
                    )}
                  >
                    <X className="h-4 w-4" />
                ...
                {checkAction === "check" && (
                  <div className="p-4 rounded-3xl bg-amber-500/5 border border-amber-500/10 space-y-3 animate-in fade-in duration-300">
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold">
                          {scannedPatient ? "✓ BIPADO" : "PENDENTE"}
                        </span>
                      </div>
                      <div
                        className={cn(
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setScanTarget({
                          medName: activeCheckItem.medName,
                    ...
                    >
                      <QrCode className="h-4 w-4" />
                      {scannedPatient && scannedMed
                        ? "Refazer Leitura Beira-Leito"
                        : "Iniciar Escaneamento HUD"}
                    </Button>
                  </div>
                )}
                ...
                {checkAction === "check" &&
                  (activeCheckItem.medName.includes("Ceftriaxona") ||
                    activeCheckItem.medName.includes("Insulina") ||
                    activeCheckItem.medName.includes("Soro")) && (
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <Label className="text-[9px] font-bold text-muted-foreground">
                              Selecionar Enfermeira Co-Responsável
                            </Label>
                            <Select
                              value={coSigningNurse}
                              onValueChange={setCoSigningNurse}
                            >
                              <SelectTrigger className="h-10 text-xs border-purple-200/50 dark:border-purple-950/20 bg-transparent">
                                <SelectValue placeholder="Selecione o profissional..." />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="Enf. Amanda Lemos (COREN 44521)">
                                  Enf. Amanda Lemos (COREN 44521)
                                </SelectItem>
                                <SelectItem value="Enf. Rodrigo Silva (COREN 12245)">
                                  Enf. Rodrigo Silva (COREN 12245)
                                </SelectItem>
                                <SelectItem value="Enf. Juliana Lemos (COREN 88523)">
                                  Enf. Juliana Lemos (COREN 88523)
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              toast.success(
                                `Co-assinatura efetuada por ${coSigningNurse}!`,
                              );
                            }}
                ...
                {checkAction === "check" ? (
                    ...
                    {activeCheckItem.medName.includes("Insulina") &&
                      ...
                      (() => {
                        ...
                        return (
                          ...
                          >
                            <span>
                              Calculadora de Protocolo Móvel de Glicemia
                            </span>
                            <span className="text-xs font-black">
                              Dosagem Recomendada: {ui} UI{" "}
                              {ui >= 10 && "⚠️ (AVISAR MÉDICO DO SETOR)"}
                            </span>
                          </div>
                        );
                      })()}
                  ...
                  <div className="space-y-2 animate-in fade-in duration-300">
                    <Label
                      ...
                      className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mr-1"
                    >
                      Justificativa Clínico-Administrativa{" "}
                      <span className="text-red-500">*</span>
                    </Label>
                    <Select
                      value={justification}
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="Paciente recusou a medicação">
                          Paciente recusou a medicação
                        </SelectItem>
                        <SelectItem value="Paciente dormindo">
                          Paciente dormindo
                        </SelectItem>
                        <SelectItem value="Medicação indisponível na farmácia">
                          Medicação indisponível na farmácia
                        </SelectItem>
                        <SelectItem value="Acesso venoso perdido - pendente novo acesso">
                          Acesso venoso perdido
                        </SelectItem>
                        <SelectItem value="Paciente em jejum para exames">
                          Paciente em jejum para exames
                        </SelectItem>
                        <SelectItem value="Suspenso sob conduta médica">
                          Suspenso sob conduta médica
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
          ...
          <>
            ...
            >
              ...
              >
                ...
                <h3 className="text-2xl font-black uppercase tracking-tight truncate leading-none mb-1 text-white">
                  {viewingCheckDetails.medName}
                </h3>
                <div className="flex items-center gap-2 mt-2">
                  <Badge
                    variant="outline"
                    className="bg-white/20 border-none text-white text-[9px] font-black uppercase tracking-wider py-0.5 px-2"
                  >
                    {viewingCheckDetails.dosage} {viewingCheckDetails.route}
                  </Badge>
                  <span className="text-xs font-bold text-white/90">
                    Horário Aprazado: {viewingCheckDetails.hourData.hour}
                  </span>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-muted/40 rounded-xl">
                      ...
                      <span className="text-xs font-bold text-foreground leading-tight block">
                        {viewingCheckDetails.hourData.nurseName}
                      </span>
                    </div>
                    <div className="p-3 bg-muted/40 rounded-xl">
                      <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground block mb-0.5">
                      ...
                      <span className="text-xs font-mono font-bold text-foreground block">
                        {viewingCheckDetails.hourData.checkedAt}
                      </span>
                    </div>
                  </div>
                    ...
                    ? viewingCheckDetails.hourData.vitalSigns && (
                        <div className="p-4 rounded-2xl bg-muted/40 border space-y-3">
                          <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block">
                            Parâmetros de Controle Coletados
                          </span>
                          <div className="grid grid-cols-2 gap-3">
          ...
          <>
            ...
            >
              <div className="p-8 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 text-foreground dark:text-white relative border-b border-slate-200 dark:border-white/10">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tight truncate leading-none mb-1 text-foreground dark:text-white">
                  {viewingPrescription.medication}
                </h3>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs font-bold text-muted-foreground dark:text-slate-300">
                    Dosagem: {viewingPrescription.dosage} • Frequência:{" "}
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="p-4 rounded-2xl bg-muted/40 border space-y-2">
                  ...
                  <p className="text-xs text-foreground/80 leading-relaxed font-medium">
                    Prescrito por **Dr. Ricardo Braga (CRM 12345)** às{" "}
                    {formatArrivalTime(selectedPatient?.arrivalTime)}. Diluir em
                    Soro Fisiológico se aplicável, e correr conforme
                    orientações. Em caso de Insulinas, monitorar glicemia
                    capilar e reportar taxas extremas.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block">
                    Cronograma de Horários de Hoje
                  </span>
                ...
                {(viewingPrescription.frequency === "Contínuo" ||
                  viewingPrescription.medication.includes("Soro")) &&
                  (() => {
                    const settings = pumpSettings[viewingPrescription.id] || {
                      velocity: 80,
                      vtbi: 500,
                      infused: 120,
                      status: "stopped",
                    ...
                    const handleUpdatePump = (
                      ...
                      toast.success(
                        `Configuração da Bomba de Infusão atualizada!`,
                      );
                    };
                    ...
                    return (
                      <div className="p-5 rounded-[2rem] bg-slate-900 border border-white/10 text-white space-y-4 animate-in fade-in duration-300">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                onClick={() =>
                                  handleUpdatePump({
                                    vtbi: Math.max(100, settings.vtbi - 50),
                                  })
                                }
                                className="h-8 w-8 bg-white/5 hover:bg-white/10 text-white rounded-lg border-white/10"
                              >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleUpdatePump({
                                    vtbi: Math.min(1000, settings.vtbi + 50),
                                  })
                                }
                                className="h-8 w-8 bg-white/5 hover:bg-white/10 text-white rounded-lg border-white/10"
                              >
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                          <Button
                            onClick={() =>
                              handleUpdatePump({ status: "running" })
                            }
                            className={cn(
                          ...
                          <Button
                            onClick={() =>
                              handleUpdatePump({ status: "paused" })
                            }
                            className={cn(
                              "flex-1 h-9 rounded-xl text-[9px] font-black uppercase tracking-wider text-white border border-transparent shadow-lg",
                              settings.status === "paused"
              ...
              <div className="w-full aspect-[4/3] rounded-3xl bg-slate-900 border border-white/10 relative overflow-hidden flex items-center justify-center">
                ...
                <div className="text-center p-6 space-y-2 z-20">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    DISPOSITIVO DE SEGURANÇA ATIVO
                  </p>
                  <p className="text-xs font-bold text-white uppercase">
                    {scanTarget.medName} • {scanTarget.patientName}
                  </p>
                </div>
              </div>
          ...
          <>
            ...
            >
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    Categoria da Intercorrência
                  </Label>
                  <Select
                    value={newOccurrence.category}
                    onValueChange={(val) =>
                      setNewOccurrence((prev) => ({ ...prev, category: val }))
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="Dor Intensa">
                        Dor Intensa (Escala de Dor)
                      </SelectItem>
                      <SelectItem value="Febre / Hipertermia">
                        Febre / Hipertermia
                      </SelectItem>
                      <SelectItem value="Vômito / Náusea">
                        Vômito / Náusea
                      </SelectItem>
                      <SelectItem value="Perda de Acesso Venoso">
                        Perda de Acesso Venoso
                      </SelectItem>
                      <SelectItem value="Suspeita de Reação Adversa">
                        Suspeita de Reação Adversa
                      </SelectItem>
                      <SelectItem value="Agitação Psicomotora">
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    Observação Clínica / Conduta Tomada
                  </Label>
                  <textarea
