...
export default function HR() {
  ...
  const [selectedStaffForDoc, setSelectedStaffForDoc] = useState<{
    name: string;
    role: string;
  } | null>(null);

  const [isDetailsModalOpen, setIsDetailsModalOpen] = useState(false);
  const [detailsModalTab, setDetailsModalTab] = useState<"details" | "history">(
    "details",
  );
  ...
  const filteredStaff = useMemo(() => {
    return staff.filter((member) => {
      ...
      const matchesCategory =
        activeCategory === "todos" || member.category === activeCategory;
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium">
            Gestão de escalas, plantões e corpo clínico
          </p>
        </div>
                ...
                <DialogHeader className="p-8 bg-primary text-white">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge className="bg-white/20 text-white border-none uppercase text-[9px] font-black tracking-widest">
                      Painel Administrativo Próximo Período
                    </Badge>
                  </div>
                  ...
                  <DialogDescription className="text-white/70 font-medium">
                    Configure a cobertura médica e assistencial para as próximas
                    24 horas.
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent className="rounded-xl border-slate-200/50 dark:border-slate-800/50 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm shadow-sm">
                              <SelectItem value="1">
                                Dr. Carlos Souza (Clínica)
                              </SelectItem>
                              <SelectItem value="2">
                                Enf. Ana Paula (Urgência)
                              </SelectItem>
                              <SelectItem value="3">
                                Dra. Beatriz Lima (Pediatria)
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {[
                          ...
                          <div
                            key={i}
                            className="flex items-center justify-between p-3 rounded-2xl bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm border border-border/50 shadow-sm"
                          >
                            ...
                            <span className="text-[10px] font-black font-mono text-muted-foreground bg-muted p-1 px-2 rounded-lg">
                              {item.time}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="rounded-3xl border border-red-500/20 bg-red-500/10 backdrop-blur-sm p-6 space-y-3">
                    ...
                    <p className="text-xs text-red-800/70 leading-relaxed font-medium">
                      A ala de **Urgência 2** está com cobertura parcial para o
                      turno noturno. É necessário remanejar ou convocar um
                      profissional de sobreaviso.
                    </p>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 mt-8">
        <Card className="glass-card-premium rounded-xl overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-sky-500/5">
          ...
          <CardContent className="flex items-center justify-between pb-6">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 bg-green-500/10 rounded-2xl flex items-center justify-center text-green-600 font-black text-xl border border-green-500/20 shadow-inner">
                {onShiftCount}
              </div>
        ...
        <Card className="glass-card-premium rounded-xl overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-sky-500/5">
          ...
          <CardContent className="flex items-center justify-between pb-6">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 bg-red-500/10 rounded-2xl flex items-center justify-center text-red-600 font-black text-xl border border-red-500/20 shadow-inner">
                {deficitCount}
              </div>
              <p className="text-sm font-medium text-foreground/80">
                Postos sem cobertura p/ noite
              </p>
            </div>
        ...
        <Card className="glass-card-premium rounded-xl overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-sky-500/5">
          ...
          <CardContent className="flex items-center justify-between pb-6">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-600 font-black text-xl border border-blue-500/20 shadow-inner">
                {new Set(staff.map((s) => s.specialty)).size}
              </div>
              <p className="text-sm font-medium text-foreground/80">
                Áreas cobertas no plantão
              </p>
            </div>
            ...
            <CardHeader className="p-8 border-b border-border/50">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant="outline"
                    className="h-10 rounded-xl font-black uppercase text-[9px] tracking-widest border-border/40"
                  >
                    Exportar
                  </Button>
                </div>
              ...
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="border-none">
                    <TableHead className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground pl-10">
                      Profissional
                    </TableHead>
                    <TableHead className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      Cargo/Especialidade
                    </TableHead>
                    <TableHead className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                      Treinamento
                    </TableHead>
                    <TableHead className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                      Documentos
                    ...
                    <TableHead className="p-5 text-right text-[10px] font-black uppercase tracking-widest text-muted-foreground pr-10">
                      Ações
                    </TableHead>
                  </TableRow>
                </TableHeader>
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 rounded-lg hover:bg-muted transition-colors opacity-40 group-hover:opacity-100"
                              >
                                <MoreHorizontal className="h-4 w-4" />
                              </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent
                              className="rounded-2xl p-2 glass-card-premium border-white/40 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] min-w-[160px] animate-in fade-in-0 zoom-in-95"
                              align="end"
                              sideOffset={8}
                            >
                              <DropdownMenuItem
                                className="rounded-xl font-bold text-xs uppercase cursor-pointer transition-colors focus:bg-black/5 dark:focus:bg-white/5 py-2.5 px-3"
                                onClick={() => {
                              ...
                              <DropdownMenuItem
                                className="rounded-xl font-bold text-xs uppercase text-red-600 focus:text-red-700 cursor-pointer transition-colors focus:bg-red-500/10 py-2.5 px-3"
                                onClick={() => {
                                  setSelectedStaffForDoc({
                                    name: member.name,
                                    role: member.role,
                                  });
              ...
              {notifications.map((n) => (
                <div
                  ...
                  className={cn(
                    ...
                    n.type === "warning"
                      ? "bg-red-500/10 border-red-500/20 text-red-700 dark:text-red-400 backdrop-blur-sm"
                      : "bg-blue-500/10 border-blue-500/20 text-blue-700 dark:text-blue-400 backdrop-blur-sm",
                  )}
                >
          ...
          <Card className="glass-card-premium rounded-xl overflow-hidden">
            ...
            <CardContent className="p-6 pt-2 space-y-4">
              <AnimatePresence>
                {pendingSwaps.slice(0, 3).map((req) => (
                  <motion.div
                    ...
                    key={req.id}
                    className="p-3 rounded-2xl bg-white/40 dark:bg-slate-800/25 border border-white/40 dark:border-slate-800/30 hover:bg-white/70 dark:hover:bg-slate-700/40 transition-all hover:shadow-lg hover:shadow-sky-500/5 group/req relative backdrop-blur-sm"
                  >
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge
                          variant="secondary"
                          className="text-[8px] font-black bg-white/50 dark:bg-slate-800"
                        >
                          {req.shift}
                        </Badge>
                      </div>
                      <div className="text-[11px] font-bold">
                        {req.from.split(" ")[1] || req.from.split(" ")[0]}{" "}
                        <ArrowRight className="inline h-2 w-2 opacity-30" />{" "}
                        {req.to.split(" ")[1] || req.to.split(" ")[0]}
                      </div>
                      <div className="flex gap-2 mt-1">
                        <Button
                          variant="outline"
                          className="h-7 px-2 text-[8px] font-black uppercase flex-1 border-white/50 dark:border-slate-700 bg-white/35 dark:bg-slate-800/35 hover:bg-white/60 dark:hover:bg-slate-700"
                          onClick={() => {
                            handleSwapAction(req.id, "approved");
                        ...
                        <Button
                          variant="ghost"
                          className="h-7 px-2 text-[8px] font-black uppercase flex-1 text-red-500 hover:bg-red-500/10"
                          onClick={() => {
                            handleSwapAction(req.id, "denied");
                            toast.error("Troca rejeitada");
                          }}
                        >
                          Negar
                        </Button>
              ...
              {pendingSwaps.length === 0 && (
                <div className="text-center p-4 text-xs font-bold text-muted-foreground">
                  Sem trocas pendentes
                </div>
              )}
              <Button
                variant="ghost"
                className="w-full h-8 text-[9px] font-black uppercase tracking-widest hover:bg-primary/5"
              ...
              <div className="space-y-4">
                <div className="flex items-end justify-between">
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-black tabular-nums tracking-tighter text-blue-950 dark:text-white drop-shadow-sm">
                      94
                    </span>
