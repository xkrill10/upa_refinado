...
export default function Patients() {
  ...
  const [activeFilter, setActiveFilter] = useState<
    "all" | "attending" | "waiting" | "completed"
  >("all");
    ...
    switch (risk) {
      case "emergency":
        return {
          label: "Emergência",
          color:
            "bg-red-500 text-white dark:bg-red-600/90 shadow-[0_0_15px_rgba(239,68,68,0.25)]",
        };
      case "very-urgent":
        return {
          label: "Muito Urgente",
      ...
      case "evasion":
        return {
          label: "Evasão",
          color: "bg-slate-400 text-white dark:bg-slate-500/90",
        };
      default:
    ...
    switch (status) {
      case "waiting":
        return {
          label: "Aguardando",
          color:
            "text-orange-600 bg-orange-500/10 border-orange-500/20 dark:text-orange-400 dark:bg-orange-550/15 dark:border-orange-500/30",
        };
      case "attending":
        return {
          label: "Em Atendimento",
          color:
            "text-[#006699] bg-[#006699]/10 border-[#006699]/20 dark:text-sky-400 dark:bg-sky-500/15 dark:border-sky-500/30",
        };
      case "completed":
        return {
          label: "Finalizado",
          color:
            "text-emerald-600 bg-emerald-500/10 border-emerald-500/20 dark:text-emerald-400 dark:bg-emerald-500/15 dark:border-emerald-500/30",
        };
      case "evasion":
        return {
          label: "Evasão",
          color:
            "text-slate-500 bg-slate-500/10 border-slate-500/20 dark:text-slate-400 dark:bg-slate-500/15 dark:border-slate-500/30",
        };
      default:
        return {
          label: status,
          color:
            "text-slate-550 bg-slate-500/10 border-slate-500/20 dark:text-slate-400 dark:bg-slate-500/15",
        };
    }
  };
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredPatients.map((p) => {
        ...
        const name = p.name.toUpperCase().includes("NÃO IDENTIFICADO")
  ...
  const attendingCount = patients.filter(
  ...
  const statsConfig = [
    {
      ...
      activeBorder:
        "border-[#006699]/40 dark:border-sky-500/40 ring-1 ring-[#006699]/10 dark:ring-sky-500/10 shadow-[0_0_20px_rgba(0,102,153,0.1)]",
      icon: Users,
    },
    {
      id: "attending",
      ...
      activeBorder:
        "border-blue-500/40 dark:border-blue-500/40 ring-1 ring-blue-500/10 dark:ring-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.1)]",
      icon: Activity,
    },
    {
      id: "waiting",
      ...
      activeBorder:
        "border-amber-500/40 dark:border-amber-550/40 ring-1 ring-amber-500/10 dark:ring-amber-500/10 shadow-[0_0_20px_rgba(245,158,11,0.1)]",
      icon: ClipboardList,
    },
    {
      id: "completed",
      ...
      activeBorder:
        "border-emerald-500/40 dark:border-emerald-500/40 ring-1 ring-emerald-500/10 dark:ring-emerald-500/10 shadow-[0_0_20px_rgba(16,185,129,0.1)]",
      icon: CheckCircle2,
    },
  ];
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statsConfig.map((stat) => (
          <div
            ...
            onClick={() => {
              setActiveFilter(
                stat.id as "all" | "attending" | "waiting" | "completed",
              );
              setCurrentPage(1);
            }}
            className={cn(
          ...
          >
            <div
              className={cn(
                "p-2.5 rounded-lg transition-transform duration-300 flex items-center justify-center shrink-0",
                stat.bgStyle,
                stat.textStyle,
                activeFilter === stat.id && "scale-110",
              )}
            >
            ...
            <div>
              ...
              <p className={cn("text-xl font-black mt-0.5", stat.textStyle)}>
                {stat.value}
              </p>
            </div>
          </div>
              ...
              <Table>
                <TableHeader className="bg-gradient-to-r from-white/40 to-white/10 dark:from-slate-900/40 dark:to-slate-900/10 backdrop-blur-md border-b border-white/40 dark:border-white/10">
                  <TableRow className="hover:bg-transparent border-none">
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest pl-6">
                      Paciente
                    </TableHead>
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest text-center">
                      Idade
                    </TableHead>
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest text-center">
                      Risco
                    </TableHead>
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest">
                      Setor
                    </TableHead>
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest text-center">
                      Status
                    </TableHead>
                    <TableHead className="text-[#006699] dark:text-sky-400 h-12 text-[10px] font-black uppercase tracking-widest text-right pr-6">
                      Ações Rápidas
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {paginatedPatients.length === 0 ? (
                    ...
                    paginatedPatients.map((patient) => {
                      ...
                      return (
                        <TableRow
                          key={patient.id}
                          className="border-b border-slate-200/30 dark:border-slate-800/30 hover:bg-slate-50/40 dark:hover:bg-slate-950/20 transition-colors group"
                        >
                          ...
                          <TableCell className="text-center">
                            <div className="flex justify-center">
                              <Badge
                                className={cn(
                                  risk.color,
                                  "text-[10px] font-black uppercase tracking-wider w-28 h-7 flex items-center justify-center border-0 rounded-lg shadow-sm whitespace-nowrap transition-transform hover:scale-105",
                                )}
                              >
                                {risk.label}
                              </Badge>
                          ...
                          <TableCell className="text-center">
                            <div className="flex justify-center">
                              <Badge
                                ...
                                className={cn(
                                  status.color,
                                  "font-black text-[10px] px-0 py-1 border rounded-lg whitespace-nowrap w-28 justify-center h-7",
                                )}
                              >
                                {status.label}
                              </Badge>
                                ...
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      toast.info(
                                        "Paciente sinalizado como Evasão",
                                      );
                                    }}
                                  >
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden rounded-2xl glass-card-premium border-white/40 dark:border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] [&>button]:hidden">
          <DialogHeader
            className={cn(
              ...
              selectedPatient?.risk === "emergency"
                ...
                : selectedPatient?.risk === "very-urgent"
                  ? "bg-orange-500"
                  : selectedPatient?.risk === "urgent"
                    ? "bg-amber-400 text-slate-950"
                    : selectedPatient?.risk === "less-urgent"
                      ? "bg-emerald-600"
                      : selectedPatient?.priority === "preferential"
                        ? "bg-purple-650 bg-purple-600"
                        : selectedPatient?.priority === "pediatric"
                          ? "bg-orange-600"
                          : "bg-[#006699]",
            )}
          >
          ...
          <div className="p-8 space-y-8 bg-transparent text-center">
            <div className="space-y-2">
                  ...
                  selectedPatient?.risk === "emergency"
                    ...
                    : selectedPatient?.risk === "very-urgent"
                      ...
                      : selectedPatient?.risk === "urgent"
                        ? "text-amber-500"
                        : selectedPatient?.risk === "less-urgent"
                          ? "text-emerald-550 text-emerald-500"
                          : selectedPatient?.priority === "preferential"
                            ? "text-purple-600"
                            : selectedPatient?.priority === "pediatric"
                              ? "text-orange-600"
              ...
              >
                {selectedPatient?.ticket || "S/N"}
              </h2>
              <p className="text-sm font-bold text-foreground uppercase">
                {selectedPatient?.name
                  .toUpperCase()
                  .includes("NÃO IDENTIFICADO") ||
                selectedPatient?.name.toUpperCase().includes("DESCONHECIDO")
                  ? "PACIENTE NÃO IDENTIFICADO"
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                className={cn(
                  ...
                  selectedPatient?.risk === "emergency"
                    ...
                    : selectedPatient?.risk === "very-urgent"
                      ...
                      : selectedPatient?.risk === "urgent"
                        ? "bg-amber-400 hover:bg-amber-500/90 shadow-amber-400/20 text-slate-950"
                        : selectedPatient?.risk === "less-urgent"
              ...
              >
                <div className="flex items-center gap-3 text-left">
                  <div
                    className={`p-2 rounded-lg ${isAudioEnabled ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-red-500/10 text-red-600"}`}
                  >
                    {isAudioEnabled ? (
