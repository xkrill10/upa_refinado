...
import StatCard from "@/pages/Escala/components/dashboard/StatCard";
import ShiftChart from "@/pages/Escala/components/dashboard/ShiftChart";
import DailyStaffChart from "@/pages/Escala/components/dashboard/DailyStaffChart";
import StatusPieChart from "@/pages/Escala/components/dashboard/StatusPieChart";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="text-left">
              ...
              <p className="text-[10px] text-muted-foreground">
                Histórico recente da escala e equipe
              </p>
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      if (isClickable) {
                        navigate(
                          `/pesquisar?q=${encodeURIComponent(log.employee_name)}`,
                        );
                      }
                    }}
                  ...
                  >
                    <div
                      className={`h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0 ${getIconBg(log.type)}`}
                    >
                      {getLogIcon(log.type)}
                    </div>
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <InfoRow
              label="Endereço"
              value="Rua Poços de Calda nº 66 - Jd. Santo Eduardo"
            />
            ...
            <InfoRow
              label="Resp. Técnica"
              value="Renata Ap. Bueno Pereira - COREN 484843"
            />
            <InfoRow
              label="Ger. Enfermagem"
              value="Paula Daniela Maciel - COREN 540838"
            />
        ...
        {activeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-card border border-border rounded-xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col z-10"
            >
              {/* Header */}
              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1">
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
                        Profissionais Cadastrados ({employees.length})
                      </h3>
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-full text-[9px] font-bold border ${
                                ...
                                emp.role === "SUPERVISÃO"
                                  ...
                                  : emp.role === "ENFERMEIRA" ||
                                      emp.role === "ENFERMEIRO"
                                    ? "bg-teal-100 text-teal-700 border-teal-300 dark:bg-teal-900/40 dark:text-teal-300"
                ...
                {activeModal === "presentes" && (
                  <>
                    ...
                    <div className="space-y-3">
                      <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
                        Profissionais em Plantão Hoje ({todayPresent})
                      </h3>
                      <div className="border border-border rounded-lg overflow-hidden divide-y divide-border max-h-[40vh] overflow-y-auto">
                        {todayPresentEmployees.length === 0 ? (
                          <div className="p-8 text-center text-muted-foreground">
                            Nenhum profissional escalado para hoje.
                          </div>
                        ) : (
                    ...
                    {onLeave.length === 0 ? (
                      <div className="space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                          Nenhum profissional afastado em licença no momento. A
                          UPA Zilda Arns está operando com força máxima! 🚀
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-3 text-left">
                        <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
                          Colaboradores em Afastamento ({onLeave.length})
                        </h3>
                            ...
                            >
                              <div>
                                ...
                                <span className="text-[10px] text-muted-foreground">
                                  Coren: {emp.coren} · Cargo: {emp.role}
                                </span>
                              </div>
                    ...
                    {certificates.length === 0 ? (
                      <div className="text-center py-6 space-y-2">
                        <p className="text-xs text-muted-foreground">
                          Nenhum atestado médico registrado neste mês de Junho
                          2026.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
                          Atestados Ativos ({certificates.length})
                        </h3>
                        <div className="border border-border rounded-lg overflow-hidden divide-y divide-border max-h-[40vh] overflow-y-auto">
                          {certificates.map((cert) => (
                            <div
                              key={cert.id}
                              className="p-3 hover:bg-muted/30 transition-colors space-y-1.5"
                            >
