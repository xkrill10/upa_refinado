...
const getPatientAvatar = (name?: string): string => {
  ...
  if (
    normalized.includes("ricardo braga") ||
    normalized.includes("joão mendes")
  ) {
  ...
  if (
    normalized.includes("ana oliveira") ||
    normalized.includes("maria clara")
  ) {
...
export default function NursingCheckRoom() {
  ...
  const activePatients = patients.filter(
    (p) => p.status === "attending" || p.status === "waiting",
  );
  ...
  useEffect(() => {
    ...
    if (!room || !nurse) {
      toast.error("Acesso negado", {
        description: "Por favor, assuma uma sala primeiro.",
      });
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-16">
      <NurseWorkspaceHeader
        activeRoom={
          localStorage.getItem("upa_active_room") || "SALA DE MEDICAÇÃO"
        }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium mt-1">
            Controle de pacientes, leitos e aprazamento eletrônico.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <Card
            key={i}
            className="p-4 glass-card border-white/15 dark:border-white/5 relative overflow-hidden flex flex-col justify-between h-28"
          >
