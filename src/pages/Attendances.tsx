...
export default function Attendances() {
  ...
  const [selectedRoom, setSelectedRoom] = useState<string>(() => {
    return (
      ...
      localStorage.getItem("selectedClinicalRoom") ||
  ...
  const classifiedPatients = patients.filter(
    (p) =>
  ...
  const gravePatients = classifiedPatients.filter(
  ...
  useEffect(() => {
    if (gravePatients.length > prevGraveCount.current) {
      toast.error("NOVO PACIENTE GRAVE AGUARDANDO!", {
        description:
          "Um novo paciente foi classificado com risco Emergência ou Muito Urgente.",
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          >
                            <p className="font-bold text-sm text-slate-800 dark:text-slate-100 uppercase tracking-tight group-hover/row:text-[#006699] dark:group-hover/row:text-sky-400 transition-colors">
                              {formatWords(patient.name)}
                            </p>
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[10px] text-slate-550 dark:text-slate-400 font-bold uppercase">
                                {patient.age} anos
                              </span>
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              <DialogDescription className="text-sm font-medium leading-relaxed px-2 text-slate-500 dark:text-slate-400">
                ...
                se retirou da unidade sem concluir o atendimento clínico.
              </DialogDescription>
            </div>
