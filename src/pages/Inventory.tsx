...
export default function Inventory() {
  ...
  // Stats calculation
  const totalStockItems = items.reduce(
  ...
  const getCategoryLabel = (cat: string) => {
    switch (cat) {
      case "consumables":
        return "Consumíveis";
      case "solutions":
        return "Soluções/Soros";
      case "needles_syringes":
        return "Seringas & Agulhas";
      case "diagnostics":
        return "Diagnósticos/Exames";
      default:
        return cat;
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <Badge
              variant="outline"
              className="text-[9px] font-black uppercase tracking-widest text-emerald-600 border-emerald-500/20"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          },
          {
            ...
            color: criticalItemsCount > 0 ? "text-red-500" : "text-emerald-500",
            bg: criticalItemsCount > 0 ? "bg-red-500/10" : "bg-emerald-500/10",
          },
          {
            title: "Requisições Pendentes",
            icon: ArrowRightLeft,
            value: pendingReqsCount,
            color: pendingReqsCount > 0 ? "text-amber-500" : "text-slate-450",
            bg: pendingReqsCount > 0 ? "bg-amber-500/10" : "bg-slate-500/10",
          },
        ].map((stat, i) => (
          <motion.div
            key={i}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-6 bg-black/5 dark:bg-white/5 border border-white/10 p-1 w-full justify-start overflow-x-auto rounded-2xl">
          ...
          <TabsTrigger
            value="pending"
            className="text-[10px] font-black uppercase tracking-widest rounded-xl text-amber-500 data-[state=active]:bg-amber-500/10"
          >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border pb-3">
                    <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      Material
                    </th>
                    <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      Categoria
                    ...
                    <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                      Estoque Atual
                    </th>
                    <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                      Estoque Mín
                    ...
                    <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">
                      Ação
                    </th>
                  </tr>
                </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4">
                          <Badge
                            variant="outline"
                            className="text-[9px] uppercase font-bold tracking-wider bg-slate-500/5 border-slate-500/20"
                          >
            ...
            {requisitions.filter((r) => r.status === "pending").length === 0 ? (
              <Card className="p-12 glass-card border-white/20 text-center flex flex-col items-center justify-center min-h-[250px] rounded-2xl">
                ...
                <p className="text-sm text-muted-foreground max-w-sm">
                  Todos os pedidos dos setores de enfermagem foram processados e
                  atendidos.
                </p>
              </Card>
            ) : (
                          ...
                          <div className="mt-5 pt-4 border-t border-border/45">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                              {req.items.map((item, idx) => (
                                <div
                                  key={idx}
                                  className="flex items-center justify-between p-2.5 rounded-lg bg-slate-500/5 border border-slate-500/10"
                                >
                ...
                {requisitions
                  ...
                  .map((req) => {
                    ...
                    return (
                      <Card
                        key={req.id}
                        className="p-5 glass-card border-white/20 rounded-2xl relative overflow-hidden opacity-90 hover:opacity-100 transition-opacity"
                      >
                        <div
                          className={`absolute top-0 left-0 w-1 h-full ${isApproved ? "bg-emerald-500" : "bg-red-500"}`}
                        />
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-3">
                              <Badge
                                className={`font-black text-[9px] uppercase border-none px-2 py-0 ${isApproved ? "bg-emerald-500 text-white" : "bg-red-500 text-white"}`}
                              >
                          ...
                          {req.processedAt && (
                            <div className="text-right text-[10px] text-muted-foreground font-bold uppercase tracking-wider">
                              ...
                              <br />
                              <span>
                                Por: <strong>{req.processedBy}</strong>
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="mt-4 pt-3 border-t border-border/30">
                          <div className="flex flex-wrap gap-2">
                            {req.items.map((item, idx) => (
                              <div
                                key={idx}
                                className="flex items-center gap-1.5 px-2 py-1 rounded bg-slate-500/5 border border-slate-500/10 text-xs font-semibold"
                              >
            ...
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-2 w-full">
                ...
                <DialogDescription className="text-sm font-medium text-slate-500 dark:text-slate-400">
                  Ajuste a quantidade em estoque para:
                  <br />
