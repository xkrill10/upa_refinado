...
function EscalaControl() {
  ...
  const getDayStatus = (p: { id: string; name: string; role: "nurse" | "technician"; shiftId: string; days?: Record<string, string> } | undefined, d: number) => {
    ...
    if (p.shiftId === "rt_lideranca") {
      ...
      const daysSinceAnchor = Math.floor((Date.UTC(selectedYear, selectedMonth - 1, d) - Date.UTC(2026, 5, 1)) / 86400000);
  ...
  const handleCellClick = (p: { id: string; name: string; role: "nurse" | "technician"; shiftId: string }, dayNum: number, currentStatus: string) => {
    if (isMonthLocked) {
      toast.error("Mês bloqueado! Destrave o mês para fazer alterações na escala.");
      return;
    }
    setEditingCell({
      memberId: p.id,
  ...
  const validateCellChange = (memberId: string, memberName: string, memberRole: string, shiftId: string, day: number, newStatus: string) => {
    ...
    if (!ABSENCE_STATUSES.includes(newStatus)) {
      if (newStatus === 'duty') {
        ...
        const daysSinceAnchor = Math.floor((Date.UTC(selectedYear, selectedMonth - 1, day) - Date.UTC(2026, 5, 1)) / 86400000);
        const isOddDay = Math.abs(daysSinceAnchor) % 2 === 0;
              ...
              <div className="flex flex-col h-full w-full">
                ...
                <div className="border-b bg-card px-4 py-2 flex items-center justify-between shadow-sm shrink-0 no-print overflow-x-auto flex-nowrap gap-4 whitespace-nowrap hide-scrollbar">
                  ...
                  <div className="flex items-center gap-2 flex-nowrap">
                    ...
                    <Dialog open={isRestoreMonthOpen} onOpenChange={setIsRestoreMonthOpen}>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Atenção! Esta ação irá limpar todas as edições manuais e folgas adicionadas neste mês, buscando o código da planilha original 12x36 implantado.
                            Deseja confirmar?
                          </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                          <Button variant="outline" onClick={() => setIsRestoreMonthOpen(false)}>Cancelar</Button>
                          <Button variant="destructive" onClick={handleRestoreMonth}>Sim</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                    ...
                    <Select value={globalShiftFilter} onValueChange={(val) => setGlobalShiftFilter(val)}>
                      ...
                      <SelectContent>
                        ...
                        {shifts.map((s) => {
                          ...
                          else if (s.id.includes("noturno")) icon = <MoonStar className="w-3 h-3 inline-block mr-1.5 text-slate-800 dark:text-slate-200 fill-slate-800 dark:text-slate-200 dark:fill-slate-200" />;
                          return (
                ...
                <div ref={printAreaRef} id="print-area-container" className="flex flex-col space-y-6 bg-background p-4 print:p-0 w-full max-w-full overflow-hidden">
                  ...
                  <div className="overflow-x-auto w-full max-w-full">
                    <table className="w-full border-collapse text-left text-xs">
                      ...
                      <thead>
                        <tr style={{ backgroundColor: 'transparent', borderBottom: '1px solid rgba(148, 163, 184, 0.2)' }}>
                          <th
                          ...
                          <th className="categoria-header" style={{ padding: 0, fontWeight: 700, color: 'inherit', textTransform: 'uppercase', borderRight: '1px solid rgba(148, 163, 184, 0.2)', width: '120px', minWidth: '120px', height: '45px' }}>
                            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <span className="pdf-content-shift">Categoria</span>
                            </div>
                          </th>
                          <th className="coren-header" style={{ padding: 0, fontWeight: 700, color: 'inherit', textTransform: 'uppercase', borderRight: '1px solid rgba(148, 163, 184, 0.2)', width: '100px', minWidth: '100px', height: '45px' }}>
                            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            return (
                              <th
                                ...
                                style={{
                                  ...
                                  minWidth: '36px',
                                  height: '45px',
                                  backgroundColor: isWeekend ? 'rgba(239,68,68,0.1)' : 'transparent',
                          ...
                          <th style={{ padding: 0, fontWeight: 700, color: 'inherit', textTransform: 'uppercase', borderRight: '1.5px solid rgba(148, 163, 184, 0.4)', width: '60px', minWidth: '60px', height: '45px' }}>
                            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              Ass.
                            </div>
                          </th>
                            ...
                            return filteredGlobals.map((p, index) => {
                              ...
                              return (
                                <React.Fragment key={p.id}>
                                  {showGroupHeader && (
                                    <tr className={`${groupColor}`} style={{ border: 'none' }}>
                                      <td 
                                        ...
                                        style={{ borderTop: 'none', borderBottom: 'none', borderLeft: 'none', borderRight: '1.5px solid rgba(148, 163, 184, 0.4)', padding: 0, height: '32px', verticalAlign: 'middle', textAlign: 'left' }}
                                      >
                                  ...
                                  <tr className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-all" style={{ borderBottom: '1px solid rgba(148, 163, 184, 0.2)' }}>
                                    {/* Name cell */}
                                    <td
                                    ...
                                    >
                                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: isGeneratingPDF ? 'center' : 'flex-start', paddingLeft: isGeneratingPDF ? '0px' : '12px', paddingRight: '12px' }}>
                                        <span style={{ display: 'block', fontSize: '11px', fontWeight: 900, color: 'inherit', lineHeight: 1.3, textAlign: isGeneratingPDF ? 'center' : 'left' }} title={p.name}>
                                          {p.name}
                                        </span>
                                      </div>
                                    </td>
                                    {/* Category badge cell */}
                                    <td className="categoria-cell" style={{ padding: 0, borderRight: '1px solid rgba(148, 163, 184, 0.2)', width: '120px', height: '40px' }}>
                                    ...
                                    {/* COREN cell */}
                                    <td className="coren-cell" style={{ padding: 0, borderRight: '1px solid rgba(148, 163, 184, 0.2)', width: '100px', height: '40px' }}>
                                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingLeft: '8px', paddingRight: '8px', fontFamily: 'monospace', fontSize: '10px', color: 'inherit', whiteSpace: 'nowrap' }}>
                                      ...
                                      return (
                                        <td
                                          ...
                                          style={{
                                            ...
                                            height: '40px',
                                            cursor: 'pointer',
                                            backgroundColor: isWeekend ? 'rgba(239,68,68,0.05)' : 'transparent',
                                          }}
                                        >
                                          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    ...
                                    <td style={{ padding: 0, borderRight: '1.5px solid rgba(148, 163, 184, 0.4)', width: '60px', height: '40px' }}>
                  ...
                  <div className="p-4 glass-panel border border-white/20 dark:border-white/5 rounded-xl flex flex-col gap-2 text-xs print-legend">
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-3 items-center mt-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Ímpar Diurno</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Ímpar Noturno</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Par Diurno</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Par Noturno</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Administrativo</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Folga Deferida</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Folga Sob Análise</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Folga Eleitoral</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Folga Abonada</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Banco de Horas</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Licença Médica</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 flex items-center justify-center text-[10px] rounded border font-extrabold" style={{ backgroundColor: '#f8fafc', borderColor: '#cbd5e1', color: 'inherit', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}><span className="pdf-inner-text">-</span></span>
                        <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">Folga de Escala</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-350 print-signature">
                    <div className="w-full flex items-center justify-between px-16">
                      <div className="flex flex-col items-start gap-1">
                        <span className="text-[10px] font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Data de Emissão:</span>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-1.5">
                        ...
                        >
                          ...
                          <Badge className="bg-slate-100 text-slate-700 dark:text-slate-300 dark:bg-slate-800 dark:text-slate-300 font-bold border-transparent text-[9px]">Padrão</Badge>
                        </button>
                  ...
                  >
                    ...
                    <SelectContent>
                      {shifts.map((group) => {
                        ...
                        else if (group.id.includes("noturno")) icon = <MoonStar className="w-3.5 h-3.5 inline-block mr-1.5 text-slate-800 dark:text-slate-200 fill-slate-800 dark:text-slate-200 dark:fill-slate-200" />;
            ...
            >
              ...
              <div className="space-y-2 w-full">
                <h3 className="text-sm font-black text-center text-card-foreground">
                  {isMonthLocked ? "Deseja destravar a escala mensal?" : "Deseja travar a escala mensal?"}
                </h3>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast.success(isMonthLocked ? "Escala mensal destravada." : "Escala mensal travada.");
                    }}
                    className="flex-1 text-xs h-9 bg-primary text-primary-foreground hover:bg-primary/95 shadow"
                  >
                    Confirmar
