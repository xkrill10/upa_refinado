      ...
      return (
        <div className="p-6 bg-red-500/10 border border-red-500 text-red-750 dark:text-red-400 rounded-xl space-y-4 m-6">
          <h2 className="text-lg font-bold">
            ⚠️ Ocorreu um erro ao carregar a página
          </h2>
...
function EscalaControl() {
  const { limits, codes, updateLimits, updateCodes, resetToDefault } =
    useScaleSettings();
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "request-portal" | "approval-desk" | "settings"
  >("overview");
  ...
  useEffect(() => {
    const loadFromDb = () => {
      Promise.all([
        ...
        db.entities.ScheduleEntry.filter({
          month: selectedMonth,
          year: selectedYear,
        }),
      ])
        .then(([employees, schedules]) => {
          const newShifts = initialShifts.map((s) => {
            ...
            if (dbShiftType === "rt_lideranca") {
              shiftEmployees = employees.filter(
                (e) =>
                  e.role === "RES.TECNICA" ||
                  e.role === "SUPERVISÃO" ||
                  e.name.toLowerCase().includes("maria eduarda"),
              );
            } else {
              shiftEmployees = employees.filter(
                (e) =>
                  e.shift_type === dbShiftType &&
                  e.role !== "RES.TECNICA" &&
                  e.role !== "SUPERVISÃO" &&
                  !e.name.toLowerCase().includes("maria eduarda"),
              );
            ...
            const staff = shiftEmployees.map((emp) => {
              const sched = schedules.find(
                (sch) => sch.employee_name?.trim() === emp.name?.trim(),
              );
              const isNurse =
                emp.role === "ENFERMEIRA" ||
                emp.role === "ENFERMEIRO" ||
                emp.role === "RES.TECNICA" ||
                emp.role === "SUPERVISÃO";
  ...
  >("all");
  const [globalShiftFilter, setGlobalShiftFilter] =
    useState<string>("impar_diurno");
  ...
  const handleRestoreMonth = () => {
    db.entities.ScheduleEntry.filter({
      ...
      Promise.all(
        ...
        toast.success(
          "Mês restaurado para o padrão 12x36 original com sucesso!",
        );
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const printOnlyElements = input.querySelectorAll(
        ".pdf-print-footer, .pdf-print-header",
      );
      const hiddenOriginalDisplays: string[] = [];

      // Backup original styles
      const originalInputStyle = input.style.cssText;
      const originalTableWrapperStyle = tableWrapper
        ? tableWrapper.style.cssText
        : "";
      const originalBorderWrapperStyle = borderWrapper
      ...
      if (borderWrapper)
        borderWrapper.style.cssText = originalBorderWrapperStyle;

      // Restore parent overflow and positioning
      parentsToUnclip.forEach(
        ({ el, overflow, position, transform, display, left, top }) => {
  ...
  const handleDaySelect = (dayNum: number) => {
    ...
    toast.info(
      `Slide de Escala: Visualizando Dia ${dayNum} de ${getMonthName(selectedMonth)} (${dayIsOdd ? "ÍMPAR" : "PAR"})`,
    );
  };
    ...
    (shifts || []).forEach((s) => {
      (s.staff || []).forEach((member) => {
        arr.push({
          ...
          role: member.role,
          roleCategory:
            (member as any).roleCategory ||
            (member.role === "nurse" ? "ENFERMEIRA" : "TEC.ENF"),
  ...
  const getDayStatus = (
    ...
    if (p.days && p.days[String(d)]) {
      ...
      if (
        ...
        return "LM"; // Licença Médica / Atestado
      if (val === "FE") return "FE";
      if (val === "FA") return "FA";
      if (val === "BH") return "BH";
      return "off-duty";
    ...
    if (p.shiftId === "rt_lideranca") {
      ...
      // Cálculo contínuo baseado em data âncora (01/06/2026) para preservar o ciclo 12x36
      const daysSinceAnchor = Math.floor(
        (Date.UTC(selectedYear, selectedMonth - 1, d) - Date.UTC(2026, 5, 1)) /
          86400000,
      );
      const isOddDay = Math.abs(daysSinceAnchor) % 2 === 0;
      const isShiftOdd =
        p.shiftId === "impar_diurno" ||
        p.shiftId === "diurno_a" ||
        p.shiftId === "impar_noturno" ||
        p.shiftId === "noturno_a";
      const isShiftEven =
        p.shiftId === "par_diurno" ||
        p.shiftId === "diurno_b" ||
        p.shiftId === "par_noturno" ||
        p.shiftId === "noturno_b";
      isScheduledWorkDay =
    ...
    switch (shiftId) {
      ...
      case "diurno_a":
        return {
          ...
          label: "Ímpar Diurno",
          iconNode: (
            <Sun className="w-3.5 h-3.5 inline-block mr-1 text-amber-500 fill-amber-500" />
          ),
          badge:
      ...
      case "diurno_b":
        return {
          ...
          label: "Par Diurno",
          iconNode: (
            <Sun className="w-3.5 h-3.5 inline-block mr-1 text-blue-500 fill-blue-500" />
          ),
          badge:
      ...
      case "noturno_a":
        return {
          ...
          label: "Ímpar Noturno",
          iconNode: (
            <MoonStar className="w-3.5 h-3.5 inline-block mr-1 text-purple-600 fill-purple-600" />
          ),
          badge:
      ...
      case "noturno_b":
        return {
          ...
          label: "Par Noturno",
          iconNode: (
            <MoonStar className="w-3.5 h-3.5 inline-block mr-1 text-indigo-600 fill-indigo-600" />
          ),
          badge:
      ...
      case "rt_lideranca":
        return {
          ...
          label: "RT / Liderança",
          iconNode: (
            <Briefcase className="w-3.5 h-3.5 inline-block mr-1 text-emerald-600" />
          ),
          badge:
  ...
  const exportToExcel = () => {
    ...
    filteredGlobals.forEach((p, index) => {
      ...
      for (let d = 1; d <= totalDays; d++) {
        ...
        if (dayStatus === "duty") {
          cellText = "P";
          if (p.shiftId === "impar_diurno" || p.shiftId === "diurno_a")
            cellClass = "class='duty-amber'";
          else if (p.shiftId === "par_diurno" || p.shiftId === "diurno_b")
            cellClass = "class='duty-blue'";
          else if (p.shiftId === "impar_noturno" || p.shiftId === "noturno_a")
            cellClass = "class='duty-purple'";
          else if (p.shiftId === "par_noturno" || p.shiftId === "noturno_b")
  ...
  const handleCellClick = (
    ...
    if (isMonthLocked) {
      toast.error(
        "Mês bloqueado! Destrave o mês para fazer alterações na escala.",
      );
      return;
    }
    setEditingCell({
  ...
  const validateCellChange = (
    ...
    if (!ABSENCE_STATUSES.includes(newStatus)) {
      if (newStatus === "duty") {
        // Cálculo contínuo baseado em data âncora (01/06/2026)
        const daysSinceAnchor = Math.floor(
        ...
        if ((isImparShift && !isOddDay) || (isParShift && isOddDay)) {
          return {
            ok: true,
            type: "warning",
            msg: `⚠️ Você está lançando um Plantão Extra no Dia ${day}, que é o dia de folga padrão do ciclo 12x36.\n\nDeseja confirmar este plantão extra?`,
          };
        }
      }
      return { ok: true };
    ...
    const sameShiftMembers = allProfessionals.filter(
      (p) => p.shiftId === shiftId && p.id !== memberId,
    );
    ...
    if (TEC_ROLES.includes(memberRole)) {
      ...
      if (tecAbsences >= maxTec) {
        return {
          ok: false,
          msg: `⚠️ Dia ${day}: limite de ${maxTec} folgas de Téc./Aux. já atingido neste plantão (${shiftId.replace("_", " ").toUpperCase()}).\nTente outro dia disponível.`,
        };
      }
    }
    ...
    if (ENF_ROLES.includes(memberRole)) {
      ...
      if (enfAbsences >= maxEnf) {
        return {
          ok: false,
          msg: `⚠️ Dia ${day}: já existe o limite de ${maxEnf} folga(s) de Enfermeiro(a) neste plantão.\nEscolha outro dia para lançar a folga.`,
        };
      }
    }
    ...
    if (newStatus === "FE" || newStatus === "FA") {
      ...
      if (currentDayStatus !== "duty") {
        return {
          ok: false,
          msg: `⚠️ ${newStatus === "FE" ? "Folga Eleitoral (FE)" : "Folga Aniversário (FA)"} só pode substituir um dia de Plantão (P).\nO dia ${day} não é Plantão para ${memberName}.`,
        };
      }
    }
    ...
    if (newStatus === "BH" || newStatus === "LM") {
      ...
      if (currentDayStatus !== "duty") {
        const label =
          newStatus === "BH" ? "Banco de Horas (BH)" : "Licença Médica (LM)";
        return {
          ok: false,
          msg: `⚠️ ${label} só pode ser lançado em dia de Plantão (P).\nO dia ${day} não é Plantão para ${memberName}.`,
        };
      }
    }
    ...
    if (newStatus === "FE" || newStatus === "FA") {
      ...
      if (extraCount >= 2) {
        return {
          ok: false,
          msg: `⚠️ Limite de 2 folgas extras (FE/FA) por mês já atingido para ${memberName}.\nNão é possível adicionar mais folgas especiais este mês.`,
        };
      }
    }
  ...
  const requestSaveCellEdits = () => {
    ...
    const shiftName =
      shifts.find((s) => s.id === editingShiftId)?.name || editingShiftId;
  ...
  const saveCellEdits = (overrideData?: {
    ...
    db.entities.ScheduleEntry.filter({ month: 6, year: 2026 }).then(
      (schedules) => {
        const sched = schedules.find(
          (s) => s.employee_name?.trim() === memberName?.trim(),
        );
        if (sched) {
          let newStatus = "F";
          if (status === "duty") newStatus = "P";
          else if (status === "leave-approved" || status === "leave-toggled")
            newStatus = "F";
          ...
          db.entities.ScheduleEntry.update(sched.id, {
            days: updatedDays,
          }).then(() => {
            queryClient.invalidateQueries({ queryKey: ["schedules"] });
            window.dispatchEvent(new Event("escala-db-updated"));
  ...
  const handleToggleRole = (
    ...
    setShifts((prev) =>
      prev.map((s) => {
        ...
        return {
          ...
          staff: s.staff.map((m) =>
            m.id === id
              ? { ...m, role: currentRole === "nurse" ? "technician" : "nurse" }
              : m,
          ),
        };
      }),
    ...
    toast.success(
      currentRole === "nurse"
        ? `Função alterada para Técnico de Enfermagem.`
        : `Função alterada para Enfermeiro Supervisor.`,
    );
  };
    ...
    setShifts((prev) => {
      ...
      const updatedShifts = prev.map((s) => {
        if (s.id === transferConfirm.currentShiftId) {
          ...
          return {
            ...s,
            staff: s.staff.filter((m) => m.id !== transferConfirm.id),
          };
        }
        return s;
      });
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 pt-2">
            <Badge
              variant="outline"
              className="bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20 text-[10px] font-bold tracking-wider uppercase"
            >
              Recursos Humanos & Escalabilidade
            </Badge>
            <Badge
              variant="outline"
              className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 text-[10px] font-bold tracking-wider uppercase"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="glass-card-premium flex-shrink-0 bg-gradient-to-br from-amber-500/5 via-transparent to-transparent shadow-sm relative overflow-hidden transition-all duration-300 hover:scale-[1.02] border border-border/60 hover:border-amber-500/30">
          <CardContent className="p-4 flex items-center justify-between gap-4 h-full">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground font-medium leading-snug">
                Limite por plantão de{" "}
                <strong className="text-foreground font-black">
                  {limits?.technicians || 3} Técnicos
                </strong>{" "}
                e{" "}
                <strong className="text-foreground font-black">
                  {limits?.nurses || 1} Enfermeiro
                </strong>{" "}
                em folga.
              </p>
            </div>
        ...
        <Card className="glass-card-premium flex-shrink-0 bg-gradient-to-br from-indigo-500/5 via-transparent to-transparent shadow-sm relative overflow-hidden transition-all duration-300 hover:scale-[1.02] border border-border/60 hover:border-indigo-500/30">
          <CardContent className="p-4 flex items-center justify-between gap-4 h-full">
            <div className="space-y-1">
              <span className="text-[9px] uppercase font-bold text-muted-foreground tracking-wider block">
              ...
              <p className="text-[11px] text-muted-foreground font-medium leading-snug">
                ...
                aguardam validação final da gestão.
              </p>
            </div>
        ...
        <Card className="glass-card-premium flex-shrink-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-transparent shadow-sm relative overflow-hidden transition-all duration-300 hover:scale-[1.02] border border-border/60 hover:border-emerald-500/30">
          <CardContent className="p-4 flex items-center justify-between gap-4 h-full">
            <div className="space-y-1">
              <span className="text-[9px] uppercase font-bold text-muted-foreground tracking-wider block">
              ...
              <div className="flex flex-col h-full w-full">
                ...
                <div className="border-b bg-card px-4 py-2 flex items-center justify-between shadow-sm shrink-0 no-print overflow-x-auto flex-nowrap gap-4 whitespace-nowrap hide-scrollbar">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="hidden sm:flex gap-2 items-center h-9">
                        <button
                          ...
                          className={cn(
                            "text-[11px] px-4 h-full rounded-full transition-all duration-300 ease-in-out border",
                            globalShiftFilter === "all"
                              ? "font-bold bg-blue-500 text-white shadow-md border-blue-500 scale-[1.02]"
                              : "font-semibold bg-blue-50 text-blue-600 border-blue-200/50 hover:bg-blue-100 hover:border-blue-300 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20",
                          )}
                        >
                          Todos os Turnos
                        </button>
                        <button
                          ...
                          className={cn(
                            "text-[11px] px-4 h-full rounded-full transition-all duration-300 ease-in-out border",
                            globalShiftFilter === "impar_diurno"
                              ? "font-bold bg-amber-500 text-white shadow-md border-amber-500 scale-[1.02]"
                              : "font-semibold bg-amber-50 text-amber-600 border-amber-200/50 hover:bg-amber-100 hover:border-amber-300 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20",
                          )}
                        >
                          Diurno A
                        </button>
                        <button
                          ...
                          className={cn(
                            "text-[11px] px-4 h-full rounded-full transition-all duration-300 ease-in-out border",
                            globalShiftFilter === "impar_noturno"
                              ? "font-bold bg-purple-600 text-white shadow-md border-purple-600 scale-[1.02]"
                              : "font-semibold bg-purple-50 text-purple-600 border-purple-200/50 hover:bg-purple-100 hover:border-purple-300 dark:bg-purple-600/10 dark:text-purple-400 dark:border-purple-500/20",
                          )}
                        >
                          Noturno A
                        </button>
                        <button
                          ...
                          className={cn(
                            "text-[11px] px-4 h-full rounded-full transition-all duration-300 ease-in-out border",
                            globalShiftFilter === "par_diurno"
                              ? "font-bold bg-emerald-500 text-white shadow-md border-emerald-500 scale-[1.02]"
                              : "font-semibold bg-emerald-50 text-emerald-600 border-emerald-200/50 hover:bg-emerald-100 hover:border-emerald-300 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20",
                          )}
                        >
                          Diurno B
                        </button>
                        <button
                          ...
                          className={cn(
                            "text-[11px] px-4 h-full rounded-full transition-all duration-300 ease-in-out border",
                            globalShiftFilter === "par_noturno"
                              ? "font-bold bg-rose-500 text-white shadow-md border-rose-500 scale-[1.02]"
                              : "font-semibold bg-rose-50 text-rose-600 border-rose-200/50 hover:bg-rose-100 hover:border-rose-300 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20",
                          )}
                        >
                          Noturno B
                        </button>
                  ...
                  <div className="flex items-center gap-2 flex-nowrap">
                    <Button
                      ...
                      className="gap-1.5 h-8 px-3 text-xs bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white dark:bg-blue-500/10 dark:text-blue-400 dark:hover:bg-blue-500 shadow-sm hover:shadow-md transition-all duration-300 mr-2 font-bold rounded-lg"
                    >
                      <Undo2 className="h-3.5 w-3.5" /> Voltar
                    </Button>

                    <Dialog
                      open={isRestoreMonthOpen}
                      onOpenChange={setIsRestoreMonthOpen}
                    >
                      <DialogTrigger asChild>
                        <Button
                          variant="default"
                          size="sm"
                          className="gap-1.5 h-8 px-3 text-xs bg-orange-50 text-orange-700 hover:bg-orange-600 hover:text-white dark:bg-orange-500/10 dark:text-orange-400 dark:hover:bg-orange-500 shadow-sm hover:shadow-md transition-all duration-300 font-bold rounded-lg mr-2"
                        >
                          <RotateCcw className="h-3.5 w-3.5" /> Restaurar Mês
                        </Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Reiniciar o Mês</DialogTitle>
                          <DialogDescription>
                            Atenção! Esta ação irá limpar todas as edições
                            manuais e folgas adicionadas neste mês, buscando o
                            código da planilha original 12x36 implantado. Deseja
                            confirmar?
                          </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                          <Button
                            variant="outline"
                            onClick={() => setIsRestoreMonthOpen(false)}
                          >
                            Cancelar
                          </Button>
                          <Button
                            variant="destructive"
                            onClick={handleRestoreMonth}
                          >
                            Sim
                          </Button>
                        </DialogFooter>
                    ...
                    {isUnlocked ? (
                      <Button
                        ...
                        onClick={() => {
                          setIsUnlocked(false);
                          toast.info("Escala bloqueada novamente.");
                        }}
                        className="gap-1.5 h-8 px-3 text-xs bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white dark:bg-emerald-500/10 dark:text-emerald-400 dark:hover:bg-emerald-500 shadow-sm hover:shadow-md transition-all duration-300 hidden md:flex font-bold rounded-lg"
                        title="Escala destravada — clique para bloquear novamente"
                      >
                        <Lock className="h-3.5 w-3.5" /> Travar
                      </Button>
                    ) : (
                      <Button
                        variant="default"
                    ...
                    >
                      <Download
                        className={`h-3.5 w-3.5 ${isGeneratingPDF ? "animate-pulse" : ""}`}
                      />
                    ...
                    {isMonthLocked ? (
                      <Button
                        ...
                        className="gap-1.5 h-8 px-3 text-xs bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white dark:bg-emerald-500/10 dark:text-emerald-400 dark:hover:bg-emerald-500 shadow-sm hover:shadow-md transition-all duration-300 hidden xl:flex font-bold rounded-lg"
                      >
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => setIsMonthLockModalOpen(true)}
                        className="gap-1.5 h-8 px-3 text-xs bg-red-50 text-red-700 hover:bg-red-600 hover:text-white dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500 shadow-sm hover:shadow-md transition-all duration-300 hidden xl:flex font-bold rounded-lg"
                      >
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 no-print">
                    ...
                    <div
                      className={cn(
                        "rounded-xl p-3 border shadow flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-all hover:scale-[1.03]",
                        globalShiftFilter === "impar_diurno"
                          ? "bg-amber-500 border-amber-600 shadow-amber-200 dark:shadow-amber-900"
                          : "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800",
                      )}
                      onClick={() =>
                        setGlobalShiftFilter(
                          globalShiftFilter === "impar_diurno"
                            ? "all"
                            : "impar_diurno",
                        )
                    ...
                    <div
                      className={cn(
                        "rounded-xl p-3 border shadow flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-all hover:scale-[1.03]",
                        globalShiftFilter === "impar_noturno"
                          ? "bg-purple-600 border-purple-700 shadow-purple-200 dark:shadow-purple-900"
                    ...
                    <div
                      className={cn(
                        "rounded-xl p-3 border shadow flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-all hover:scale-[1.03]",
                        globalShiftFilter === "par_noturno"
                          ? "bg-rose-500 border-rose-600 shadow-rose-200 dark:shadow-rose-900"
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          {shifts.map((s) => {
                            let icon = (
                              <Sun className="w-3 h-3 inline-block mr-1.5 text-amber-500 fill-amber-500" />
                            );
                            if (s.id === "rt_lideranca")
                              icon = (
                                <Briefcase className="w-3 h-3 inline-block mr-1.5 text-slate-500" />
                              );
                            else if (s.id.includes("noturno"))
                              icon = (
                                <MoonStar className="w-3 h-3 inline-block mr-1.5 text-slate-800 dark:text-slate-200 fill-slate-800 dark:text-slate-200 dark:fill-slate-200" />
                              );
                            return (
                  ...
                  {isExcelMode && (
                    <div className="flex items-center gap-2 bg-[#f3f4f6] dark:bg-slate-900 border border-[#c0c0c0] dark:border-slate-850 p-2 rounded-xl text-xs font-mono select-none shadow-sm no-print">
                      {/* Name/Address Box */}
                      <div className="glass-panel border border-white/20 dark:border-white/5 px-3 py-1.5 text-center font-black text-emerald-600 min-w-[70px] rounded-lg shadow-inner">
                        {selectedExcelCell
                      ...
                      <div className="flex-1 glass-panel border border-white/20 dark:border-white/5 px-3 py-1.5 rounded-lg flex items-center shadow-inner overflow-hidden text-slate-800 dark:text-slate-200 truncate">
                        {selectedExcelCell ? (
                  ...
                  <div
                    ...
                    className="flex flex-col space-y-6 bg-background p-4 print:p-0 w-full max-w-full overflow-hidden"
                  >
                    {/* Print Only Header */}
                    <div className="pdf-print-header hidden force-print-flex items-center justify-between border-b border-slate-300 pb-4 mb-4 w-full">
                      ...
                      <div className="flex-grow text-center flex flex-col items-center justify-center">
                        <h1 className="text-[14px] font-black uppercase text-slate-800 dark:text-slate-200 tracking-wider">
                        ...
                        <h2 className="text-[10px] font-semibold text-slate-700 dark:text-slate-300 mt-0.5">
                          Rua Poços de Caldas nº 65 - Jardim Santo Eduardo - CEP
                          06823-310
                        </h2>
                        <div className="text-[11px] font-extrabold text-slate-800 dark:text-slate-200 mt-1.5 flex items-center justify-center gap-10">
                      ...
                      <div className="overflow-x-auto w-full max-w-full">
                        <table className="w-full border-collapse text-left text-xs">
                          ...
                          <thead>
                            ...
                            >
                              <th
                                className="glass-panel colaborador-header text-slate-800 dark:text-slate-200"
                                style={{
                                  padding: 0,
                                  fontWeight: 700,
                                  color: "inherit",
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    display: "flex",
                                    alignItems: "center",
                                    justifyContent: isGeneratingPDF
                                      ? "center"
                                      : "flex-start",
                                    paddingLeft: isGeneratingPDF
                                      ? "0px"
                                      : "12px",
                                  }}
                                >
                                  <span className="pdf-content-shift">
                                    Colaborador
                                  </span>
                                </div>
                              </th>
                                  ...
                                  <th
                                    ...
                                    style={{
                                      ...
                                      backgroundColor: isWeekend
                                        ? "rgba(239,68,68,0.1)"
                                        : "transparent",
                                      color: isWeekend ? "#ef4444" : "inherit",
                                    }}
                                  >
                                    <div
                                      style={{
                          ...
                          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                            {filteredGlobals.length === 0 ? (
                              <tr>
                                ...
                                >
                                  Nenhum profissional assistencial corresponde
                              ...
                              (() => {
                                const STATUS_STYLES = {
                                  ...
                                  "leave-approved": {
                                    ...
                                    text: "text-gray-650 dark:text-gray-300",
                                    border:
                                  ...
                                  BH: {
                                    ...
                                    text: "text-violet-750 dark:text-violet-300",
                                    border:
                                  ...
                                  FI: {
                                    ...
                                    text: "text-red-850 dark:text-red-200",
                                ...
                                return filteredGlobals.map((p, index) => {
                                  ...
                                  const prevRole = prevEntry
                                    ? prevEntry.roleCategory ||
                                      (prevEntry.role === "nurse"
                                        ? "ENFERMEIRA"
                                        : "TEC.ENF")
                                    : null;
                                  const prevOrder = prevRole
                                    ? getRoleOrder(prevRole)
                                    : -1;
                                  const currOrder = getRoleOrder(role);

                                  const showGroupHeader =
                                    index === 0 || currOrder !== prevOrder;
                                  const groupLabel =
                                    currOrder === 0
                                  ...
                                  return (
                                    <React.Fragment key={p.id}>
                                      ...
                                      >
                                        {/* Name cell */}
                                        <td
                                          className="glass-panel colaborador-cell text-slate-800 dark:text-slate-200"
                                          style={{
                                            padding: 0,
                                            borderRight:
                                              "1px solid rgba(148, 163, 184, 0.2)",
                                        ...
                                        >
                                          ...
                                          >
                                            <span
                                              className={`inline-flex items-center justify-center px-2 py-0.5 rounded text-[9px] font-semibold border ${roleBadgeColor(p.roleCategory || p.role)}`}
                                              style={{
                                                display: "inline-flex",
                                                alignItems: "center",
                                                justifyContent: "center",
                                        ...
                                        {daysArray.map((d) => {
                                          ...
                                          const style =
                                            STATUS_STYLES[dayStatus] ||
                                            STATUS_STYLES["off-duty"];
                                          ...
                                          return (
                                            ...
                                            >
                                              ...
                                              >
                                                <span
                                                  className={`inline-flex items-center justify-center h-5 w-5 rounded text-[9px] font-semibold border ${style.bg} ${style.text} ${style.border}`}
                                                  style={{
                                                    display: "inline-flex",
                                                    alignItems: "center",
                                                    justifyContent: "center",
                    ...
                    <div
                      className="pdf-print-footer mt-6 space-y-6"
                      style={{ display: "none" }}
                    >
                      {/* Legend Replica for Printing */}
                      <div className="p-4 glass-panel border border-white/20 dark:border-white/5 rounded-xl flex flex-col gap-2 text-xs print-legend">
                        <strong className="text-slate-800 dark:text-slate-200 text-[11px] uppercase font-black">
                          Legenda e Código de Escala:
                        </strong>
                        <div className="flex flex-wrap gap-x-6 gap-y-3 items-center mt-1">
                          <div className="flex items-center gap-2">
                            <span
                              className="w-5 h-5 flex items-center justify-center text-[10px] rounded font-black text-white"
                              style={{
                            ...
                            >
                              <span className="pdf-inner-text">P</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Ímpar Diurno
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className="pdf-inner-text">F</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Folga Deferida
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className="pdf-inner-text">?</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Folga Sob Análise
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className="w-5 h-5 flex items-center justify-center text-[9.5px] rounded font-black text-white"
                              style={{
                            ...
                            >
                              <span className="pdf-inner-text">FE</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Folga Eleitoral
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className="pdf-inner-text">FA</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Folga Abonada
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className="pdf-inner-text">BH</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Banco de Horas
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className="pdf-inner-text">LM</span>
                            </span>
                            <span className="text-slate-800 dark:text-slate-200 text-[10px] font-semibold">
                              Licença Médica
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className="w-5 h-5 flex items-center justify-center text-[10px] rounded border font-extrabold"
                              style={{
                      ...
                      <div className="pt-6 border-t border-slate-350 print-signature">
                        <div className="w-full flex items-center justify-between px-16">
                          <div className="flex flex-col items-start gap-1">
                            ...
                            <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 border-b border-dotted border-black w-32 h-5 text-center mt-1">
                              ____ / ____ / ________
                            </span>
                          </div>
                          ...
                          <div className="flex flex-col items-center gap-1.5 min-w-[280px]">
                            ...
                            <span className="text-[11.5px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-wider text-center">
                  ...
                  {isAuthModalOpen && (
                    <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0, scale: 0.95 }}
                        className="relative w-full max-w-sm glass-card-premium rounded-xl shadow-2xl p-6 overflow-hidden"
                      >
                        <button
                        ...
                        <div className="flex flex-col items-center text-center gap-4">
                          ...
                          <div className="space-y-1">
                            <h3 className="text-base font-bold text-card-foreground">
                              Liberar Folgas Extras
                            </h3>
                            <p className="text-xs text-muted-foreground">
                              Insira as credenciais administrativas para
                              habilitar os itens verdes da escala.
                            </p>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-1.5">
                        ...
                        >
                          ...
                          <Badge className="bg-slate-100 text-slate-700 dark:text-slate-300 dark:bg-slate-800 dark:text-slate-300 font-bold border-transparent text-[9px]">
                            Padrão
                          </Badge>
                        </button>
        ...
        {pendingConfirm && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            >
              {/* Ícone */}
              <div
                className={`mx-auto h-12 w-12 rounded-full flex items-center justify-center ${pendingConfirm.type === "warning" ? "bg-amber-500/10" : "bg-primary/10"}`}
              >
              ...
              <div className="space-y-2 w-full">
                <h3
                  className={`text-sm font-black text-center ${pendingConfirm.type === "warning" ? "text-amber-600" : "text-card-foreground"}`}
                >
                  {pendingConfirm.type === "warning"
                    ? "Aviso de Alteração de Escala"
                ...
                >
                  ...
                  <div className="group cursor-pointer p-1 -m-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-all w-fit pr-6">
                    <span className="text-[9px] uppercase tracking-wider text-muted-foreground block font-bold transition-colors group-hover:text-primary">
                      Colaborador(a)
                    </span>
                    <span className="text-[11px] font-bold text-foreground inline-block transform transition-transform group-hover:translate-x-0.5">
                      {pendingConfirm.memberName}
                    </span>
                  </div>
                  <div className="flex items-center gap-6 border-t border-border/60 pt-1.5 text-[10px]">
                    <div className="group cursor-pointer p-1 -m-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-all">
                      <span className="text-[8px] uppercase tracking-wider text-muted-foreground block font-medium transition-colors group-hover:text-primary">
                        Cargo
                      </span>
                      <span className="font-semibold text-foreground inline-block transform transition-transform group-hover:translate-x-0.5">
                        {pendingConfirm.memberRole}
                      </span>
                    </div>
                    <div className="group cursor-pointer p-1 -m-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-all">
                      <span className="text-[8px] uppercase tracking-wider text-muted-foreground block font-medium transition-colors group-hover:text-primary">
                    ...
                    <div className="group cursor-pointer p-1 -m-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-all">
                      ...
                      <span className="font-semibold text-foreground inline-block transform transition-transform group-hover:scale-[1.05] origin-left">
                        {pendingConfirm.day}
                      </span>
                    </div>
                  </div>
              ...
              <div className="space-y-2 w-full">
                ...
                >
                  ...
                  <div className="group p-1 -mx-1 rounded-md hover:bg-destructive/10 transition-all duration-300 w-fit pr-6">
                    <span className="text-[9px] uppercase tracking-wider text-destructive/80 block font-bold transition-colors group-hover:text-destructive">
                      Colaborador(a)
                    </span>
                    <span className="text-[11px] font-bold text-destructive inline-block transform transition-transform duration-300 group-hover:translate-x-1">
                      {cellAlert.memberName}
                    </span>
                  </div>
                  <div className="flex items-center gap-6 border-t border-destructive/10 pt-1.5 text-[10px]">
                    <div className="group p-1 -mx-1 rounded-md hover:bg-destructive/10 transition-all duration-300">
                      <span className="text-[8px] uppercase tracking-wider text-destructive/80 block font-medium transition-colors group-hover:text-destructive">
                        Cargo
                      </span>
                      <span className="font-semibold text-destructive inline-block transform transition-transform duration-300 group-hover:translate-x-1">
                        {cellAlert.memberRole}
                      </span>
                    </div>
                    <div className="group p-1 -mx-1 rounded-md hover:bg-destructive/10 transition-all duration-300">
                      <span className="text-[8px] uppercase tracking-wider text-destructive/80 block font-medium transition-colors group-hover:text-destructive">
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-4">
            <Card className="glass-card-premium glass-card-premium shadow-sm rounded-xl relative overflow-hidden">
              <CardHeader className="pb-3 border-b border-border/15">
                ...
                <CardDescription className="text-xs">
                  ...
                  da Escala 12x36 Correspondente em{" "}
              ...
              <CardContent className="pt-4">
                <div className="grid grid-cols-7 gap-1.5 text-center mb-4">
                  ...
                  {(() => {
                    const startingOffset = new Date(
                      selectedYear,
                      selectedMonth - 1,
                      1,
                    ).getDay();
                    return Array.from({ length: startingOffset }).map(
                      (_, i) => (
                        <div
                          key={`empty-${i}`}
                ...
                <div className="bg-slate-50 dark:bg-slate-900/60 p-3 rounded-xl border text-center space-y-1 text-xs">
                  <div className="font-semibold text-slate-800 dark:text-slate-200">
                    Mapeamento Selecionado: Dia {selectedDay} de{" "}
                    {getMonthName(selectedMonth)} de {selectedYear}
                  </div>
            ...
            <Card className="glass-card-premium glass-card-premium shadow-sm rounded-xl">
              <CardHeader className="pb-3 border-b border-border/15">
                <CardTitle className="text-xs uppercase font-black text-slate-700 dark:text-slate-300">
                  Resumo das Escalas 12x36
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {shifts.map((s) => {
                  const sNurses = (s.staff || []).filter(
                    (m) => m.role === "nurse",
                  );
                  const sTechs = (s.staff || []).filter(
                    (m) => m.role === "technician",
                  );
                  ...
                  const isWarn =
                    offTechs > (limits?.technicians || 3) ||
                    offNurses > (limits?.nurses || 1);
      ...
      {activeTab === "request-portal" && (
        <div className="space-y-6 animate-fade-in text-xs">
          ...
          <Card className="glass-card-premium border-border/40 bg-card shadow-sm relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <CardDescription className="text-xs">
                    ...
                    para {getMonthName(selectedMonth)} de {selectedYear}.
                  </CardDescription>
                </div>
                {selectedCollabId && (
                  <Button
                ...
                <div className="md:col-span-4 space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent>
                      {shifts.map((group) => {
                        let icon = (
                          <Sun className="w-3.5 h-3.5 inline-block mr-1.5 text-amber-500 fill-amber-500" />
                        );
                        if (group.id === "rt_lideranca")
                          icon = (
                            <Briefcase className="w-3.5 h-3.5 inline-block mr-1.5 text-slate-500" />
                          );
                        else if (group.id.includes("noturno"))
                          icon = (
                            <MoonStar className="w-3.5 h-3.5 inline-block mr-1.5 text-slate-800 dark:text-slate-200 fill-slate-800 dark:text-slate-200 dark:fill-slate-200" />
                          );
              ...
              {selectedCollabObj && (
                <div className="mt-4 p-3 rounded-xl border border-blue-500/15 bg-blue-500/[0.02] dark:bg-blue-950/15 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs font-extrabold text-slate-700 dark:text-slate-300 dark:text-slate-200 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-7 space-y-4">
              <Card className="glass-card-premium border-border/40 shadow-sm relative overflow-hidden h-full flex flex-col">
                <CardHeader className="pb-3 border-b border-border/15 shrink-0">
                  ...
                  <CardDescription className="text-xs">
                    Selecione no calendário o dia de{" "}
                    {getMonthName(selectedMonth)} de {selectedYear} que você
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label className="text-xs font-black uppercase tracking-wider text-slate-500">
                            Calendário de Turnos ({getMonthName(selectedMonth)}{" "}
                            de {selectedYear})
                          </Label>
                          <Badge className="bg-indigo-600 text-white font-black uppercase rounded text-[10px] px-2 py-0.5 animate-pulse">
                            Dia {collabRequestDay} de{" "}
                            {getMonthName(selectedMonth)} Selecionado
                          </Badge>
                        </div>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/35 rounded-xl p-4 border border-border/80 shadow-inner">
                          ...
                          <div className="mt-3 p-3 glass-panel border border-white/20 dark:border-white/5 text-center text-xs rounded-xl text-slate-600 dark:text-slate-400 font-semibold leading-relaxed">
                            ⚡ Status Real: Dia {collabRequestDay} de{" "}
                            {getMonthName(selectedMonth)} de {selectedYear} é um
                            dia de{" "}
                            {(() => {
            ...
            <div className="lg:col-span-5 space-y-4 flex flex-col">
              {/* SUBMIT FORM CARD */}
              <Card className="glass-card-premium border-border/40 shadow-sm relative overflow-hidden shrink-0">
                <CardHeader className="pb-3 border-b border-border/15">
                  <CardTitle className="text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-tight flex items-center gap-1.5">
              ...
              <Card className="glass-card-premium border-border/40 shadow-sm flex-1 flex flex-col min-h-0">
                  ...
                  {requests.length === 0 ? (
                    ...
                    [...requests].reverse().map((r) => (
                      ...
                      >
                        ...
                        <div className="glass-panel p-1.5 rounded border border-white/20 dark:border-white/5 text-[10px] leading-relaxed text-slate-500 italic">
      ...
      {activeTab === "approval-desk" && (
        <Card className="glass-card-premium border-border/40 shadow-sm relative overflow-hidden">
                    ...
                    .map((req) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Dia e Plantão */}
                          <TableCell className="align-top py-4 text-slate-600 dark:text-slate-400">
                            <span className="font-extrabold text-foreground block">
                              Dia {req.requestedDay} de{" "}
                              {getMonthName(selectedMonth)}
                            </span>
      ...
      {activeTab === "settings" && (
        <div className="space-y-6">
          <Card className="glass-card-premium border border-border/40 shadow-sm relative overflow-hidden">
            <CardHeader className="pb-3 border-b border-border/15">
              <CardTitle className="text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-tight flex items-center gap-1.5">
                <Settings className="h-4.5 w-4.5 text-slate-650" />{" "}
                Configurações Gerais da Escala
              </CardTitle>
              <CardDescription className="text-xs">
                Defina os limites globais de folga permitidos por plantão para a
                equipe.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-4 pb-4">
                ...
                <div className="space-y-4 p-4 rounded-xl border bg-slate-50 dark:bg-slate-900">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      O alerta será disparado se a soma de enfermeiros em
                      folga/licença passar deste valor.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateLimits({
                          ...limits,
                          technicians: parseInt(e.target.value) || 0,
                        })
                      }
                      className="max-w-[200px]"
                    />
                    <p className="text-[10px] text-muted-foreground">
                      O alerta será disparado se a soma de técnicos em
                      folga/licença passar deste valor.
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-8">
                <h3 className="text-sm font-black text-slate-700 dark:text-slate-300 uppercase mb-3 border-b pb-2">
                  Códigos e Direitos (Legenda)
                </h3>
                <p className="text-xs text-muted-foreground mb-4">
                  Gerencie as legendas disponíveis na tabela global. Esses
                  códigos aparecem ao clicar nas células da escala.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {codes.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col gap-1 border p-3 rounded-lg shadow-sm bg-card relative"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span
                          className={`w-8 h-8 flex items-center justify-center text-xs rounded-lg font-black ${item.bgColor} ${item.color.replace("text-", "text-")}`}
                        >
                          {item.code}
                        </span>
                        <Badge
                      ...
                      <Input
                        ...
                        maxLength={3}
                      />
                      <Label className="text-[10px] text-muted-foreground font-bold mt-1">
                        Significado
                      </Label>
                      <Input
                        value={item.label}
                  ...
                  <div
                    className="flex items-center justify-center border p-3 rounded-lg border-dashed bg-slate-50 dark:bg-slate-900/50 cursor-pointer hover:bg-slate-100 transition-colors"
                    onClick={() =>
                      updateCodes([
                        ...codes,
                        {
        ...
        <DialogContent className="sm:max-w-[425px] overflow-hidden border-destructive/20 shadow-2xl shadow-destructive/10">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="pt-3 pb-2 text-sm leading-relaxed text-muted-foreground">
              ...
              e pode afetar imediatamente o dimensionamento do setor.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setTransferConfirm(null)}
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="pt-2 text-xs">
              Selecione o novo turno/equipe para onde deseja transferir{" "}
              <strong className="text-foreground">
          ...
          <div className="py-4">
            ...
            <Select
              ...
              onValueChange={setTransferTargetShift}
            >
              ...
              <SelectContent>
                {shifts
                  .filter((s) => s.id !== transferConfirm?.currentShiftId)
                  .map((s) => (
          ...
          <DialogFooter>
            ...
            <Button
              disabled={!transferTargetShift}
              onClick={executeTransferShift}
        ...
        <DialogContent className="sm:max-w-[480px]">
          ...
          <div className="py-2 space-y-4">
            <div className="bg-slate-50 dark:bg-slate-900/40 p-3 rounded-lg border flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground uppercase">
                  {historyModal?.role === "nurse"
                    ? "Enfermeiro(a)"
                    : "Téc. Enfermagem"}{" "}
                  • {shifts.find((s) => s.id === historyModal?.shiftId)?.name}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="border p-3 rounded-xl bg-card">
                ...
                <p className="text-2xl font-black text-foreground">
                  {historyModal
                    ? Object.values(
                        allProfessionals.find((p) => p.id === historyModal.id)
                          ?.days || {},
                      ).filter((d) => d === "P").length
                    : 0}
                </p>
            ...
            <div className="text-[11px] text-muted-foreground bg-primary/5 p-3 rounded-lg border border-primary/10">
              ℹ️ O log completo de auditoria das alterações manuais da escala
              (com horários e responsáveis) será integrado junto ao módulo de
              fechamento de folha.
            </div>
          </div>
          <DialogFooter>
      ...
      {/* POPUP DE BLOQUEIO MENSAL */}
      <AnimatePresence>
        {isMonthLockModalOpen && (
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setIsMonthLockModalOpen(false)}
              className="absolute inset-0 bg-black/50"
            />
            <motion.div
              initial={{ scale: 0.95, opacity: 0, y: 15 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 15 }}
              className="relative w-full max-w-sm bg-card border border-border rounded-xl shadow-2xl p-6 z-10 flex flex-col items-center gap-4"
            >
              <div className="mx-auto h-12 w-12 rounded-full flex items-center justify-center bg-primary/10">
                <Lock className="h-6 w-6 text-primary" />
              </div>
              <div className="space-y-2 w-full">
                <h3 className="text-sm font-black text-center text-card-foreground">
                  {isMonthLocked
                    ? "Deseja destravar a escala mensal?"
                    : "Deseja travar a escala mensal?"}
                </h3>
                <p className="text-[11px] text-muted-foreground text-center leading-relaxed px-2">
                  {isMonthLocked
                    ? "Ao destravar a escala, as edições de células voltarão a ser permitidas."
                    : "Ao travar a escala, nenhuma célula poderá ser clicada ou editada até ser destravada novamente."}
                </p>
                <div className="flex gap-3 pt-2">
                  <Button
                    type="button"
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setIsMonthLockModalOpen(false);
                      toast.success(
                        isMonthLocked
                          ? "Escala mensal destravada."
                          : "Escala mensal travada.",
                      );
                    }}
                    className="flex-1 text-xs h-9 bg-primary text-primary-foreground hover:bg-primary/95 shadow"
                  >
