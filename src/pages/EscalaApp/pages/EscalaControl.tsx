    ...
    setRequests(prev => {
      const updated = prev.map(r => {
        ...
        return {
          ...
          rtApproval,
          status: newStatus,
          reviewedBy: newStatus !== "pending" ? "RT Renata" : undefined,
      ...
      if (finalReq) {
        if (finalReq.status === "approved") {
          ...
          toast.success("FOLGA DEFERIDA!", {
            description: `Renata (RT) homologou a folga para ${finalReq.memberName} no dia ${finalReq.requestedDay}.`
          });
          ...
          toast.error("SOLICITAÇÃO INDEFERIDA!", {
            description: `Renata (RT) reprovou a folga para ${finalReq.memberName} no dia ${finalReq.requestedDay}.`
          });
        }
      }
      return updated;
    ...
    setRequests(prev => {
      const updated = prev.map(r => {
        ...
        return {
          ...
          leadApproval,
          status: newStatus,
          reviewedBy: newStatus !== "pending" ? "Liderança Maria Eduarda" : undefined,
          reviewedAt: newStatus !== "pending" ? new Date().toISOString() : undefined
        };
      });
      ...
      if (finalReq) {
        if (finalReq.status === "approved") {
          ...
          toast.success("FOLGA DEFERIDA!", {
            description: `Maria Eduarda (Liderança) homologou a folga para ${finalReq.memberName} no dia ${finalReq.requestedDay}.`
          });
        } else if (finalReq.status === "rejected") {
          setShifts(prevShifts => prevShifts.map(s => {
            if (s.id !== finalReq.shiftId) return s;
          ...
          toast.error("SOLICITAÇÃO INDEFERIDA!", {
            description: `Maria Eduarda (Liderança) reprovou a folga para ${finalReq.memberName} no dia ${finalReq.requestedDay}.`
          });
        }
      }
      return updated;
        ...
        <Card className="border-border/40 shadow-sm relative overflow-hidden">
          ...
          <CardContent className="pt-4 p-0">
            <div className="p-4 bg-purple-500/5 border-b text-xs leading-relaxed font-semibold text-purple-800 dark:text-purple-400">
              ⚡ <strong>Regra de Autonomia Compartilhada:</strong> Tanto Renata (RT Enfermagem) quanto Maria Eduarda (Liderança) possuem total autonomia para aprovar ou reprovar as folgas. A decisão de qualquer uma das duas é suficiente para homologação oficial na escala.
            </div>
