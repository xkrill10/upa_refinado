...
export default function Management() {
  ...
  const changeShift = useMutation({
    mutationFn: ({ id, shift_type }) =>
      db.entities.Employee.update(id, { shift_type }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employees"] });
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      if (activeFilter === "has_shifts") {
        ...
        const hasShifts =
          schedule && Object.values(schedule.days || {}).includes("P");
      ...
      if (activeFilter === "has_atestado") {
        ...
        const hasAtestado =
          schedule && Object.values(schedule.days || {}).includes("AT");
      ...
      if (activeFilter === "has_vacation") {
        ...
        const hasVacation =
          schedule && Object.values(schedule.days || {}).includes("V");
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gerencie status, turnos e colaboradores
          </p>
        </div>
        <Button
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.05 }}
        className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 glass-card-premium p-4 rounded-xl shadow-sm"
      >
        {/* Left: Search Input */}
        ...
        <div className="flex items-center gap-1 bg-muted/40 p-1 rounded-lg border border-border self-start lg:self-auto">
          <button
            onClick={() =>
              setShiftFilter(shiftFilter === "diurno_a" ? "ALL" : "diurno_a")
            }
          ...
          <button
            onClick={() =>
              setShiftFilter(shiftFilter === "diurno_b" ? "ALL" : "diurno_b")
            }
            className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all ${
          ...
          <button
            onClick={() =>
              setShiftFilter(shiftFilter === "noturno_a" ? "ALL" : "noturno_a")
            }
            className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all ${
          ...
          <button
            onClick={() =>
              setShiftFilter(shiftFilter === "noturno_b" ? "ALL" : "noturno_b")
            }
            className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all ${
                  ...
                  >
                    ...
                    <TableCell>
                      <Select
                        ...
                        onValueChange={(v) =>
                          changeShift.mutate({ id: emp.id, shift_type: v })
                        }
                      >
                    ...
                    <TableCell>
                      <Badge
                        className={`text-[9px] font-bold border ${statusColors[emp.status || "active"]}`}
                      >
        ...
        {quickCertEmployee && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 15 }}
              className="relative w-full max-w-md glass-card-premium rounded-xl shadow-2xl overflow-hidden z-10"
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/20">
                <div className="flex items-center gap-2 text-destructive">
                  <FileHeart className="h-4 w-4" />
                  <h2 className="text-sm font-bold text-card-foreground">
                    Lançar Atestado Rápido
                  </h2>
                </div>
                <button
                  onClick={() => setQuickCertEmployee(null)}
              ...
              <form onSubmit={handleCertSubmit} className="p-6 space-y-4">
                <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-3">
                  ...
                  <p className="text-xs font-bold text-card-foreground mt-0.5">
                    {quickCertEmployee.name}
                  </p>
                  <span className="text-[10px] text-muted-foreground">
                    Coren: {quickCertEmployee.coren} · Setor:{" "}
                    {quickCertEmployee.sector || "Sem Setor"}
                  </span>
                </div>
