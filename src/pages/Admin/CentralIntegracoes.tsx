...
export default function CentralIntegracoes() {
  const integracoes = [
    ...
    },
    {
      nome: "TISS / TUSS (ANS)",
      descricao:
        "Integração de códigos de faturamento para convênios e auditoria.",
      status: "online",
    ...
    },
    {
      ...
      descricao:
        "Central de regulação para transferência de pacientes críticos (UTI).",
      status: "online",
    ...
    },
    {
      ...
      descricao:
        "Comunicação direta com máquinas de Raio-X (DICOM) e Sangue (HL7).",
      status: "online",
      ultimaSinc: "Tempo Real",
      icone: FlaskConical,
      tipo: "Equipamentos",
    ...
    },
    {
      ...
      descricao:
        "Preenchimento automático de endereço do paciente na Recepção.",
      status: "online",
      ultimaSinc: "Tempo Real",
      icone: Globe,
      tipo: "Utilitário",
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1400px] mx-auto w-full pb-10">
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400 max-w-3xl">
        Painel de demonstração arquitetural. Aqui serão configurados os
        Webhooks, chaves de API (Tokens) e monitorados os status das conexões do
        UPA Control com órgãos governamentais (SUS, CFM) e sistemas de
        faturamento.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-4">
        {integracoes.map((api, index) => (
          <Card
            key={index}
            className="shadow-md border-slate-200/60 dark:border-slate-800 flex flex-col transition-all hover:shadow-lg hover:-translate-y-1"
          >
            <CardHeader className="pb-4 border-b border-slate-100 dark:border-slate-800/50">
              ...
              <CardTitle className="text-base mt-4 font-bold text-slate-800 dark:text-white">
                {api.nome}
              </CardTitle>
              <Badge
                variant="secondary"
                className="w-fit text-[10px] mt-1 uppercase"
              >
                {api.tipo}
              </Badge>
            </CardHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-violet-600 bg-violet-50 hover:bg-violet-100"
      ...
      <div className="mt-8 p-6 rounded-2xl border border-dashed border-violet-300 dark:border-violet-900 bg-violet-50/50 dark:bg-violet-900/10 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md mb-4">
          Nossa arquitetura está preparada para receber novos plugins. Quando o
          banco de dados for estabelecido, novas conexões poderão ser ativadas
