...
import { FlaskConical, Beaker, Microscope, FileSignature, Clock, Printer, XCircle, History, AlertTriangle, Cpu, Thermometer, Package, Activity } from "lucide-react";
...
function SlaTimer({ deadline }: { deadline?: string }) {
  ...
  return (
    <span className={cn("ml-auto", timeRemaining.startsWith('-') ? "text-red-500" : "text-amber-500")}>
      {timeRemaining}
    </span>
  );
}
...
export default function Laboratory() {
  ...
  const allExams = patients.flatMap(p => (p.exams || []).map(e => ({ patient: p, exam: e })));
  ...
  const filteredExams = useMemo(() => {
    return allExams.filter(({ patient, exam }) => {
      ...
      const matchesPriority = filters.priority ? exam.priority === filters.priority : true;
  ...
  const pendingExams = filteredExams
    ...
    .sort((a, b) => {
      ...
      if (!aOverdue && bOverdue) return 1;
      return new Date(a.exam.requestedAt).getTime() - new Date(b.exam.requestedAt).getTime();
    });
  ...
  const completedExams = filteredExams.filter(e => e.exam.status === 'completed');
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <header className="mb-2">
        <div className="flex items-center gap-3 mb-2">
          ...
          <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest text-purple-600 border-purple-500/20">
            Apoio Diagnóstico
          </Badge>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={() => setCollectorMode(true)} className="font-bold text-xs uppercase tracking-widest gap-2 bg-blue-600 hover:bg-blue-700 text-white border-none shadow-lg shadow-blue-500/20">
              Ativar Modo Coletor
            </Button>
            <Button onClick={() => setShowHistory(true)} variant="outline" className="font-bold text-xs uppercase tracking-widest gap-2 bg-background/50 backdrop-blur-sm border-white/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <Card className="p-4 glass-card border-white/20 dark:border-white/5 flex items-center gap-6 overflow-hidden relative min-h-[140px]">
          <div className="flex-1 min-w-0 pl-2 relative z-10">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Produtividade Hoje</h3>
            <div className="text-5xl font-black tracking-tighter text-foreground">{dashboardData.totalToday}</div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-1 mb-2">Laudos Liberados</p>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-6 bg-black/5 dark:bg-white/5 border border-white/10 p-1 w-full justify-start overflow-x-auto">
          <TabsTrigger value="all" className="text-[10px] font-black uppercase tracking-widest">Todos</TabsTrigger>
          <TabsTrigger value="lab" className="text-[10px] font-black uppercase tracking-widest text-purple-600 data-[state=active]:bg-purple-500/10">Análises Clínicas</TabsTrigger>
          <TabsTrigger value="image" className="text-[10px] font-black uppercase tracking-widest text-teal-600 data-[state=active]:bg-teal-500/10">Imagem</TabsTrigger>
        </TabsList>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              ...
              <h3 className="font-bold uppercase tracking-wider text-sm">Fila de Coleta</h3>
              <Badge className="ml-auto bg-amber-500/20 text-amber-600 hover:bg-amber-500/30 border-none">{pendingExams.length}</Badge>
            </div>
            <div className="space-y-3">
              {pendingExams.map(renderExamCard)}
              {pendingExams.length === 0 && <p className="text-center text-xs text-muted-foreground font-semibold py-8 uppercase tracking-widest">Nenhum exame pendente</p>}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              ...
              <h3 className="font-bold uppercase tracking-wider text-sm">Análise Técnica</h3>
              <Badge className="ml-auto bg-blue-500/20 text-blue-600 hover:bg-blue-500/30 border-none">{inAnalysisExams.length}</Badge>
            </div>
            <div className="space-y-3">
              {inAnalysisExams.map(renderExamCard)}
              {inAnalysisExams.length === 0 && <p className="text-center text-xs text-muted-foreground font-semibold py-8 uppercase tracking-widest">Nenhum exame em análise</p>}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              ...
              <h3 className="font-bold uppercase tracking-wider text-sm">Laudos Prontos</h3>
              <Badge className="ml-auto bg-emerald-500/20 text-emerald-600 hover:bg-emerald-500/30 border-none">{completedExams.length}</Badge>
            </div>
            <div className="space-y-3">
              {completedExams.map(renderExamCard)}
              {completedExams.length === 0 && <p className="text-center text-xs text-muted-foreground font-semibold py-8 uppercase tracking-widest">Nenhum laudo liberado</p>}
            </div>
          </div>
        </div>
      </Tabs>
