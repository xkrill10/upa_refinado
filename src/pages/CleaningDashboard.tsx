    ...
    <div className="fixed inset-0 bg-slate-950 text-white z-50 overflow-auto p-8 flex flex-col">
      <div className="flex items-center justify-between border-b border-white/10 pb-6 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black uppercase tracking-widest text-orange-500 drop-shadow-lg">Painel de Higienização</h1>
            <p className="text-xl font-bold text-slate-400 mt-1 uppercase tracking-[0.2em]">Prioridade de Leitos</p>
          </div>
        </div>
      ...
      <div className="flex-1">
        {cleaningBeds.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center gap-6 opacity-50">
            ...
            <h2 className="text-4xl font-black uppercase tracking-widest text-emerald-500 text-center">Nenhum leito<br/>aguardando limpeza</h2>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <AnimatePresence>
              {cleaningBeds.sort((a, b) => new Date(a.requestedAt || 0).getTime() - new Date(b.requestedAt || 0).getTime()).map(bed => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <h3 className="text-5xl font-black tracking-tighter text-white drop-shadow-md">{bed.name}</h3>
                      <p className="text-xl font-bold text-slate-300 uppercase tracking-widest mt-2">{bed.room}</p>
                    </div>
                    
                    <div className="mt-12 mb-8 flex items-center gap-3">
                      <Clock className={cn("h-6 w-6", elapsed.isLate ? "text-red-400" : "text-orange-400")} />
                      <span className={cn("text-3xl font-black", elapsed.isLate ? "text-red-400" : "text-orange-400")}>
                        {elapsed.text}
                      </span>
                    </div>
