...
export default function EvolucaoNutricao() {
  const { id } = useParams();
  ...
  const medicationItems = isChild
    ? PEDIATRIC_MEDICATION_CARE_ITEMS
    : MEDICATION_CARE_ITEMS;
  const careItems = isChild ? PEDIATRIC_NURSING_CARE_ITEMS : NURSING_CARE_ITEMS;
  const comfortItems = isChild
    ? PEDIATRIC_COMFORT_CARE_ITEMS
    : COMFORT_CARE_ITEMS;
  const movementItems = isChild
    ? PEDIATRIC_MOVEMENT_CARE_ITEMS
    : MOVEMENT_CARE_ITEMS;
  const admissionItems = isChild
    ? PEDIATRIC_ADMISSION_ROUTINE_ITEMS
    : ADMISSION_ROUTINE_ITEMS;
  ...
  const [evolutionType, setEvolutionType] = useState("Evolução da Nutrição");
  ...
  useEffect(() => {
    if (isExpressMode && !isFormOpen) {
      ...
      setEvolutionType("Evolução da Nutrição");
    }
  ...
  const toggleCareItem = (itemText: string, toastMsg: string) => {
    ...
    if (normDesc.includes(normItem)) {
      ...
      const filteredLines = descLines.filter((line) => {
        ...
        return !itemLines.some(
          (itemLine) => normalizeText(itemLine) === normLine,
        );
      });

      const newDesc = filteredLines
        .join("\n")
        .replace(/\n{3,}/g, "\n\n")
        .trim();
      ...
      // Smart CID Logic
      const isSyndrome =
        MEDICAL_SYNDROME_ITEMS.some((i) => i.toastMsg === toastMsg) ||
        PEDIATRIC_SYNDROME_ITEMS.some((i) => i.toastMsg === toastMsg);
      if (isSyndrome) {
        if (
          (normItem.includes("infecção") && normItem.includes("urinário")) ||
          normItem.includes("itu")
        ) {
          const cid = CID10_DATABASE.find((c) => c.code === "N390");
          if (cid) setSelectedCid(cid);
        } else if (
          normItem.includes("dor torácica") ||
          normItem.includes("iam") ||
          normItem.includes("coronariana")
        ) {
          ...
          normItem.includes("hipertensiva") ||
          normItem.includes("hipertensão")
        ) {
  ...
  const [stampCouncil, setStampCouncil] = useState(
    () => localStorage.getItem("upa_stamp_council") || "CRM",
  ); // CRM, COREN, Outro
  const [stampNumber, setStampNumber] = useState(
    () => localStorage.getItem("upa_stamp_number") || "",
  );
  const [stampState, setStampState] = useState(
    () => localStorage.getItem("upa_stamp_state") || "SP",
  );
  ...
  const medicalEvolutionsCount = evolutions.filter((e) =>
    e.type.includes("Médica"),
  ).length;
  const nursingEvolutionsCount = evolutions.filter(
    (e) => e.type.includes("Enfermagem") || e.type.includes("Anotação"),
  ).length;
  const multiEvolutionsCount =
    evolutions.length - medicalEvolutionsCount - nursingEvolutionsCount;

  const filteredEvolutions = (() => {
    if (activeTab === "all") return evolutions;
    if (activeTab === "evolutions") {
      return evolutions.filter(
        (e) => e.type.includes("Evolução") || e.type.includes("Anotação"),
      );
    }
  ...
  const getRiskDetails = (risk: string) => {
    switch (risk) {
      case "emergency":
        return { label: "Emergência", color: "bg-red-600" };
      case "very-urgent":
        return { label: "Muito Urgente", color: "bg-orange-500" };
      case "urgent":
        return { label: "Urgente", color: "bg-yellow-500" };
      case "less-urgent":
        return { label: "Pouco Urgente", color: "bg-green-500" };
      case "not-urgent":
        return { label: "Não Urgente", color: "bg-blue-500" };
      default:
        return { label: risk, color: "bg-slate-500" };
    }
  };
  ...
  const handleSaveEvolution = () => {
    ...
    if (
      !evolutionType ||
      !professional ||
      (evolutionType !== "Sinais Vitais" && !description)
    ) {
    ...
    if (
      finalDescription.includes("SOLICITAÇÃO DE INTERNAÇÃO / LEITO:") ||
      finalDescription.includes("SOLICITAÇÃO DE INTERNAÇÃO PEDIÁTRICA:")
    ) {
      updatePatient(id!, {
        admissionRequest: {
          status: "pending",
      ...
      finalDescription.includes("SOLICITAÇÃO DE OBSERVAÇÃO CLÍNICA:") ||
      finalDescription.includes("SOLICITAÇÃO DE OBSERVAÇÃO PEDIÁTRICA:")
    ) {
      updatePatient(id!, {
        admissionRequest: {
          status: "pending",
      ...
      toast.success(
        "A Enfermagem foi notificada para providenciar o leito de observação.",
      );
      new BroadcastChannel("upa_sync_channel").postMessage("sync_all");
    }
      ...
      <div className={cn("space-y-2 relative", isOpen ? "z-30" : "z-10")}>
        ...
        <div className="relative clinical-dropdown-container">
          <button
            ...
            onClick={toggleOpen}
            className={cn(
              "flex items-center justify-between w-full px-4 py-2.5 rounded-xl border bg-white/45 dark:bg-slate-900/45 hover:bg-white/60 dark:hover:bg-slate-900/60 backdrop-blur-sm text-xs font-semibold shadow-sm transition-all focus:outline-none focus:ring-2",
              focusRing,
              isOpen
                ? `${borderOpen} text-foreground`
                : "border-white/60 dark:border-white/10 text-muted-foreground",
            )}
          >
            <div className="flex items-center gap-2">
              <span className={cn("p-1 rounded-lg", bgLight, textMain)}>
                {icon}
              </span>
              ...
              {(() => {
                ...
                return count > 0 ? (
                  <Badge
                    className={cn(
                      bgBadge,
                      "text-white font-black text-[9px] px-1.5 py-0.5 rounded-full border-none shadow-sm animate-in zoom-in-50 duration-200",
                    )}
                  >
                    {count}
                  </Badge>
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className={cn(
                  "absolute left-0 right-0 mt-2 p-1.5 rounded-xl border border-white/20 dark:border-white/10 bg-white/90 dark:bg-slate-950/90 text-popover-foreground shadow-2xl z-50 max-h-[280px] overflow-y-auto overflow-x-hidden space-y-1 backdrop-blur-md",
                  borderLight,
                )}
              >
                {items.map((item) => {
                  const isActive = normalizeText(description).includes(
                    normalizeText(item.text).trim(),
                  );
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? `${hoverLight} ${textDark} font-bold border ${borderLight}`
    ...
    >
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3">
              ...
              <Badge
                variant="outline"
                className={`${status.color} border-0 text-[10px] uppercase font-bold px-3 py-1 rounded-full`}
              >
                {status.label}
              </Badge>
            </div>
              ...
              {patient.socialName && (
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-black uppercase tracking-wider">
                    NOME SOCIAL: {patient.socialName.toUpperCase()}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-hover:text-blue-500/80 transition-colors duration-300">
                  Queixa & Alergias
                </p>
                <p
                  className="text-sm font-bold text-foreground truncate mt-0.5"
                  title={patient.mainComplaint}
                >
          ...
          >
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-hover:text-indigo-500/80 transition-colors duration-300">
                  Evoluções
                </p>
                ...
                <p className="text-[9px] font-semibold text-slate-500 dark:text-slate-400 mt-0.5">
                  {medicalEvolutionsCount} Méd · {nursingEvolutionsCount} Enf ·{" "}
                  {multiEvolutionsCount} Multi
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          {[
            ...
            return (
              ...
              >
                ...
                {tab.label}
                {tab.badge ? (
                  <Badge className="ml-1 bg-red-500 text-white border-none px-1.5 py-0.5 text-[10px] font-black h-4 min-w-[18px] flex items-center justify-center animate-pulse shadow-sm">
                    {tab.badge}
                  </Badge>
                ) : null}
                {isActive && (
        ...
        {!isFormOpen && !isExpressMode && (
          <Button
            onClick={() => {
              setIsFormOpen(true);
              if (activeTab === "prescriptions") {
                handleEvolutionTypeChange("Evolução da Nutrição");
              ...
              <CardContent className="p-4 space-y-4">
                  ...
                  >
                    ...
                    {evolutionType === "Evolução Enfermagem" ? (
                      <p className="text-muted-foreground leading-relaxed text-[11px]">
                        <strong>Privativo do Enfermeiro (Ciclo de 24h):</strong>{" "}
                        Análise crítica e julgamento clínico do estado do
                        paciente. Deve incluir: nível de consciência, dados
                        neurológicos/cardiorrespiratórios, integridade de pele,
                        funcionamento de dispositivos (acessos/sondas/drenos),
                        eliminações (intestinal/vesical) e resposta às condutas
                        anteriores.
                      </p>
                    ) : (
                      <p className="text-muted-foreground leading-relaxed text-[11px]">
                        <strong>
                          Dados Brutos e Pontuais (Toda a Equipe):
                        </strong>{" "}
                        Registro imediato de fatos observados ou executados (ex:
                        administração de medicação com via/dose, banho,
                        curativos, etc.). Serve de subsídio para a evolução do
                        enfermeiro.
                      </p>
                    )}
                  </motion.div>
                ...
                {(evolutionType === "Evolução Médica" ||
                  ...
                  <div className="space-y-1">
                    <Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                      Diagnóstico / CID-10{" "}
                      <span className="text-[9px] text-muted-foreground font-medium lowercase italic">
                        (opcional)
                      </span>
                    </Label>
                    <SmartCidSelector
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setVsConsciousness("A");
                            toast.success(
                              "Parâmetros normais (estáveis) aplicados",
                            );
                          }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                      <div className="space-y-1">
                        ...
                        <Select
                          value={vsConsciousness}
                          onValueChange={setVsConsciousness}
                        >
                          <SelectTrigger
                            className={cn(
                              "h-8 text-xs bg-white/45 dark:bg-slate-900/45 border-white/60 dark:border-white/10 hover:bg-white/60 dark:hover:bg-slate-900/60 rounded-xl backdrop-blur-sm shadow-sm transition-all focus:ring-1 focus:ring-[#006699]/20",
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div className="space-y-1">
                            ...
                            <Select
                              ...
                              onValueChange={(v) =>
                                setPrescWizard({ ...prescWizard, route: v })
                              }
                            >
                          ...
                          <div className="space-y-1">
                            ...
                            <Select
                              ...
                              onValueChange={(v) =>
                                setPrescWizard({ ...prescWizard, frequency: v })
                              }
                            >
                              <SelectTrigger className="h-8 text-xs bg-white/50 dark:bg-black/20">
                                <SelectValue placeholder="Selecione" />
                              ...
                              <SelectContent>
                                <SelectItem value="Dose única">
                                  Dose única
                                </SelectItem>
                                <SelectItem value="Contínuo">
                                  Contínuo
                                </SelectItem>
                        ...
                        >
                          <Plus className="h-3 w-3 mr-1" /> Adicionar à
                          Prescrição
                        </Button>
                      </div>
                        ...
                        <div className="bg-white/40 dark:bg-black/20 rounded-lg border border-white/40 dark:border-white/10 p-2 min-h-[120px] max-h-[160px] overflow-y-auto space-y-1.5">
                          {prescribedMedications.length === 0 ? (
                            ...
                            prescribedMedications.map((med, idx) => (
                              <div
                                key={idx}
                                className="flex flex-col bg-white/60 dark:bg-slate-800/60 p-2 rounded-md border border-slate-500/20 text-xs relative group"
                              >
                                <button
                                  onClick={() =>
                                    handleRemovePrescriptionItem(idx)
                                  }
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black uppercase text-muted-foreground">
                          3. Escalas Clínicas Úteis (Prescrição Adaptativa)
                        </span>
                        <Badge
                          variant="outline"
                          ...
                          <div className="flex items-center justify-between w-full mb-1.5">
                            ...
                            <div
                              className={cn(
                                ...
                                selectedMews ? "bg-blue-500" : "bg-blue-500/15",
                              )}
                            >
                              <Activity
                                className={cn(
                                  "h-3 w-3",
                                  selectedMews ? "text-white" : "text-blue-600",
                                )}
                              />
                            </div>
                          </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isDischargeTypeDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                {(() => {
                                  const selectedItem =
                                    DISCHARGE_TYPE_ITEMS.find((item) =>
                                      normalizeText(description).includes(
                                        normalizeText(item.text).trim(),
                                      ),
                                    );
                                  return selectedItem
                                    ? selectedItem.label
                                    : "Selecionar Tipo de Alta...";
                                })()}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = DISCHARGE_TYPE_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isDischargeConductDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = DISCHARGE_CONDUCT_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                                ...
                                {DISCHARGE_CONDUCT_ITEMS.map((item) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() =>
                                        toggleCareItem(item.text, item.toastMsg)
                                      }
                                      className={cn(
                                        "group flex items-center justify-between w-full px-3 py-2 rounded-lg text-left text-xs transition-all",
                                        isActive
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black uppercase text-muted-foreground">
                          3. Protocolos & Termos Obrigatórios (Desfecho
                          Adaptativo)
                        </span>
                        <Badge
                          variant="outline"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isMedicalNeuroDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          1. Exame Físico & Neuro
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count =
                                  MEDICAL_PHYSICAL_NEURO_ITEMS.filter((item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                                  ).length;
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isMedicalSyndromeDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          2. Síndromes Clínicas Comuns
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = MEDICAL_SYNDROME_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isMedicalConductDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          3. Solicitações e Condutas Rápidas
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = MEDICAL_CONDUCT_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                        <button
                          type="button"
                          onClick={() => setOpenQsofaCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-purple-500/30 bg-purple-500/10 hover:bg-purple-500/20 text-purple-700 dark:text-purple-400 text-[10px] font-black uppercase transition-all"
                        >
                          <Activity className="h-3 w-3" /> qSOFA
                        </button>
                        <button
                          type="button"
                          onClick={() => setOpenHeartCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-700 dark:text-red-400 text-[10px] font-black uppercase transition-all"
                        >
                          <Heart className="h-3 w-3" /> HEART Score
                        </button>
                        <button
                          type="button"
                          onClick={() => setOpenCurb65Calc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-sky-500/30 bg-sky-500/10 hover:bg-sky-500/20 text-sky-700 dark:text-sky-400 text-[10px] font-black uppercase transition-all"
                        >
                          <Activity className="h-3 w-3" /> CURB-65
                        </button>
                        <button
                          type="button"
                          onClick={() => setOpenWellsCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-orange-500/30 bg-orange-500/10 hover:bg-orange-500/20 text-orange-700 dark:text-orange-400 text-[10px] font-black uppercase transition-all"
                        >
                          <Activity className="h-3 w-3" /> Wells (TEP)
                        </button>
                        <button
                          type="button"
                          onClick={() => setOpenNihssCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-indigo-500/30 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 text-[10px] font-black uppercase transition-all"
                        >
                          <Brain className="h-3 w-3" /> NIHSS (AVC)
                        </button>
                        <button
                          type="button"
                          onClick={() => setOpenParklandCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-orange-500/30 bg-orange-500/10 hover:bg-orange-500/20 text-orange-700 dark:text-orange-400 text-[10px] font-black uppercase transition-all"
                        >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isPediatricNeuroDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          1. Exame Físico Inicial & Traumas
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count =
                                  PEDIATRIC_PHYSICAL_NEURO_ITEMS.filter(
                                    (item) =>
                                      normalizeText(description).includes(
                                        normalizeText(item.text).trim(),
                                      ),
                            ...
                            {isPediatricNeuroDropdownOpen && (
                              <>
                                <motion.div
                                  ...
                                  className="absolute left-0 right-0 mt-2 p-1.5 rounded-xl border border-green-500/20 border-white/20 dark:border-white/10 bg-white/90 dark:bg-slate-950/90 text-slate-900 dark:text-slate-100 shadow-2xl backdrop-blur-md z-50 max-h-[280px] overflow-y-auto overflow-x-hidden space-y-1 scrollbar-thin scrollbar-thumb-muted"
                                >
                                  {PEDIATRIC_PHYSICAL_NEURO_ITEMS.map(
                                    (item) => {
                                      const isActive = normalizeText(
                                        description,
                                      ).includes(
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isPediatricSyndromeDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          2. Síndromes Pediátricas Comuns
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = PEDIATRIC_SYNDROME_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isPediatricConductDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          3. Condutas Rápidas (Pediatria)
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = PEDIATRIC_CONDUCT_ITEMS.filter(
                                  (item) =>
                                    normalizeText(description).includes(
                                      normalizeText(item.text).trim(),
                                    ),
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setOpenDehydrationCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-sky-500/30 bg-sky-500/10 hover:bg-sky-500/20 text-sky-700 dark:text-sky-400 text-[10px] font-black uppercase transition-all"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setOpenWoodDownesCalc(true)}
                          className="flex items-center justify-center gap-1.5 p-2 rounded-xl border border-teal-500/30 bg-teal-500/10 hover:bg-teal-500/20 text-teal-700 dark:text-teal-400 text-[10px] font-black uppercase transition-all"
                        >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isMedicationDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          1. Rotinas de Medicação e Acesso
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span className="p-1 rounded-lg bg-red-500/10 text-red-600">
                                ...
                                >
                                  <path d="m18 2 4 4" />
                                  <path d="m17 7 3-3" />
                                  <path d="M19 9 8.7 19.3c-.2.2-.5.3-.7.3H5v-3c0-.3.1-.5.3-.7L15.6 5.6" />
                                  <path d="m9 11 4 4" />
                                  <path d="m5 19-3 3" />
                                  <path d="m14 4 6 6" />
                                </svg>
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = medicationItems.filter((item) =>
                                  normalizeText(description).includes(
                                    normalizeText(item.text).trim(),
                                  ),
                                ).length;
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <svg
                                ...
                                className={cn(
                                  "lucide lucide-chevron-down text-muted-foreground/60 transition-transform duration-200",
                                  isCareDropdownOpen && "transform rotate-180",
                                )}
                              >
                                <path d="m6 9 6 6 6-6" />
                              </svg>
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isComfortDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          3. Cuidados, Conforto e Dieta Padrão
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = comfortItems.filter((item) =>
                                  normalizeText(description).includes(
                                    normalizeText(item.text).trim(),
                                  ),
                                ).length;
                      ...
                      <div
                        className={cn(
                          "space-y-2 relative",
                          isMovementDropdownOpen ? "z-30" : "z-10",
                        )}
                      >
                        <span className="text-[9px] font-black uppercase text-muted-foreground block">
                          4. Movimentação e Ocorrências
                        ...
                        <div className="relative clinical-dropdown-container">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {(() => {
                                const count = movementItems.filter((item) =>
                                  normalizeText(description).includes(
                                    normalizeText(item.text).trim(),
                                  ),
                                ).length;
                ...
                {evolutionType === "Evolução da Fisioterapia" && (
                  <motion.div
                    initial={{ opacity: 0, y: -5 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="p-4 rounded-xl border bg-slate-500/5 dark:bg-slate-500/10 border-slate-500/20 space-y-4 relative z-20"
                  >
                    <div className="flex items-center justify-between border-b border-slate-500/20 pb-2">
                      <span className="font-extrabold text-[#006699] uppercase tracking-wider text-[11px]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "blue",
                        isChild ? FISIO_PED_ITEMS : FISIO_ADULT_ITEMS,
                        isFisioEvalDropdownOpen,
                        () => {
                          setIsFisioEvalDropdownOpen(!isFisioEvalDropdownOpen);
                          setIsFisioProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        "2. Procedimentos e Condutas",
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="14"
                          height="14"
                        ...
                        "green",
                        isChild ? FISIO_PED_PROCEDURES : FISIO_ADULT_PROCEDURES,
                        isFisioProcDropdownOpen,
                        () => {
                          setIsFisioProcDropdownOpen(!isFisioProcDropdownOpen);
                          setIsFisioEvalDropdownOpen(false);
                        },
                      )}
                    </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "amber",
                        isChild ? NUTRI_PED_ITEMS : NUTRI_ADULT_ITEMS,
                        isNutriEvalDropdownOpen,
                        () => {
                          setIsNutriEvalDropdownOpen(!isNutriEvalDropdownOpen);
                          setIsNutriProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        ...
                        "green",
                        isChild ? NUTRI_PED_PROCEDURES : NUTRI_ADULT_PROCEDURES,
                        isNutriProcDropdownOpen,
                        () => {
                          setIsNutriProcDropdownOpen(!isNutriProcDropdownOpen);
                          setIsNutriEvalDropdownOpen(false);
                        },
                      )}
                    </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "purple",
                        isChild ? PSICO_PED_ITEMS : PSICO_ADULT_ITEMS,
                        isPsicoEvalDropdownOpen,
                        () => {
                          setIsPsicoEvalDropdownOpen(!isPsicoEvalDropdownOpen);
                          setIsPsicoProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        ...
                        "pink",
                        isChild ? PSICO_PED_PROCEDURES : PSICO_ADULT_PROCEDURES,
                        isPsicoProcDropdownOpen,
                        () => {
                          setIsPsicoProcDropdownOpen(!isPsicoProcDropdownOpen);
                          setIsPsicoEvalDropdownOpen(false);
                        },
                      )}
                    </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "blue",
                        isChild ? SOCIAL_PED_ITEMS : SOCIAL_ADULT_ITEMS,
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "amber",
                        isChild ? TO_PED_ITEMS : TO_ADULT_ITEMS,
                        isToEvalDropdownOpen,
                        () => {
                          setIsToEvalDropdownOpen(!isToEvalDropdownOpen);
                          setIsToProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        "2. Condutas e Estimulação",
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="14"
                          height="14"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "blue",
                        isChild ? FONO_PED_ITEMS : FONO_ADULT_ITEMS,
                        isFonoEvalDropdownOpen,
                        () => {
                          setIsFonoEvalDropdownOpen(!isFonoEvalDropdownOpen);
                          setIsFonoProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        "2. Condutas e Disfagia",
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="14"
                          height="14"
                        ...
                        "green",
                        isChild ? FONO_PED_PROCEDURES : FONO_ADULT_PROCEDURES,
                        isFonoProcDropdownOpen,
                        () => {
                          setIsFonoProcDropdownOpen(!isFonoProcDropdownOpen);
                          setIsFonoEvalDropdownOpen(false);
                        },
                      )}
                    </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {renderPanelDropdown(
                        ...
                        "amber",
                        isChild ? FARMA_PED_ITEMS : FARMA_ADULT_ITEMS,
                        isFarmaEvalDropdownOpen,
                        () => {
                          setIsFarmaEvalDropdownOpen(!isFarmaEvalDropdownOpen);
                          setIsFarmaProcDropdownOpen(false);
                        },
                      )}
                      {renderPanelDropdown(
                        "2. Condutas e Reconciliação",
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="14"
                          height="14"
                        ...
                        "pink",
                        isChild ? FARMA_PED_PROCEDURES : FARMA_ADULT_PROCEDURES,
                        isFarmaProcDropdownOpen,
                        () => {
                          setIsFarmaProcDropdownOpen(!isFarmaProcDropdownOpen);
                          setIsFarmaEvalDropdownOpen(false);
                        },
                      )}
                    </div>
                ...
                {evolutionType === "Evolução Enfermagem" && (
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-sky-500/20 pb-2">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[8px] uppercase tracking-wider bg-sky-500/10 text-sky-600 border-sky-500/20"
                      >
                      ...
                      <div className="space-y-2">
                        ...
                        {(() => {
                          const stats = beds.reduce(
                            (acc, bed) => {
                              if (!acc[bed.ward])
                                acc[bed.ward] = { available: 0, total: 0 };
                              acc[bed.ward].total++;
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {Object.entries(stats).map(
                                ([ward, wardStats]) => (
                                  <div
                                    key={ward}
                                    className="p-2.5 rounded-xl border bg-white/45 dark:bg-slate-900/45 shadow-sm"
                                  >
                                    ...
                                    <div className="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
                                      <div
                                        ...
                                        style={{
                                          width: `${((wardStats.total - wardStats.available) / wardStats.total) * 100}%`,
                                        }}
                                      />
                                    </div>
                                  </div>
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="COREN">
                              COREN (Enfermeiro)
                            </SelectItem>
                            <SelectItem value="COREN-TEC">
                              COREN (Técnico)
                            </SelectItem>
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/50 pb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            className={cn(
                              "border-none font-bold text-[10px] uppercase py-0.5 px-2 rounded-full shadow-sm",
                          ...
                          <span className="text-xs font-bold text-muted-foreground/80">
                            {new Date(exam.requestedAt).toLocaleTimeString(
                              "pt-BR",
                      ...
                      <div className="text-sm">
                        <p className="text-muted-foreground font-semibold text-[10px] uppercase">
                          Médico Solicitante: {exam.doctor}
                        </p>
                        {exam.priority === "urgent" && (
                          <p className="text-red-500 font-bold text-[10px] uppercase mt-1">
                            SLA URGENTE (1h)
                          </p>
                        )}
                        {exam.result && (
                          <p className="mt-2 text-foreground font-medium">
                            <strong>Laudo:</strong> {exam.result}
                          </p>
                        )}
                      </div>
                    </CardContent>
      ...
      <Dialog open={isBedDialogOpen} onOpenChange={setIsBedDialogOpen}>
        <DialogContent className="sm:max-w-[425px] rounded-[2rem] p-6 overflow-hidden glass-card-premium bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black uppercase tracking-tight text-[#006699] dark:text-sky-400">
              Gerenciar Leito
            </DialogTitle>
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/30 dark:border-emerald-500/25 flex items-center gap-4 shadow-sm backdrop-blur-md">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] font-semibold text-emerald-600/70 dark:text-emerald-400/70 mt-0.5 truncate">
                      {patientBed.ward} · {patientBed.room}
                    </p>
                  </div>
                </div>
                <Button
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-bold text-sm text-foreground transition-colors group-hover:text-primary dark:group-hover:text-sky-450 truncate">
                              {bed.name}
                            </p>
