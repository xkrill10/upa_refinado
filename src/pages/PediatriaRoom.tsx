...
export default function PediatriaRoom() {
  ...
  const [selectedRoom, setSelectedRoom] = useState(() => localStorage.getItem("upa_active_room") || "SALA PEDIATRIA");
  const [occupiedRooms] = useState<Record<string, string>>({
    'TRIAGEM INFANTIL 2': 'ENF. JULIANA LIMA'
  });
  ...
  useEffect(() => {
    ...
    if (!room || !nurse) {
      toast.error("Acesso negado", { description: "Por favor, assuma uma sala primeiro." });
  ...
  const maskTemp = (v: string) => {
    const clean = v.replace(/\D/g, "");
    if (clean.length <= 2) return clean;
    return `${clean.slice(0, 2)}.${clean.slice(2, 3)}`;
  };
  const maskNumbers = (v: string, limit?: number) => {
    const clean = v.replace(/\D/g, "");
  ...
  const maskSPO2 = (v: string) => {
    let clean = v.replace(/\D/g, "").substring(0, 3);
    if (parseInt(clean) > 100) clean = "100";
    return clean;
  };
  ...
  const completeTriage = (risk: Patient['risk']) => {
    ...
    toast.success("Triagem Pediátrica Finalizada!");
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3 bg-white/70 dark:bg-slate-900/45 p-2 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 shadow-sm self-start md:self-auto transition-colors duration-500 w-full">
          <div className="hidden lg:flex items-center gap-4 px-4 border-r border-slate-200 dark:border-slate-800/60 mr-2">
            {[
              { id: 'TRIAGEM PEDIÁTRICA 1', status: occupiedRooms['TRIAGEM PEDIÁTRICA 1'] ? 'ocupado' : 'livre' },
              { id: 'TRIAGEM PEDIÁTRICA 2', status: occupiedRooms['TRIAGEM PEDIÁTRICA 2'] ? 'ocupado' : 'livre' },
              { id: 'OBSERVAÇÃO PEDIÁTRICA', status: occupiedRooms['OBSERVAÇÃO PEDIÁTRICA'] ? 'ocupado' : 'livre' },
            ].map(room => (
              <div key={room.id} className="flex flex-col items-center justify-center">
                <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-[0.1em]">{room.id}</span>
                <div className="flex items-center gap-1.5">
                  <div className={cn(
                    "h-2 w-2 rounded-full",
                    room.status === 'livre' ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"
                  )} />
                  <span className={cn(
                    "text-[9px] font-black uppercase tracking-tight",
                    room.status === 'livre' ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
                  )}>
                    {room.status}
                  </span>
                </div>
          ...
          <div className="hidden lg:flex flex-col items-center justify-center px-4 border-r border-slate-200 dark:border-slate-800/60 mr-2">
            <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Aguardando</span>
            <span className="text-sm font-black text-orange-500 dark:text-orange-400">{waitingForTriage.length} CRIANÇAS</span>
          </div>

          <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-2">Sua Sala:</span>
          <div className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest bg-orange-500 dark:bg-orange-600 text-white shadow-md flex items-center gap-2">
          ...
          <Card className="glass-card border-none shadow-xl rounded-xl overflow-hidden transition-all duration-500">
            <CardHeader className="pb-4 border-b border-slate-200/40 dark:border-slate-800/40">
              <div className="flex items-center gap-2">
                ...
                <Badge className="ml-auto bg-orange-500/10 dark:bg-orange-500/20 text-orange-655 dark:text-orange-400 border-0 text-[10px] font-bold px-3 py-1 rounded-full">
                  {waitingForTriage.length} crianças na fila
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {waitingForTriage.length === 0 ? (
                <div className="p-24 text-center flex flex-col items-center gap-4">
                  <div className="h-16 w-16 rounded-full bg-orange-500/10 dark:bg-orange-500/20 flex items-center justify-center">
                    <Baby className="h-8 w-8 text-orange-400/80" />
                  </div>
                  <p className="text-sm font-bold text-muted-foreground">
                    Nenhuma criança na fila de triagem
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          {patient.ticket && (
                            <span className="px-4 py-1.5 rounded-xl bg-orange-500/10 dark:bg-orange-500/20 text-orange-650 dark:text-orange-400 border border-orange-200/30 dark:border-orange-500/25 font-mono font-bold text-sm shadow-sm w-20 text-center group-hover:scale-105 transition-transform shrink-0">
                              {patient.ticket}
                            </span>
                          )}
                      ...
                      {patient.risk ? (
                        ...
                        <div className="w-[300px] flex items-center justify-end gap-3 shrink-0">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-10 w-10 rounded-xl p-0 text-muted-foreground hover:text-orange-500 dark:hover:text-orange-450 hover:bg-orange-500/10 dark:hover:bg-orange-500/20"
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-10 text-xs font-bold gap-2 border-orange-500/30 text-orange-600 dark:text-orange-400 hover:bg-orange-500/5 dark:hover:bg-orange-500/10 rounded-xl bg-transparent"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleCall(patient);
                            }}
                          ...
                          <Button
                            size="sm"
                            className="h-10 text-xs font-bold gap-2 bg-orange-500 dark:bg-orange-600 hover:bg-orange-600 dark:hover:bg-orange-700 text-white rounded-xl shadow-md group-hover:scale-105 transition-all border-0"
                            onClick={(e) => {
                              e.stopPropagation();
          ...
          {completed.length > 0 && (
            <Card className="glass-card border-none shadow-xl rounded-xl overflow-hidden transition-all duration-500">
              <CardHeader className="pb-4 border-b border-emerald-100/20 dark:border-emerald-800/20 bg-emerald-500/5 dark:bg-emerald-950/10">
                <CardTitle className="text-sm font-black uppercase tracking-widest text-emerald-700 dark:text-emerald-300 flex items-center gap-2">
                  <CheckCircle2 className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
                  Finalizados Hoje ({completed.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0">
          ...
          <Card className="glass-card border-none shadow-xl rounded-xl overflow-hidden transition-all duration-500">
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-orange-500 dark:text-orange-450 flex items-center gap-1.5">
                  Check-list Equipamentos
                </p>
                <div className="grid grid-cols-1 gap-2">
                  {Object.entries(equipmentStatus).map(([item, checked]) => (
                    <button
                      key={item}
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl bg-slate-50 dark:bg-slate-950 [&>button]:hidden">
          ...
          <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 text-center">
            ...
            <div className="space-y-4">
              ...
              <div
                className="flex items-center justify-between p-4 rounded-2xl bg-slate-100/50 dark:bg-slate-900/40 border border-slate-200/20 dark:border-slate-800/20 cursor-pointer hover:bg-slate-150/60 dark:hover:bg-slate-900/60 transition-colors"
            ...
            {selectedPatient && (
              ...
              >
                <Card className="glass-card shadow-2xl border-primary/20 overflow-hidden h-[90vh] md:h-auto md:max-h-[85vh] flex flex-col">
                  <CardHeader className="bg-[#8b5cf6]/5 p-8 md:p-10 pb-6 md:pb-8 shrink-0">
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <CardTitle className="text-lg font-black mission-control-title uppercase tracking-tight flex items-center gap-2">
                          {activeFlowchart ? (
                            ...
                            <>
                              <Stethoscope className="h-5 w-5 text-[#8b5cf6]" />
                          ...
                          >
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t border-border/20">
                              ...
                              <Button
                                ...
                                className="rounded-xl h-14 px-12 font-black uppercase tracking-widest shadow-xl shadow-primary/20 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white gap-2 transition-all hover:scale-[1.02]"
                              >
                              ...
                              <div className="lg:col-span-4 space-y-8">
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-4 relative z-10">
                                    <div className="h-14 w-14 rounded-2xl bg-[#8b5cf6]/10 flex items-center justify-center text-[#8b5cf6] shadow-inner group-hover:scale-110 transition-transform">
