...
import { Search, Volume2, ChevronDown, Printer, Megaphone, User, Activity, MoreVertical, LogOut, Users, Clock, AlertCircle, CheckCircle2, VolumeX, RotateCcw, ArrowRightLeft, LayoutGrid, X, Eye, Baby } from "lucide-react";
...
export default function Queue() {
  ...
  const getRiskDetails = (risk: string) => {
    switch(risk) {
      case 'emergency': return { label: 'Emergência', color: 'bg-red-600 hover:bg-red-700 text-white', target: 0, pulse: true };
      case 'very-urgent': return { label: 'Muito Urgente', color: 'bg-orange-500 hover:bg-orange-600 text-white', target: 10, pulse: true };
      case 'urgent': return { label: 'Urgente', color: 'bg-[#FFDE21] hover:bg-[#FFDE21]/90 text-black', target: 60, pulse: false };
      case 'less-urgent': return { label: 'Pouco Urgente', color: 'bg-green-500 hover:bg-green-600 text-white', target: 120, pulse: false };
      case 'not-urgent': return { label: 'Não Urgente', color: 'bg-blue-500 hover:bg-blue-600 text-white', target: 240, pulse: false };
      default: return { label: risk, color: 'bg-slate-500 text-white', target: 999, pulse: false };
    }
  };
  ...
  const avgWaitTime = waitingPatients.length > 0 
    ? Math.floor(waitingPatients.reduce((acc, p) => {
  ...
  const stats = {
    ...
    critical: patients.filter(p => (p.risk === 'emergency' || p.risk === 'very-urgent') && p.status !== 'completed').length,
    avgWait: avgWaitTime,
  ...
  const getTicketColorLight = (ticket: string) => {
    ...
    if (prefix === 'E') return "bg-red-50 text-red-650 border-red-100 dark:bg-red-950/30 dark:text-red-400 dark:border-red-900/30";
    if (prefix === 'C') return "bg-orange-50 text-orange-600 border-orange-100 dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-900/30";
    if (prefix === 'P') return "bg-purple-50 text-purple-600 border-purple-100 dark:bg-purple-950/30 dark:text-purple-400 dark:border-purple-900/30";
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <h1 className="text-4xl font-black tracking-tight text-[#006699] dark:text-sky-400 uppercase">Fila de Atendimento</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          const isActive = (item.label === 'Total na Unidade' && riskFilter === 'all' && statusFilter === 'all' && audienceFilter === 'all') ||
            (item.label === 'Aguardando' && statusFilter === 'waiting' && audienceFilter === 'all') ||
            (item.label === 'Em Atendimento' && statusFilter === 'attending' && audienceFilter === 'all') ||
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-[#006699] dark:text-sky-400">{item.label}</p>
            ...
            <Table>
              <TableHeader className="bg-slate-50/50 dark:bg-slate-950/20 border-b border-slate-200/40 dark:border-slate-800/40">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest pl-6">Paciente</TableHead>
                  ...
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest">Queixa</TableHead>
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest">Setor</TableHead>
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest">Chegada</TableHead>
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest text-center">Status</TableHead>
                  <TableHead className="text-[#006699] dark:text-sky-400 h-14 text-[10px] font-black uppercase tracking-widest text-right pr-6">Ação</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredPatients.map((patient) => {
                  ...
                  const overTarget = isOverTarget(patient.arrivalTime, patient.risk);
                  ...
                  return (
                    <TableRow 
                      ...
                      className={cn(
                        ...
                        patient.risk === 'emergency' ? "bg-red-500/5 dark:bg-red-950/10 hover:bg-red-500/10 dark:hover:bg-red-950/15" : 
                        patient.risk === 'very-urgent' ? "bg-orange-500/5 dark:bg-orange-950/10 hover:bg-orange-500/10 dark:hover:bg-orange-950/15" : 
                        ...
                        <div className="flex items-center gap-2">
                          {risk.pulse && (
                            <div className="relative flex h-2 w-2">
                              <span className={cn("animate-ping absolute inline-flex h-full w-full rounded-full opacity-75", risk.color)}></span>
                              <span className={cn("relative inline-flex rounded-full h-2 w-2", risk.color)}></span>
                            </div>
                          )}
                          <button 
                            onClick={() => navigate(`/paciente/${patient.id}/evolucao`, { state: { from: '/fila', label: 'Fila de Atendimento' } })}
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center gap-1">
                            <span className={cn(
                              ...
                              overTarget ? "text-red-555 dark:text-red-400 animate-pulse" : "text-muted-foreground dark:text-slate-500"
                            )}>
                              Há {getTimeElapsed(patient.arrivalTime)}
                            </span>
                            {overTarget && <AlertCircle className="h-3 w-3 text-red-555 dark:text-red-400" />}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right pr-6">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button 
                            ...
                            size="icon" 
                            className="h-8 w-8 text-slate-500 hover:text-[#006699] dark:hover:text-sky-400 hover:bg-[#006699]/5 dark:hover:bg-sky-400/5 cursor-pointer"
                            onClick={() => navigate(`/paciente/${patient.id}`, { state: { from: '/fila', label: 'Fila de Atendimento' } })}
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              ...
                              navigate(`/triagem`);
                              toast.info(`Paciente ${patient.name} enviado para reclassificação.`);
                            }}
                            title="Reclassificar"
                          >
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              setTransferPatient({ id: patient.id, name: patient.name, sector: patient.sector || "" });
                              setNewSector(patient.sector || "Consultório Clínico 1");
                            }}
                            title="Trocar Setor / Encaminhar"
                          >
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              toast.success(`Encaminhando para impressão: ${patient.ticket}...`);
                              setPrintingPatient(patient);
                            }}
                            title="Reimprimir Senha"
                          >
      ...
      <Dialog open={showCallControl} onOpenChange={setShowCallControl}>
        <DialogContent className="max-w-md p-0 overflow-hidden rounded-xl border-none shadow-2xl bg-white dark:bg-slate-950 [&>button]:hidden">
          <DialogHeader className={cn(
            ...
            callingTicket?.risk === 'emergency' ? 'bg-red-655 text-white' :
            callingTicket?.risk === 'very-urgent' ? 'bg-orange-500 text-white' :
            callingTicket?.risk === 'urgent' ? 'bg-[#FFDE21] text-black' :
            callingTicket?.risk === 'less-urgent' ? 'bg-green-500 text-white' :
            (callingTicket?.priority === 'preferential' ? 'bg-purple-600 text-white' : 
             callingTicket?.priority === 'pediatric' ? 'bg-orange-500 text-white' : 'bg-[#006699] text-white')
          )}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-xl font-black uppercase tracking-tight">Controle de Chamada</DialogTitle>
                  <DialogDescription className={cn(
                    "text-[10px] font-bold uppercase tracking-widest mt-0.5",
                    callingTicket?.risk === 'urgent' ? "text-black/70" : "text-white/70"
              ...
              <Button 
                onClick={() => {
                  if (callingTicket && callingTicket.ticket) {
                    callTicket(callingTicket.ticket, callingTicket.room, callingTicket.risk as Patient['risk'], callingTicket.patientName);
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isAudioEnabled ? "bg-green-500" : "bg-slate-400 dark:bg-slate-700"
                  )}
                >
                  <div className={cn(
      ...
      <Dialog open={!!transferPatient} onOpenChange={(open) => !open && setTransferPatient(null)}>
        <DialogContent className="sm:max-w-[550px] rounded-xl p-0 overflow-hidden border-none shadow-2xl bg-white dark:bg-slate-950 text-foreground transition-all duration-500">
          <div className={cn(
            "p-6 text-white bg-[#006699] dark:bg-sky-950/60"
          )}>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 bg-white/20 rounded-lg">
                <ArrowRightLeft className="h-5 w-5 text-white" />
              </div>
              <DialogTitle className="text-xl font-black uppercase tracking-tight">Encaminhar Paciente</DialogTitle>
            </div>
            <p className="text-white/80 text-[10px] font-bold uppercase tracking-widest">Sala ou setor de destino</p>
          </div>
          ...
          <div className="p-6 space-y-5">
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200/40 dark:border-slate-800/40 flex items-center justify-between transition-colors">
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-1">Paciente</p>
                <p className="text-base font-black text-slate-800 dark:text-white leading-tight">{formatWords(transferPatient?.name || "")}</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-1">Setor Atual</p>
                <Badge variant="outline" className="bg-white/50 dark:bg-slate-950/50 text-[10px] font-bold border-slate-200 dark:border-slate-800">{transferPatient?.sector || "Triagem"}</Badge>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  >
                    <span className="text-[10px] font-bold uppercase tracking-tight truncate">{sector}</span>
                    <div className={cn(
                      "h-3.5 w-3.5 rounded-full border-2 flex items-center justify-center transition-all shrink-0",
                      newSector === sector ? "border-[#006699] dark:border-sky-400 bg-[#006699] dark:bg-sky-400" : "border-slate-300 dark:border-slate-700"
                    )}>
                      {newSector === sector && <div className="h-1 w-1 rounded-full bg-white dark:bg-black" />}
                    </div>
                  </button>
                ))}
              </div>
              ...
              <Button 
                ...
                onClick={() => {
                  if (transferPatient && newSector) {
                    updatePatient(transferPatient.id, { sector: newSector, status: 'waiting' });
                    toast.success(`Paciente ${transferPatient.name} encaminhado para ${newSector}`);
      ...
      {printingPatient && (
        <div className="print-only">
          ...
          <div className="text-xs uppercase tracking-widest mb-2 font-bold tracking-widest">Sua senha de atendimento</div>
          <div className="text-6xl font-black my-6">{printingPatient.ticket}</div>
          <div className="text-sm font-bold mb-1 uppercase">
            {printingPatient.priority === 'emergency' ? 'ATENÇÃO: EMERGÊNCIA' : 
             printingPatient.priority === 'pediatric' ? 'PEDIATRIA / CRIANÇA' :
             printingPatient.priority === 'preferential' ? 'PREFERENCIAL' : 'NORMAL'}
          </div>
          <div className="text-xs mb-6">Local: {printingPatient.priority === 'emergency' ? 'SALA VERMELHA' : printingPatient.priority === 'pediatric' ? 'Triagem Pediátrica' : 'Triagem'}</div>
