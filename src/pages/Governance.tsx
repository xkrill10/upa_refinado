  ...
  switch (priority) {
    case "urgent":
      return (
        <Badge
          variant="outline"
          className="bg-red-500/10 text-red-600 border-red-500/20 text-[9px] uppercase tracking-wider"
        >
    ...
    case "high":
      return (
        <Badge
          variant="outline"
          className="bg-amber-500/10 text-amber-600 border-amber-500/20 text-[9px] uppercase tracking-wider"
        >
          Alta
        </Badge>
      );
...
const IsolationBadge = () => (
  <Badge
    variant="outline"
    className="bg-purple-500/10 text-purple-600 border-purple-500/20 text-[9px] uppercase tracking-wider font-bold animate-pulse"
  >
...
export default function Governance() {
  ...
  // Modals state
  const [assignModalTask, setAssignModalTask] = useState<ContextBed | null>(
    null,
  );
  const [finishModalTask, setFinishModalTask] = useState<ContextBed | null>(
    null,
  );
  const [maintenanceModalTask, setMaintenanceModalTask] =
    useState<ContextBed | null>(null);
  const [historyModalTask, setHistoryModalTask] = useState<CleaningLog | null>(
    null,
  );
  const [maintenanceReason, setMaintenanceReason] = useState(
    "Grade da cama emperrada / quebrada",
  );

  // Filtros Avançados
  const [filterSector, setFilterSector] = useState<
    "ALL" | "Emergência" | "Observação"
  >("ALL");
  ...
  const handleReportMaintenance = (bed: ContextBed) => {
    ...
    toast.error(
      `Defeito reportado no ${bed.name}. O.S. enviada à Engenharia Clínica.`,
    );
  };

  // Aplicação de Filtros
  const filteredBeds =
    filterSector === "ALL" ? beds : beds.filter((b) => b.ward === filterSector);
  const filteredHistory =
    filterSector === "ALL"
      ? cleaningHistory
      : cleaningHistory.filter((h) => h.ward === filterSector);

  const waitingTasks = filteredBeds.filter(
    (b) => b.status === "cleaning" && b.cleaningStatus === "waiting",
  );
  const inProgressTasks = filteredBeds.filter(
    (b) => b.status === "cleaning" && b.cleaningStatus === "in_progress",
  );
  const maintenanceTasks = filteredBeds.filter(
  ...
  return (
    <div className="flex h-full gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
        ...
        <header className="shrink-0 flex justify-between items-end">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <Badge
                variant="outline"
                className="text-[9px] font-black uppercase tracking-widest text-cyan-600 border-cyan-500/20"
              >
          ...
          <div className="flex gap-2 bg-muted/40 p-1.5 rounded-xl border border-border/50">
            {["ALL", "Emergência", "Observação"].map((sec) => (
              <Button
                ...
                className={cn(
                  "text-xs font-bold",
                  filterSector === sec ? "shadow-sm" : "opacity-50",
                )}
                onClick={() =>
                  setFilterSector(sec as "ALL" | "Emergência" | "Observação")
                }
              >
              ...
              <AnimatePresence>
                {sortedWaitingTasks.map((task) => (
                  ...
                  >
                    <Card
                      className={cn(
                        "p-3.5 shadow-sm transition-all border",
                    ...
                    >
                      <div className="flex flex-col gap-2">
                        ...
                        <Button
                          ...
                          className="w-full text-[10px] font-black uppercase tracking-widest bg-cyan-600 hover:bg-cyan-700 h-8 mt-1"
                        >
                  ...
                  >
                    <Card className="p-3.5 bg-card border-cyan-500/30 shadow-sm relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-2 pl-2 mt-3 mb-3 bg-muted/50 p-1.5 rounded-lg border border-border/50">
                        <div
                          className={`h-6 w-6 rounded-md ${task.assignedCleaner?.color} text-[9px] font-black flex items-center justify-center text-white`}
                        >
                          {task.assignedCleaner?.avatar}
                        </div>
                        <span className="text-xs font-bold">
                          {task.assignedCleaner?.name}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 pl-2 border-t border-border/40 pt-3">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setMaintenanceModalTask(task)}
                          className="text-[9px] font-black uppercase tracking-widest text-muted-foreground hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/30 h-7 px-1"
                        >
                          Defeito
                        </Button>
                        <Button
                ...
                {maintenanceTasks.map((task) => (
                  ...
                  >
                    <Card className="p-3.5 bg-card border-red-500/30 shadow-sm">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-bold text-base leading-none text-red-600 dark:text-red-400">
                            {task.name}
                          </h4>
                          <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1">
                            {task.ward} - {task.room}
                        ...
                        <Badge
                          variant="outline"
                          className="text-[9px] bg-red-500/10 text-red-500 border-red-500/20"
                        >
                          O.S. Aberta
                        </Badge>
                      </div>
                      <p className="text-[10px] font-bold text-muted-foreground mt-3 p-2 bg-red-500/5 rounded-lg border border-red-500/10">
                        {task.maintenanceReason ||
                          "Aguardando liberação da Engenharia Clínica."}
                      </p>
                    </Card>
                  </motion.div>
                ))}
                ...
                {filteredHistory.map((log) => (
                  ...
                  >
                    <Card className="p-3 bg-card border-emerald-500/20 flex flex-col gap-3 group">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-bold text-sm text-emerald-600 dark:text-emerald-400 leading-none">
                            {log.bedName}
                          </h4>
                        ...
                        <Badge
                          variant="outline"
                          className="text-[8px] border-emerald-500/30 text-emerald-600 bg-emerald-500/5 px-1 py-0 h-4"
                        >
                          {log.finishedAt.toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </Badge>
        ...
        <Card className="p-5 glass-card border-border/40 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-black tracking-tighter">
              18<span className="text-xs text-muted-foreground ml-1">min</span>
            </p>
          </div>
        </Card>
        ...
        <Card className="flex-1 glass-card border-border/40 p-4 flex flex-col">
          ...
          <div className="space-y-4 overflow-y-auto pr-1">
            {cleaners.map((cleaner) => {
              ...
              const activeTask = inProgressTasks.find(
                (t) => t.assignedCleaner?.id === cleaner.id,
              );
              // Quantos leitos limpou hoje
              const cleanedCount = cleaningHistory.filter(
                (h) => h.cleanerName === cleaner.name,
              ).length;

              return (
                <div
                  key={cleaner.id}
                  className="p-3 rounded-2xl bg-card border border-border/50 shadow-sm relative overflow-hidden"
                >
                  {/* Indicador lateral */}
                  <div
                    className={cn(
                  ...
                  <div className="flex items-center gap-3 pl-2">
                    <div
                      className={`h-10 w-10 shrink-0 rounded-xl ${cleaner.color} text-sm font-black flex items-center justify-center text-white shadow-inner`}
                    >
                      {cleaner.avatar}
                    </div>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span
                          className={cn(
                            ...
                            activeTask ? "text-cyan-600" : "text-emerald-600",
                          )}
                        >
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setAssignModalTask(null)}
      >
        <DialogContent className="sm:max-w-[400px] glass-card-premium border-white/10">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione quem fará a higienização do{" "}
              <strong>{assignModalTask?.name}</strong>.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-4">
            {cleaners.map((cleaner) => {
              const isBusy = beds.some(
                (b) =>
                  b.cleaningStatus === "in_progress" &&
                  b.assignedCleaner?.id === cleaner.id,
              );
              return (
                <Button
                  key={cleaner.id}
                  ...
                  disabled={isBusy}
                  className={cn(
                    "justify-start h-14 relative overflow-hidden group",
                    isBusy && "opacity-50",
                  )}
                  onClick={() =>
                    assignModalTask && handleStartTask(assignModalTask, cleaner)
                  }
                >
                  <div
                    className={`absolute left-0 top-0 h-full w-1.5 ${cleaner.color}`}
                  />
                  <div
                    className={`h-8 w-8 rounded-full ${cleaner.color} text-xs font-black flex items-center justify-center text-white mr-3`}
                  >
                    {cleaner.avatar}
                  </div>
                  <div className="text-left">
                    <span className="font-bold block leading-none mb-1">
                      {cleaner.name}
                    </span>
                    <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                      {isBusy ? "Já está ocupado" : "Disponível"}
                    </span>
                  </div>
                </Button>
              );
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setFinishModalTask(null)}
      >
        <DialogContent className="sm:max-w-[550px] glass-card-premium border-white/10 overflow-hidden">
          <DialogHeader>
            <div className="flex justify-between items-start">
              <div>
                ...
                <DialogDescription className="mt-1">
                  Checklist e registro de insumos para liberar o{" "}
                  <strong>{finishModalTask?.name}</strong>.
                </DialogDescription>
              </div>
              <div className="bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 px-3 py-1.5 rounded-lg text-center">
                ...
                <span className="font-black text-sm flex items-center gap-1 justify-center">
                  ...
                  {calculateElapsedTime(finishModalTask?.startedAt)}
                </span>
              </div>
            </div>
            {finishModalTask?.isIsolation && (
              <div className="mt-3 p-2 bg-purple-500/10 border border-purple-500/30 rounded-lg text-xs text-purple-700 dark:text-purple-400 font-bold flex items-center gap-2 animate-pulse">
                <ShieldAlert className="h-4 w-4 shrink-0" /> Este leito requer
                Desinfecção de Alto Nível (Isolamento).
              </div>
            )}
          </DialogHeader>
          ...
          <div className="py-2 space-y-5 max-h-[60vh] overflow-y-auto pr-2">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      finishModalTask?.isIsolation
                        ...
                        : "border-cyan-500/30 bg-cyan-500/5 text-cyan-700",
                    )}
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <Button
                      ...
                      className={cn(
                        "h-10 px-2 justify-start gap-1.5 transition-all text-[10px]",
                        supplies.includes(item.id)
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                <MessageSquare className="h-3 w-3" /> Observações da Equipe
              </Label>
              <textarea
                value={observations}
          ...
          <div className="pt-4 mt-2 border-t border-border/50 space-y-4">
            <label className="flex items-start gap-3 p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl cursor-pointer hover:bg-amber-500/10 transition-colors">
              <input
                type="checkbox"
                checked={ccihConfirmed}
                onChange={(e) => setCcihConfirmed(e.target.checked)}
              ...
              <span className="text-xs font-medium text-amber-900 dark:text-amber-200/80 leading-relaxed">
                Declaro sob minha responsabilidade que todos os protocolos da{" "}
                <strong>CCIH</strong> foram rigorosamente cumpridos na
                higienização deste leito.
              </span>
            </label>
            <div className="flex justify-end gap-3">
              <Button variant="ghost" onClick={() => setFinishModalTask(null)}>
                Cancelar
              </Button>
              <Button
                ...
                onClick={() =>
                  finishModalTask && handleFinishTask(finishModalTask)
                }
              >
      ...
      >
        <DialogContent className="sm:max-w-[450px] glass-card-premium border-emerald-500/30">
          <DialogHeader>
            ...
            <DialogDescription>
              Registro imutável da limpeza do{" "}
              <strong>{historyModalTask?.bedName}</strong>.
            </DialogDescription>
          </DialogHeader>
          {historyModalTask && (
            <div className="py-4">
              <div className="relative border-l-2 border-border/50 ml-3 space-y-6 pb-2">
                ...
                <div className="relative pl-6">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    {historyModalTask.requestedAt?.toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    }) || "--:--"}
                  </p>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Enfermagem sinalizou desocupação.
                  </p>
                </div>
                ...
                <div className="relative pl-6">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    {historyModalTask.startedAt?.toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    }) || "--:--"}
                  </p>
                  ...
                  <div className="flex items-center gap-2 mt-1.5 bg-muted/40 p-2 rounded-lg border border-border/50 w-fit">
                    <div
                      className={`h-6 w-6 rounded-md ${historyModalTask.cleanerColor} text-[8px] font-black flex items-center justify-center text-white`}
                    >
                      {historyModalTask.cleanerAvatar}
                    </div>
                    <span className="text-xs font-bold">
                      {historyModalTask.cleanerName}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative pl-6">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    {historyModalTask.finishedAt.toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </p>
                  <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                    Leito Pronto e Liberado
                  </p>

                  <div className="mt-2 space-y-2">
                    <Badge variant="outline" className="text-[9px]">
                      <Timer className="h-3 w-3 mr-1" /> Duração:{" "}
                      {historyModalTask.durationMinutes} min
                    </Badge>

                    {historyModalTask.supplies.length > 0 && (
                      <p className="text-xs text-muted-foreground">
                        <span className="font-bold">Insumos Repostos:</span>{" "}
                        {historyModalTask.supplies.join(", ")}
                      </p>
                    )}
                    ...
                    {historyModalTask.ccihConfirmed && (
                      <p className="text-[10px] font-bold text-amber-600 flex items-center gap-1 mt-2">
                        <CheckCircle2 className="h-3 w-3" /> Protocolo CCIH
                        validado.
                      </p>
                    )}
                  </div>
