...
> = {
  ...
  preferential: {
    label: "Preferencial (P)",
    description: "Idoso, gestante, PCD, etc",
    color: "bg-amber-500 hover:bg-amber-600",
    icon: Accessibility,
  },
  pediatric: {
    label: "Pediatria (C)",
    description: "Criança até 12 anos",
    color: "bg-orange-500 hover:bg-orange-600",
    icon: Baby,
  },
  emergency: {
    label: "Emergência (E)",
    description: "Risco imediato à vida",
...
export default function Arrival() {
  ...
  useEffect(() => {
    if (cpf.length === 14 && !isUnidentified) {
      ...
      if (existing) {
        ...
        toast.info(
          `Dados de ${existing.name.split(" ")[0]} carregados automaticamente.`,
        );
      }
    }
  ...
  const getTicketColor = (ticket: string) => {
    ...
    if (prefix === "E")
      return "bg-red-500 text-white shadow-[0_10px_30px_rgba(239,68,68,0.3)] border-red-500";
    if (prefix === "P")
      return "bg-amber-500 text-white shadow-[0_10px_30px_rgba(245,158,11,0.3)] border-amber-500";
    if (prefix === "C")
      return "bg-orange-500 text-white shadow-[0_10px_30px_rgba(249,115,22,0.3)] border-orange-500";
  ...
  const getTicketColorLight = (ticket: string) => {
    const prefix = ticket.charAt(0).toUpperCase();
    if (prefix === "E")
      return "text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-950/20";
    if (prefix === "P")
      return "text-amber-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20";
    if (prefix === "C")
      return "text-orange-500 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/20";
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground text-sm font-medium">
          Pré-cadastro do paciente e emissão de senha para classificação de
          risco
        </p>
      </div>
          ...
          >
            <Card className="glass-card-premium border-none shadow-2xl rounded-xl overflow-hidden transition-all duration-500">
              <CardHeader className="pt-8 px-8 pb-4">
                <CardTitle className="text-xl md:text-2xl font-black uppercase mission-control-title text-foreground">
                  Identificação do Paciente
                </CardTitle>
                <CardDescription className="text-sm text-muted-foreground">
                  Se o CPF já estiver cadastrado, os dados serão reaproveitados
                  automaticamente.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-8 pt-2">
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div
                    className="flex items-center gap-4 p-5 rounded-2xl glass-card-premium border-white/40 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] mb-4 transition-all group cursor-pointer hover:shadow-[0_12px_40px_rgb(0,0,0,0.15)]"
                  ...
                  >
                    <button
                      ...
                      className={cn(
                        ...
                        isUnidentified
                          ? "bg-primary shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)]"
                          : "bg-black/10 dark:bg-white/10 shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)]",
                      )}
                    >
                      <div
                        className={cn(
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider transition-colors">
                        Ativação para casos de emergência sem documentos
                      </span>
                    </div>
                  </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pb-4 px-1">
                          ...
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <Input
                              ...
                              value={susCard}
                              onChange={(e) =>
                                setSusCard(maskSUS(e.target.value))
                              }
                            ...
                            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">
                              Necessário para atendimento gratuito (SUS)
                            </p>
                          </div>
                          <div className="space-y-2 md:col-span-2">
                            <Label className="text-slate-700 dark:text-slate-300 font-bold text-sm">
                            ...
                            <Input
                              ...
                              value={motherName}
                              onChange={(e) =>
                                setMotherName(formatWords(e.target.value))
                              }
                              className="h-14 focus-visible:ring-offset-0 focus-visible:ring-2 focus-visible:ring-primary/20 transition-all border border-black/10 dark:border-white/10 backdrop-blur-md shadow-[inset_0_2px_8px_rgba(0,0,0,0.05)] dark:shadow-[inset_0_2px_8px_rgba(0,0,0,0.2)] rounded-2xl placeholder:text-muted-foreground/50"
                            />
                          </div>
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              !active &&
                                "border-transparent bg-white/40 dark:bg-slate-900/40 hover:bg-white/70 dark:hover:bg-slate-800/70 text-muted-foreground shadow-sm",
                              !active &&
                                key === "normal" &&
                                "hover:border-blue-400 hover:shadow-[0_8px_20px_rgba(59,130,246,0.15)]",
                              !active &&
                                key === "preferential" &&
                                "hover:border-amber-400 hover:shadow-[0_8px_20px_rgba(245,158,11,0.15)]",
                              !active &&
                                key === "pediatric" &&
                                "hover:border-orange-400 hover:shadow-[0_8px_20px_rgba(249,115,22,0.15)]",
                              !active &&
                                key === "emergency" &&
                                "hover:border-red-400 hover:shadow-[0_8px_20px_rgba(239,68,68,0.15)]",
                              active &&
                                "scale-[1.02] text-foreground font-medium",
                              active &&
                                key === "normal" &&
                                "border-blue-500 shadow-[0_10px_30px_rgba(59,130,246,0.3)] bg-blue-500/15 dark:bg-blue-900/40",
                              active &&
                                key === "preferential" &&
                                "border-amber-500 shadow-[0_10px_30px_rgba(245,158,11,0.3)] bg-amber-500/15 dark:bg-amber-900/40",
                              active &&
                                key === "pediatric" &&
                                "border-orange-500 shadow-[0_10px_30px_rgba(249,115,22,0.3)] bg-orange-500/15 dark:bg-orange-900/40",
                              active &&
                                key === "emergency" &&
                                "border-red-500 shadow-[0_10px_30px_rgba(239,68,68,0.3)] bg-red-500/15 dark:bg-red-900/40",
                            )}
                          >
                            <div
                              className={cn(
                                "absolute top-0 right-0 p-12 transform translate-x-12 -translate-y-12 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700 pointer-events-none",
                                active && key === "normal"
                                  ? "bg-blue-400/40 dark:bg-blue-500/30"
                                  : "",
                                active && key === "preferential"
                                  ? "bg-amber-400/40 dark:bg-amber-500/30"
                                  : "",
                                active && key === "pediatric"
                                  ? "bg-orange-400/40 dark:bg-orange-500/30"
                                  : "",
                                active && key === "emergency"
                            ...
                            <div
                              className={cn(
                                "h-10 w-10 rounded-xl text-white flex items-center justify-center transition-transform group-hover:scale-110 duration-300",
                                meta.color,
                              )}
                            >
                            ...
                            <span className="text-xs text-muted-foreground group-hover:text-muted-foreground/90 transition-colors leading-normal">
                              {meta.description}
                            </span>
                          </button>
                        );
                  ...
                  <Button
                    type="submit"
                    size="lg"
                    className="w-full h-16 text-lg font-black uppercase tracking-widest gap-2 bg-[#006699] hover:bg-[#005580] rounded-xl shadow-xl hover:shadow-sky-500/20 active:scale-[0.98] transition-all text-white"
                  >
              ...
              <CardContent className="p-12 flex flex-col items-center text-center gap-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-6 print:hidden w-full max-w-sm">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="lg"
                    className="flex-1 h-14 rounded-xl font-bold active:scale-95 transition-all border-border/80 dark:border-slate-800/80 bg-transparent"
                  >
                    Nova Identificação
                  </Button>
                </div>
