...
export default function NirDashboard() {
  ...
  const filteredPatients = transferPatients.filter((p) => {
    ...
    const matchesPriority =
      filterPriority === "all" ||
      p.transferRequest?.priority === filterPriority;
    return matchesSearch && matchesPriority;
  });

  const stats = {
    total: transferPatients.length,
    pending: transferPatients.filter(
      (p) => p.transferRequest?.status === "requested",
    ).length,
    accepted: transferPatients.filter(
      (p) =>
        p.transferRequest?.status === "accepted" ||
        p.transferRequest?.status === "transporting",
    ).length,
    denied: transferPatients.filter(
      (p) => p.transferRequest?.status === "denied",
    ).length,
  };
    ...
    >
      ...
      <Card className="glass-card-premium border-none shadow-2xl">
        <CardHeader className="p-6 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <CardTitle className="text-sm font-black tracking-widest text-orange-500 uppercase">
            Fila de Regulação
          </CardTitle>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-white/5 dark:bg-slate-900/50">
              <TableRow>
                <TableHead className="font-black text-[10px] uppercase tracking-widest pl-6">
                  Paciente
                </TableHead>
            ...
            <TableBody>
              {filteredPatients
                .sort(
                  (a, b) =>
                    new Date(b.transferRequest!.requestedAt).getTime() -
                    new Date(a.transferRequest!.requestedAt).getTime(),
                )
                .map((p) => {
                  ...
                  return (
                    <TableRow key={p.id} className="group hover:bg-muted/30">
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs font-semibold mt-1 truncate max-w-[200px]">
                          {req.reason}
                        </div>
                        {req.crossCode && (
                      ...
                      <TableCell>
                        {req.status === "requested" && (
                          <Badge className="bg-amber-500 text-white text-[9px] uppercase font-black">
                            Aguardando Vaga
                          </Badge>
                        )}
                        {req.status === "accepted" && (
                          <Badge className="bg-emerald-500 text-white text-[9px] uppercase font-black">
                            Vaga Aceita
                          </Badge>
                        )}
                        {req.status === "transporting" && (
                          <Badge className="bg-blue-500 text-white text-[9px] uppercase font-black animate-pulse">
                            Aguardando SAMU
                          </Badge>
                        )}
                        {req.status === "denied" && (
                          <Badge className="bg-red-500 text-white text-[9px] uppercase font-black">
                            Negado
                          </Badge>
                        )}
                        {req.hospitalName && (
                          <div className="text-[9px] font-bold mt-1 text-emerald-600">
                            {req.hospitalName}
                          </div>
                        )}
                        {req.ambulanceType && (
                          <div className="text-[9px] font-black uppercase text-blue-500 mt-0.5">
                            {req.ambulanceType}
                          </div>
                        )}
                      </TableCell>
                      <TableCell className="text-right pr-6">
                        {req.status === "requested" && (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-[9px] font-black uppercase tracking-widest hover:bg-orange-500 hover:text-white border-orange-500/50 text-orange-500"
                        ...
                        {req.status === "accepted" && (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-[9px] font-black uppercase tracking-widest bg-blue-500/10 hover:bg-blue-600 hover:text-white border-blue-500/50 text-blue-600"
                            onClick={() =>
                              updateTransferStatus(p.id, "transporting")
                            }
                          >
              ...
              {transferPatients.length === 0 && (
                <TableRow>
                  <TableCell
                    colSpan={5}
                    className="h-32 text-center text-muted-foreground font-black text-xs uppercase tracking-widest"
                  >
          ...
          <DialogContent className="sm:max-w-[450px] rounded-[2rem] p-0 overflow-hidden border-none shadow-2xl">
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <DialogDescription className="text-white/60 font-medium text-xs mt-1">
                      Registrar vaga cedida e protocolo
                    </DialogDescription>
                  </div>
                </div>
              </DialogHeader>
              <div className="p-8 space-y-6">
                <div>
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1 mb-1">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select name="hospital" required>
                      ...
                      <SelectContent>
                        <SelectItem value="Hospital das Clínicas (HC)">
                          Hospital das Clínicas (HC)
                        </SelectItem>
                        <SelectItem value="Hospital São Paulo">
                          Hospital São Paulo
                        </SelectItem>
                        <SelectItem value="Santa Casa de Misericórdia">
                          Santa Casa de Misericórdia
                        </SelectItem>
                        <SelectItem value="Hospital do Servidor Público">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select name="ambulance" required>
                        ...
                        <SelectContent>
                          <SelectItem value="SAMU - Básica">
                            Básica (USB)
                          </SelectItem>
                          <SelectItem value="SAMU - Avançada (UTI)">
                            Avançada / UTI (USA)
                          </SelectItem>
                        </SelectContent>
                      </Select>
              ...
              <DialogFooter className="p-8 pt-0 flex gap-3">
                <Button
                  type="button"
                  variant="outline"
                  className="rounded-xl h-12 flex-1 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
                  onClick={() => {
                ...
                <Button
                  type="submit"
                  className="rounded-xl h-12 flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-600/20"
                >
