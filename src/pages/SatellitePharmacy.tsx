...
import { Pill, Search, Plus, ArrowRight, PackageOpen, ClipboardList, Activity, ArrowDownToLine, ArchiveRestore, X, Check, Clock, User, AlertTriangle, TrendingUp, TrendingDown, BellRing } from "lucide-react";
...
export default function SatellitePharmacy() {
  ...
  const [dispenseItem, setDispenseItem] = useState<{name: string, amount: number, unit: string} | null>(null);

  const inventoryItems = [
    { name: "Dipirona 500mg/ml", amount: 120, unit: "Ampola", critical: false, trend: "up", history: [20, 30, 45, 80, 100, 115, 120] },
    { name: "Adrenalina 1mg/ml", amount: 8, unit: "Ampola", critical: true, trend: "down", history: [25, 25, 20, 15, 12, 10, 8] },
    { name: "Soro Fisiológico 0.9% 500ml", amount: 45, unit: "Frasco", critical: false, trend: "up", history: [10, 15, 22, 30, 35, 40, 45] },
    { name: "Ondansetrona 2mg/ml", amount: 5, unit: "Ampola", critical: true, trend: "down", history: [18, 18, 15, 10, 8, 6, 5] },
  ];
  ...
  useEffect(() => {
    if (criticalItemsCount > 0) {
      toast.error(`Atenção: Você possui ${criticalItemsCount} medicamento(s) com estoque crítico.`, {
  ...
  const Sparkline = ({ data, color }: { data: number[], color: string }) => {
    ...
    return (
      <svg width="60" height="24" className="overflow-visible">
        ...
        <polyline fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" points={points} className="drop-shadow-[0_0_3px_currentColor]" />
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] relative">
      ...
      {criticalItemsCount > 0 && (
        ...
        >
          ...
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div>
              <h4 className="text-rose-600 dark:text-rose-400 font-bold tracking-wide">Alerta de Estoque Crítico</h4>
              <p className="text-sm text-rose-600/80 dark:text-rose-200/80 mt-0.5">{criticalItemsCount} item(s) atingiram a quantidade mínima. Solicite ressuprimento imediato.</p>
            </div>
          </div>
          <button 
      ...
      <div className="flex items-center justify-between mb-6 px-6 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium mt-1">Gestão de estoque avançado para setores críticos.</p>
          </div>
        </div>
      ...
      <div className="flex gap-6 flex-1 min-h-0 px-6 pb-6">
        ...
        <div className="w-64 flex flex-col gap-2 shrink-0">
          {[
            { id: "estoque", label: "Estoque Satélite", icon: PackageOpen, desc: "Visualizar itens no setor" },
            { id: "pedidos", label: "Meus Pedidos", icon: ClipboardList, desc: "Status de requisições" },
            { id: "consumo", label: "Consumo Beira-Leito", icon: Activity, desc: "Histórico de saídas" },
            { id: "transferencias", label: "Recebimentos", icon: ArrowDownToLine, desc: "Itens a receber" }
          ].map(tab => (
            <button
              key={tab.id}
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-1 relative z-10">
                <tab.icon className={cn(
                  ...
                  activeTab === tab.id ? "text-sky-500" : "text-muted-foreground group-hover:text-foreground"
                )} />
                <span className={cn(
              ...
              <span className={cn(
                ...
                activeTab === tab.id ? "text-sky-600/70 dark:text-sky-200/70" : "text-muted-foreground/60"
              )}>
                {tab.desc}
              </span>
            </button>
        ...
        <div className="flex-1 rounded-2xl border glass-card-premium overflow-hidden flex flex-col min-w-0 shadow-lg">
          ...
          <div className="flex-1 overflow-auto p-6 flex flex-col gap-4 custom-scrollbar">
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4 mb-2">
                  <div className="p-4 rounded-xl bg-muted/30 border border-border/50 flex items-center gap-4 hover:bg-muted/50 transition-colors">
                    ...
                    <div>
                      <span className="text-2xl font-black text-foreground font-mono">{inventoryItems.length}</span>
                      <span className="text-xs text-muted-foreground uppercase font-bold tracking-widest block mt-0.5">Itens Diferentes</span>
                    </div>
                  </div>
                  <div className="p-4 rounded-xl bg-muted/30 border border-border/50 flex items-center gap-4 relative overflow-hidden group hover:border-rose-500/30 transition-colors">
                    <div className={cn("absolute inset-0 bg-gradient-to-r from-rose-500/10 to-transparent opacity-0 transition-opacity", criticalItemsCount > 0 && "opacity-100 animate-pulse")} />
                    ...
                    <div className="relative z-10">
                      <span className="text-2xl font-black text-foreground font-mono">{criticalItemsCount}</span>
                      <span className="text-xs text-muted-foreground uppercase font-bold tracking-widest block mt-0.5">Em Falta</span>
                    </div>
                  </div>
                  <div className="p-4 rounded-xl bg-muted/30 border border-border/50 flex items-center gap-4 hover:bg-muted/50 transition-colors">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-muted-foreground uppercase font-bold tracking-widest block mt-0.5">Saídas Hoje</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  {filteredInventory.map((item, i) => (
                    <div key={i} className="group relative flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/50 hover:border-border transition-all duration-300">
                      <div className="flex items-center gap-4 w-1/3">
                        <div className={cn(
                          ...
                          <PackageOpen className={cn(
                            "h-5 w-5",
                            item.critical ? "text-rose-500" : "text-emerald-500"
                          )} />
                        </div>
                        <div>
                          <span className="font-bold text-foreground tracking-wide block truncate">{item.name}</span>
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center border-l border-r border-border/50 px-6">
                        <div className="flex items-center gap-4 w-full max-w-[200px]">
                           <div className="flex flex-col">
                             <span className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Tendência 24h</span>
                             <div className="flex items-center gap-1">
                                {item.trend === 'up' ? <TrendingUp className="h-3 w-3 text-emerald-500" /> : <TrendingDown className="h-3 w-3 text-rose-500" />}
                                <span className={cn("text-xs font-bold", item.trend === 'up' ? "text-emerald-500" : "text-rose-500")}>
                                  {item.trend === 'up' ? '+12%' : '-8%'}
                                </span>
                             </div>
                           </div>
                           <div className="flex-1 flex justify-end">
                             <Sparkline data={item.history} color={item.trend === 'up' ? '#10b981' : '#f43f5e'} />
                           </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-6 w-1/3 justify-end pl-6">
                        <div className="flex flex-col items-end">
                          <span className={cn("text-2xl font-black font-mono", item.critical ? "text-rose-500" : "text-foreground")}>{item.amount}</span>
                          <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Qtd.</span>
                        </div>
                        <button 
            ...
            {activeTab === 'pedidos' && (
              <>
                {[
                  { id: "REQ-0042", date: "Hoje, 14:30", items: 3, status: "pending" },
                  { id: "REQ-0041", date: "Ontem, 09:15", items: 12, status: "completed" },
                ].map((req, i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="font-bold text-foreground tracking-wide block">{req.id}</span>
                        <span className="text-xs text-muted-foreground">{req.date} • {req.items} itens</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      {req.status === 'pending' ? (
            ...
            {activeTab === 'consumo' && (
              <>
                {[
                  { item: "Adrenalina 1mg/ml", patient: "João da Silva", bed: "Leito 04 (Vermelha)", time: "14:15" },
                  { item: "Soro Fisiológico 0.9% 500ml", patient: "Maria Souza", bed: "Leito 02 (Amarela)", time: "12:30" },
                ].map((consumo, i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="font-bold text-foreground tracking-wide block">{consumo.item}</span>
                        <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                          <User className="h-3 w-3" /> {consumo.patient} • {consumo.bed}
                        </div>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-muted-foreground font-mono">{consumo.time}</span>
                  </div>
                ))}
              </>
            )}
              ...
              <>
                <div className="flex items-center justify-between p-4 rounded-xl bg-sky-500/10 border border-sky-500/30 relative overflow-hidden hover:bg-sky-500/20 transition-colors">
                   ...
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="font-bold text-foreground tracking-wide block">Malote de Reposição #229</span>
                        <span className="text-xs text-muted-foreground">Origem: Farmácia Central • 14 itens</span>
                      </div>
                    </div>
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-border/50 flex items-center justify-between bg-muted/30">
                ...
                <button onClick={() => setIsRequestModalOpen(false)} className="h-8 w-8 rounded-lg hover:bg-muted/50 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6 flex flex-col gap-4">
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Medicamento / Material</label>
                  <input type="text" placeholder="Digite para buscar no estoque central..." className="w-full h-11 bg-background/50 border border-border/50 rounded-xl px-4 text-sm focus:outline-none focus:border-sky-500/50 text-foreground" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Quantidade Desejada</label>
                  <input type="number" placeholder="Ex: 50" className="w-full h-11 bg-background/50 border border-border/50 rounded-xl px-4 text-sm focus:outline-none focus:border-sky-500/50 text-foreground" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Prioridade</label>
                  <div className="flex gap-2">
                    <button className="flex-1 h-10 rounded-lg bg-muted/30 border border-border/50 text-sm font-bold text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors">Normal</button>
                    <button className="flex-1 h-10 rounded-lg bg-rose-500/10 border border-rose-500/30 text-sm font-bold text-rose-500 hover:bg-rose-500/20 transition-colors">Urgência</button>
                  </div>
                </div>
              </div>
              <div className="p-6 border-t border-border/50 bg-muted/10 flex justify-end gap-3">
                <button onClick={() => setIsRequestModalOpen(false)} className="px-5 py-2.5 rounded-xl font-bold text-sm text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors">Cancelar</button>
                <button 
                  onClick={() => {
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-border/50 flex items-center justify-between bg-muted/30">
                ...
                <button onClick={() => setDispenseItem(null)} className="h-8 w-8 rounded-lg hover:bg-muted/50 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6 flex flex-col gap-4">
                <div className="p-4 rounded-xl bg-muted/30 border border-border/50 mb-2">
                  <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-1">Item Selecionado</span>
                  <span className="text-lg font-black text-foreground">{dispenseItem.name}</span>
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Leito / Paciente</label>
                  <input type="text" placeholder="Buscar paciente na fila ou leito..." className="w-full h-11 bg-background/50 border border-border/50 rounded-xl px-4 text-sm focus:outline-none focus:border-sky-500/50 text-foreground" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Quantidade Administrada</label>
                  <input type="number" defaultValue="1" className="w-full h-11 bg-background/50 border border-border/50 rounded-xl px-4 text-sm focus:outline-none focus:border-sky-500/50 text-foreground" />
                </div>
              </div>
              <div className="p-6 border-t border-border/50 bg-muted/10 flex justify-end gap-3">
                <button onClick={() => setDispenseItem(null)} className="px-5 py-2.5 rounded-xl font-bold text-sm text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors">Cancelar</button>
                <button 
                  onClick={() => {
