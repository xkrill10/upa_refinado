...
export default function PatientRecord({ patientId }: { patientId?: string }) {
  ...
  const patientOrders = orders.filter(
    (o) => String(o.patientId) === String(id),
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-muted-foreground text-center">
          O prontuário que você está tentando acessar não existe ou foi
          removido.
        </p>
        {!patientId && (
  ...
  if (/febre|infec|dor/.test(complaint))
    suggestedExams.push({
      name: "Hemograma Completo",
      reason: "Investigação de processo infeccioso/inflamatório",
    });
  if (/dor.*peito|tora|cardia|palpita/.test(complaint))
    suggestedExams.push({
      name: "Eletrocardiograma (ECG)",
      reason: "Avaliação cardiológica",
    });
  if (/abdom|barriga|náuse|vomito|vômito/.test(complaint))
    suggestedExams.push({
      name: "Ultrassom Abdominal",
      reason: "Investigação de queixa abdominal",
    });
  if (/respira|tosse|falta.*ar|dispne/.test(complaint))
    suggestedExams.push({
      name: "Raio-X de Tórax",
      reason: "Avaliação respiratória",
    });
  if (/cabeça|cefal|tontura|desmai/.test(complaint))
    suggestedExams.push({
      name: "Tomografia de Crânio",
      reason: "Investigação neurológica",
    });
  if (suggestedExams.length === 0 && patient.mainComplaint) {
    suggestedExams.push({
      name: "Avaliação Clínica Geral",
      reason: "Triagem inicial conforme queixa",
    });
  }
    ...
    >
      ...
      <Tabs defaultValue={activeTab} className="w-full">
        <TabsList className="bg-muted/50 p-1 rounded-xl mb-8">
          <TabsTrigger value="clinical" className="rounded-lg gap-2">
            <FileText className="h-4 w-4" /> Clínica
          </TabsTrigger>
          ...
          <TabsTrigger value="meds" className="rounded-lg gap-2">
                    ...
                    {patient.justification && (
                      <div className="p-4 rounded-xl border border-[#006699]/20 bg-[#006699]/5">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-[#006699]">
                            Justificativa da Classificação (Triagem)
                          </span>
                        </div>
              ...
              <Card className="glass-card">
                <CardHeader className="border-b bg-muted/20">
                  <div className="flex items-center justify-between">
                    ...
                    {patient.triaged && (
                      <Badge className="bg-emerald-500 text-white border-0 text-[10px] uppercase font-bold">
                        Triagem Concluída
                      </Badge>
                    )}
                  </div>
        ...
        <TabsContent value="history" className="space-y-6">
          <Card className="glass-card">
            <CardHeader className="border-b bg-muted/20">
              ...
              <CardDescription>
                Eventos gerados a partir do cadastro do paciente.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <ol className="relative border-l border-border/60 ml-2 space-y-6">
                <li className="ml-6">
                  ...
                  <time className="block text-[11px] text-muted-foreground uppercase tracking-widest">
                    {arrivalDate.toLocaleString()}
                  </time>
                  <p className="text-sm mt-1">
                    Paciente registrado no setor{" "}
                    <strong>{patient.sector || "Triagem"}</strong>.
                  </p>
                </li>
                <li className="ml-6">
                  ...
                  <p className="text-sm mt-1">
                    ...
                    <strong className="uppercase">
                      {riskLabels[patient.risk] || patient.risk}
                    </strong>
                    .
                  </p>
                </li>
        ...
        <TabsContent value="exams" className="space-y-6">
          <Card className="glass-card">
            <CardHeader className="border-b bg-[#006699]/10 dark:bg-sky-500/10">
              ...
              <CardDescription>
                Acompanhamento dos exames laboratoriais e de imagem em
                andamento.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              {!patient.exams || patient.exams.length === 0 ? (
                <p className="text-sm italic text-muted-foreground">
                  Nenhum exame solicitado para este paciente até o momento.
                </p>
              ) : (
                <div className="space-y-4">
                  {patient.exams.map((exam, i) => (
                    <div
                      key={i}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-[#006699]/20 dark:border-sky-500/20 bg-white dark:bg-slate-900 shadow-sm gap-4"
                    >
          ...
          <Card className="glass-card">
            <CardHeader className="border-b bg-muted/20">
              ...
              <CardDescription>
                Sugestões automáticas com base na queixa principal informada no
                cadastro.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              {suggestedExams.length === 0 ? (
                <p className="text-sm italic text-muted-foreground">
                  Nenhuma queixa registrada — não há exames sugeridos.
                </p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <TabsContent value="meds">
          <Card className="glass-card mb-6">
            <CardHeader className="border-b bg-[#006699]/10">
              ...
              <CardDescription>
                Medicamentos prescritos durante o atendimento na UPA.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
              {patientOrders.length === 0 ? (
                <p className="text-sm italic text-muted-foreground">
                  Nenhuma prescrição médica registrada para este atendimento.
                </p>
              ) : (
                <div className="space-y-4">
                  {patientOrders.map((order, orderIdx) => (
                    <div
                      key={orderIdx}
                      className="p-4 rounded-xl border border-[#006699]/20 bg-white dark:bg-slate-900 shadow-sm"
                    >
                      ...
                      <div className="space-y-3">
                        {order.medications.map((med, medIdx) => (
                          <div
                            key={medIdx}
                            className="flex flex-col p-3 rounded-lg border border-border/50 bg-slate-50 dark:bg-slate-800"
                          >
                            <div className="flex justify-between">
                              ...
                              >
                                {med.status === "awaiting_pharmacy"
                                  ? "Aguardando Farmácia"
                                  : med.status === "awaiting_nursing"
          ...
          <Card className="glass-card">
            <CardHeader className="border-b bg-muted/20">
              ...
              <CardDescription>
                Lista de medicamentos declarados na triagem/cadastro.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
              {medicationsList.length === 0 ? (
                <p className="text-sm italic text-muted-foreground">
                  Nenhuma medicação registrada no cadastro.
                </p>
              ) : (
                <div className="space-y-3">
