...
export default function TriageRoom() {
  ...
  useEffect(() => {
    ...
    if (!room || !nurse) {
      toast.error("Acesso negado", {
        description: "Por favor, assuma uma sala de triagem primeiro.",
      });
  ...
  const maskPA = (value: string) => {
    ...
    if (cleanValue.length <= 4) {
      if (parseInt(cleanValue.slice(0, 3)) > 300) {
        // Likely old style or typo
        return `${cleanValue.slice(0, 2)}/${cleanValue.slice(2)}`;
      }
  ...
  const complaintMetadata: Record<
    string,
    {
      icon: React.ElementType;
      description: string;
  ...
  > = {
    "Dor Torácica": {
      icon: Heart,
      description: "Desconforto ou dor na região do peito",
      color: "text-red-500",
      bgColor: "bg-red-50 text-red-500",
    },
    Cefaleia: {
      icon: Brain,
      description: "Dor de cabeça intensa ou persistente",
      color: "text-purple-500",
    ...
    "Dor Abdominal": {
      icon: AlertCircle,
      description: "Desconforto na região do abdômen",
      color: "text-emerald-500",
      bgColor: "bg-emerald-50 text-emerald-500",
    },
    Vômitos: {
      icon: Droplets,
      description: "Náuseas e vômitos frequentes",
      color: "text-blue-400",
      bgColor: "bg-blue-50 text-blue-400",
    },
    Diarreia: {
      icon: Droplets,
      description: "Aumento na frequência de evacuações",
      color: "text-amber-600",
      bgColor: "bg-amber-50 text-amber-600",
    },
    Tosse: {
      icon: Activity,
      description: "Tosse seca ou com secreção",
      color: "text-slate-500",
      bgColor: "bg-slate-50 text-slate-500",
    },
    Síncope: {
      icon: Zap,
      description: "Desmaio ou perda súbita de consciência",
      color: "text-yellow-500",
      bgColor: "bg-yellow-50 text-yellow-500",
    },
    Trauma: {
      icon: Bone,
      description: "Lesões causadas por impacto físico",
      color: "text-slate-700",
      bgColor: "bg-slate-100 text-slate-700",
    },
    Lombalgia: {
      icon: Bone,
      description: "Dor na região lombar (costas)",
      color: "text-orange-700",
      bgColor: "bg-orange-100 text-orange-700",
    },
    Disúria: {
      icon: Droplets,
      description: "Dor ou ardor ao urinar",
      color: "text-blue-300",
      bgColor: "bg-blue-50 text-blue-300",
    },
  };
  ...
  const inTriage = patients
    .filter(
      (p) =>
        !p.triaged && (p.sector === "TRIAGEM 1" || p.sector === "TRIAGEM 2"),
    )
  ...
  const completeTriage = (risk: string) => {
    ...
    toast.success(
      `${formatWords(selectedPatient.name)} classificado como ${riskLabels[risk] || risk}`,
    );
  ...
  const getTicketColor = (ticket: string) => {
    ...
    if (prefix === "E")
      return "bg-red-500 text-white border-red-500 shadow-lg shadow-red-500/20";
    if (prefix === "C" || prefix === "I")
      return "bg-orange-500 text-white border-orange-500 shadow-lg shadow-orange-500/20";
    if (prefix === "P")
      return "bg-amber-500 text-white border-amber-500 shadow-lg shadow-amber-500/20";
  ...
  const getTicketColorLight = (ticket: string) => {
    const prefix = ticket.charAt(0).toUpperCase();
    if (prefix === "E")
      return "bg-red-50 dark:bg-red-950/20 text-red-500 dark:text-red-400 border-red-100 dark:border-red-900/20";
    if (prefix === "C" || prefix === "I")
      return "bg-orange-50 dark:bg-orange-950/20 text-orange-500 dark:text-orange-400 border-orange-100 dark:border-orange-900/20";
    if (prefix === "P")
        ...
        <div className="hidden lg:flex items-center gap-4 px-4 border-r border-slate-200 dark:border-slate-800/60 mr-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-black text-[#006699] dark:text-sky-400">
              {
                patients.filter(
                  (p) => p.triaged && p.sector === selectedTriageRoom,
                ).length
              }{" "}
              PACIENTES
            </span>
            ...
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-black text-red-700 dark:text-red-400">
                  {waitingForTriage.find((p) => p.priority === "emergency")
                    ?.ticket || "NENHUM"}
                </p>
              </div>
              <Button
            ...
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-black text-amber-750 dark:text-amber-400">
                  {waitingForTriage.find((p) => p.priority === "preferential")
                    ?.ticket || "NENHUM"}
                </p>
              </div>
              <Button
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        {patient.ticket && (
                          <span
                            className={`px-4 py-1.5 rounded-xl font-mono font-bold text-xs transition-all border ${getTicketColorLight(patient.ticket)} group-hover:scale-105 w-20 text-center shrink-0`}
                          >
                            {patient.ticket}
                          </span>
                        )}
                        <div className="space-y-1 min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-muted-foreground font-medium flex items-center gap-1.5 opacity-70 truncate">
                            ...
                            {patient.susCard && ` · SUS: ${patient.susCard}`} ·
                            Chegada:{" "}
                          ...
                          className={cn(
                            ...
                            patient.risk === "emergency"
                              ...
                              : patient.risk === "very-urgent"
                                ...
                                : patient.risk === "urgent"
                                  ? "bg-[#FFDE21] hover:bg-[#FFDE21]/90 text-black"
                    ...
                    <div className="w-[300px] flex items-center justify-end gap-2 shrink-0">
                      <Button
                        ...
                        className="text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-xl h-10 w-10 transition-all"
                      >
            ...
            <Card className="glass-card border-none shadow-xl rounded-xl overflow-hidden transition-all duration-500">
              <CardHeader className="pb-4 border-b border-emerald-100/20 dark:border-emerald-800/20 bg-emerald-500/5 dark:bg-emerald-950/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-black uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-300">
                    Pacientes em Atendimento (Triagem)
                  </CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-0">
          ...
          <Card className="glass-card border-none shadow-xl rounded-xl overflow-hidden transition-all duration-500">
            <CardHeader className="pb-3 border-b border-slate-200/40 dark:border-slate-800/40">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-[#006699] dark:text-sky-400">
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Object.entries(
                    equipmentStatus[selectedTriageRoom] || {},
                  ).map(([item, checked]) => (
                    <button
                      key={item}
      ...
      <Dialog open={showNewPatientModal} onOpenChange={setShowNewPatientModal}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto p-0 gap-0">
          <DialogHeader className="p-8 pb-0">
            <div className="flex items-center gap-3">
              <div>
                <DialogTitle className="text-2xl mission-control-title bg-gradient-to-r from-foreground to-foreground/60 bg-clip-text text-transparent">
                  Admiss\xE3o de Paciente
                </DialogTitle>
          ...
          <form onSubmit={handleNewPatientSubmit} className="space-y-6 p-8">
            ...
            <Card className="glass-card border-0 shadow-xl rounded-[2rem] overflow-hidden">
              <CardHeader className="bg-primary/5 border-b border-primary/10 p-8">
                ...
                <CardDescription className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground mt-2 opacity-60">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 p-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <p className="text-xs font-medium italic">
                  Confirme as informações antes de finalizar o registro.
                </p>
              </div>
              <div className="flex gap-3 w-full md:w-auto">
                <Button
                  ...
                  className="rounded-xl px-8 h-12 font-bold uppercase tracking-wider"
                >
                  Cancelar
                </Button>
                <Button
                  type="submit"
                  className="flex-1 md:flex-none gap-2 rounded-xl px-10 h-12 font-black uppercase tracking-widest shadow-lg shadow-primary/20"
                >
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden rounded-2xl border-none shadow-2xl [&>button]:hidden">
          ...
          <div className="p-8 space-y-6 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-[#006699]">
                  Abertura Ocular (AO)
                </Label>
                <Select
            ...
            <div className="bg-[#006699]/5 rounded-xl p-6 flex items-center justify-between border border-[#006699]/10">
              <div className="space-y-1">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#006699]/60">
                  Pontuação Total
                </p>
            ...
            <Button
              onClick={() => {
                ...
                if (total < 15)
                  status =
                    total >= 13 ? "Leve" : total >= 9 ? "Moderada" : "Grave";
                setClinicalData((prev) => ({
                  ...prev,
                  glasgow: `${total} / ${status} (AO:${glasgowValues.eye} RV:${glasgowValues.verbal} RM:${glasgowValues.motor})`,
                }));
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              <DialogDescription className="text-sm font-medium leading-relaxed px-2 text-slate-500 dark:text-slate-400">
                ...
                se retirou da unidade sem concluir a triagem.
              </DialogDescription>
            </div>
