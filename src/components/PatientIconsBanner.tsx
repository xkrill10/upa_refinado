...
export const IconTip: React.FC<IconTipProps> = ({
  label,
  color,
  children,
  critical,
...
export const DICT_BRONCOASPIRACAO = [
  "broncoaspiração",
  "broncoaspiracao",
  "engasgo",
];
export const DICT_ACOMPANHANTE = ["acompanhante"];
export const DICT_IMUNOSSUPRIMIDO = [
  "reverso",
  "imunossuprimido",
  "imuno",
  "oncológico",
...
export const DICT_DESCONHECIDO = [
  "desconhecido",
  "ignorado",
  "sem documento",
  "sem identificação",
...
export const hasSmartKeyword = (
  text: string | undefined | null,
  positiveKeywords: string[],
  negativeKeywords: string[] = DICT_NEGATORS,
): boolean => {
  if (!text) return false;
  const lowerText = text.toLowerCase();
...
export const PatientIconsBanner: React.FC<PatientIconsBannerProps> = ({
  patient,
  iconSize = 13,
  gap = "2px",
}) => {
  ...
  const evolucaoText =
    patient.evolutions?.map((e) => e.description).join(". ") || "";
  ...
  const dietaText =
    fullClinicalText.toLowerCase().includes("jejum") ||
    fullClinicalText.toLowerCase().includes("npo")
      ? "NPO/Jejum"
      : "";
  const testeCovidText = "";
  const dispositivosText =
    fullClinicalText.toLowerCase().includes("sonda") ||
    fullClinicalText.toLowerCase().includes("cvc")
      ? "Sonda/Acesso"
      : "";
  const atbText = medicacoesText.toLowerCase().includes("antibiótico")
    ? "Sim"
    : "";
  const dvaText =
    fullClinicalText.toLowerCase().includes("noradrenalina") ||
    fullClinicalText.toLowerCase().includes("vasopressina")
      ? "Ativa"
      : "";
  const suporteO2Text =
    fullClinicalText.toLowerCase().includes("vni") ||
    fullClinicalText.toLowerCase().includes("cateter") ||
    fullClinicalText.toLowerCase().includes("vm")
      ? "Em uso"
      : "";
  ...
  return (
    <>
      ...
      `}</style>
      <div
        className="patient-icons-banner"
        style={{
          display: "flex",
          gap: gap,
          alignItems: "center",
      ...
      >
        {/* Dieta / Jejum */}
        {hasValue(dietaText) &&
          (dietaText!.toLowerCase().includes("jejum") ||
            dietaText!.toLowerCase().includes("npo")) && (
        ...
        {(hasSmartKeyword(fullClinicalText, DICT_COVID) ||
          (hasValue(testeCovidText) &&
            (testeCovidText!.toLowerCase().includes("positivo") ||
              testeCovidText!.toLowerCase().includes("suspeito")))) && (
          <IconTip
            label="COVID / Infeccioso Suspeito ou Positivo"
            color="#dc2626"
            critical
          >
        ...
        {hasValue(patient.comorbidities) && (
          <IconTip
            label={`Comorbidades: ${patient.comorbidities}`}
            color="#be123c"
          >
        ...
        {patient.age !== undefined &&
          !isNaN(parseInt(String(patient.age), 10)) &&
          parseInt(String(patient.age), 10) < 12 && (
        ...
        {patient.age !== undefined &&
          !isNaN(parseInt(String(patient.age), 10)) &&
          parseInt(String(patient.age), 10) >= 65 && (
        ...
        {/* Alergia */}
        {hasValue(patient.allergies) &&
          patient.allergies!.toLowerCase() !== "nenhuma" && (
            <IconTip
        ...
        {hasValue(precaucoesText) &&
          precaucoesText!.toLowerCase() !== "padrão" && (
        ...
        {hasValue(patient.dischargePrediction) &&
          (patient.dischargePrediction!.toLowerCase().includes("hoje") ? (
            ...
            <IconTip
              label={`Alta prevista: ${patient.dischargePrediction}`}
              color="#10b981"
            >
        ...
        {/* Cirurgia Programada */}
        {(hasSmartKeyword(fullClinicalText, DICT_CIRURGIA) ||
          hasKeyword(patient.dischargePrediction, DICT_CIRURGIA) ||
          hasKeyword(examesText, DICT_CIRURGIA)) && (
        ...
        {(hasSmartKeyword(fullClinicalText, DICT_PSIQUIATRIA) ||
          hasSmartKeyword(patient.risk, DICT_PSIQUIATRIA)) && (
        ...
        {(hasSmartKeyword(fullClinicalText, DICT_BRONCOASPIRACAO) ||
          hasSmartKeyword(patient.risk, DICT_BRONCOASPIRACAO)) && (
        ...
        {hasSmartKeyword(fullClinicalText, DICT_SOCIAL) && (
          <IconTip
            label="Acompanhamento Serviço Social (Vulnerabilidade)"
            color="#0d9488"
          >
        ...
        {hasSmartKeyword(fullClinicalText, DICT_ESCOLTA) && (
          <IconTip
            label="Paciente sob Escolta Policial / Custódia"
            color="#dc2626"
            critical
          >
        ...
        {(hasSmartKeyword(fullClinicalText, DICT_MATERNIDADE) ||
          (patient.sector &&
            patient.sector.toLowerCase().includes("maternidade"))) && (
        ...
        {(hasSmartKeyword(patient.name, DICT_DESCONHECIDO) ||
          hasSmartKeyword(fullClinicalText, DICT_DESCONHECIDO)) && (
          <IconTip
            label="Paciente Desconhecido / Sem Identificação"
            color="#64748b"
          >
        ...
        {patient.id === "super-dummy-test" && (
          <>
            <IconTip label="Extra Teste 1 (Para chegar em 30)" color="#3b82f6">
              <Activity size={iconSize} color="#3b82f6" />
            </IconTip>
            <IconTip label="Extra Teste 2 (Para chegar em 30)" color="#10b981">
              <AlertTriangle size={iconSize} color="#10b981" />
            </IconTip>
            <IconTip label="Extra Teste 3 (Para chegar em 30)" color="#f59e0b">
              <ShieldAlert size={iconSize} color="#f59e0b" />
            </IconTip>
            <IconTip label="Extra Teste 4 (Para chegar em 30)" color="#ef4444">
              <Brain size={iconSize} color="#ef4444" />
            </IconTip>
            <IconTip label="Extra Teste 5 (Para chegar em 30)" color="#8b5cf6">
              <Stethoscope size={iconSize} color="#8b5cf6" />
            </IconTip>
            <IconTip label="Extra Teste 6 (Para chegar em 30)" color="#ec4899">
              <Flower2 size={iconSize} color="#ec4899" />
            </IconTip>
            <IconTip label="Extra Teste 7 (Para chegar em 30)" color="#64748b">
              <Users size={iconSize} color="#64748b" />
            </IconTip>
          </>
        )}
      </div>
