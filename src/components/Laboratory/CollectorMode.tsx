...
interface CollectorModeProps {
  ...
  onConfirmCollection: (patientId: string, examId: string, examType: 'lab'|'image') => void;
  onExit: () => void;
}

export function CollectorMode({ pendingExams, onConfirmCollection, onExit }: CollectorModeProps) {
  const [ticket, setTicket] = useState("");
  const [foundExam, setFoundExam] = useState<{ patient: Patient; exam: ExamRequest } | null>(null);

  const handleScan = () => {
    if (!ticket.trim()) return;
    const found = pendingExams.find(
      (e) => e.patient.ticket.toLowerCase() === ticket.toLowerCase() || e.patient.name.toLowerCase().includes(ticket.toLowerCase())
    );
    if (found) {
      setFoundExam(found);
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => { if (!open) onExit(); }}>
      <DialogContent className="sm:max-w-[700px] rounded-[2rem] p-8 glass-card-premium border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.1)] flex flex-col items-center">
        ...
        <div className="flex w-full gap-2 mt-4 mb-8">
          <Input
            ...
            autoFocus
          />
          <Button onClick={handleScan} className="h-14 px-8 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold uppercase tracking-widest shadow-lg shadow-blue-500/20">
            Buscar
          </Button>
        </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="space-y-4 flex-1">
                <div className="flex items-center gap-3 text-blue-500">
                  ...
                  <h2 className="text-xl font-black uppercase tracking-tight text-foreground">{foundExam.patient.name}</h2>
                </div>
                <div className="flex items-center gap-3 text-foreground/80">
                  <Beaker className="w-5 h-5" />
                  <span className="font-bold text-md uppercase">{foundExam.exam.name}</span>
                </div>
              ...
              <Button 
                onClick={() => {
                  onConfirmCollection(foundExam.patient.id, foundExam.exam.id, foundExam.exam.type);
                  setFoundExam(null);
                  setTicket("");
                }}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-black uppercase tracking-widest">Confirmar Coleta</span>
                </div>
              </Button>
            </div>
          </Card>
