...
export function ExamHistoryModal({ open, onClose }: ExamHistoryModalProps) {
  ...
  const completedExams = useMemo(() => {
    return patients
      ...
      .sort((a, b) => new Date(b.exam.releasedAt || 0).getTime() - new Date(a.exam.releasedAt || 0).getTime());
  }, [patients]);
          ...
          {filteredExams.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredExams.map(({ patient, exam }) => (
                <div key={exam.id} className="p-4 rounded-xl border border-white/5 bg-black/5 dark:bg-white/5 flex flex-col gap-4">
                  <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                    <div>
                      <div className="flex items-center gap-2">
                        <Badge className={`text-[9px] uppercase font-bold tracking-widest ${exam.type === 'lab' ? 'bg-purple-500/10 text-purple-600' : 'bg-teal-500/10 text-teal-600'}`}>
                          {exam.type === 'lab' ? 'Lab' : 'Imagem'}
                        </Badge>
                        <h4 className="font-bold text-sm">{exam.name}</h4>
                        {exam.isCritical && (
                          <Badge className="bg-red-500/20 text-red-600 border-none text-[9px] animate-pulse">VALOR CRÍTICO</Badge>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1 font-semibold uppercase">{patient.name} <span className="text-zinc-500">| TICKET: {patient.ticket}</span></p>
                      <p className="text-[10px] text-zinc-500 mt-1">Liberado em: {exam.releasedAt ? format(new Date(exam.releasedAt), "dd 'de' MMMM 'às' HH:mm", { locale: ptBR }) : 'N/D'}</p>
                    </div>
