      ...
      <DialogContent className="sm:max-w-[800px] p-0 overflow-hidden glass-card-premium border-white/40 dark:border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] rounded-[2rem]">
        <DialogHeader className="p-8 bg-gradient-to-r from-amber-500 to-amber-700 text-white relative">
          ...
          <DialogDescription className="text-amber-100 font-medium">
            Avalie e aprove as solicitações de troca de plantão do corpo
            clínico.
          </DialogDescription>
        </DialogHeader>
            ...
            <AnimatePresence mode="popLayout">
              {filteredRequests.length === 0 ? (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="p-8 text-center border-2 border-dashed border-border/50 rounded-3xl"
                >
                  <CheckCircle2 className="h-12 w-12 text-emerald-500/50 mx-auto mb-3" />
                  <p className="text-sm font-bold text-muted-foreground">
                    Nenhuma troca pendente na fila.
                  </p>
                </motion.div>
              ) : (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div className="flex-1 space-y-4">
                        <div className="flex items-center gap-3">
                          <Badge
                            variant="outline"
                            className="text-[10px] font-black uppercase bg-background shadow-sm"
                          >
                            Ref #{req.id}
                          </Badge>
                      ...
                      {req.status === "pending" ? (
                        <div className="flex md:flex-col gap-2 shrink-0 w-full md:w-32">
                          <Button
                            ...
                            className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 font-black uppercase tracking-widest text-[10px] rounded-xl h-12"
                          >
                            Aprovar
                          </Button>
                          <Button
                            ...
                            variant="outline"
                            className="flex-1 border-red-200 text-red-600 hover:bg-red-50 font-black uppercase tracking-widest text-[10px] rounded-xl h-12 dark:border-red-900/50 dark:hover:bg-red-900/20"
                          >
                            Negar
                          </Button>
                        </div>
