...
import { ArrowRightLeft, CalendarDays, Clock, Send, ShieldAlert, ArrowRight, UserSquare2 } from "lucide-react";
...
export function RequestSwapModal({ open, onOpenChange }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.success("Solicitação enviada com sucesso!", {
      description: "Sua solicitação de troca foi enviada para aprovação da coordenação/RH."
    });
    setTimeout(() => {
      onOpenChange(false);
      setStep(1);
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader className="p-8 bg-gradient-to-r from-indigo-600 to-purple-600 text-white relative overflow-hidden">
            ...
            <DialogDescription className="text-indigo-100 font-medium">
              Transfira seu plantão para outro profissional. Sujeito à aprovação da gestão.
            </DialogDescription>
          </DialogHeader>

          <div className="p-8 space-y-8">
            <div className="flex items-center justify-center gap-4 mb-2">
              <div className={cn("h-2 w-12 rounded-full transition-colors", step >= 1 ? "bg-indigo-500" : "bg-slate-200 dark:bg-slate-800")} />
              <div className={cn("h-2 w-12 rounded-full transition-colors", step >= 2 ? "bg-indigo-500" : "bg-slate-200 dark:bg-slate-800")} />
            </div>
              ...
              {step === 1 && (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <Select onValueChange={(val) => setFormData(p => ({ ...p, myShift: val }))} value={formData.myShift}>
                      ...
                      <SelectContent className="rounded-xl border-white/40 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
                        <SelectItem value="25/05">Sábado, 25/05/2030 - Diurno (07:00 - 19:00)</SelectItem>
                        <SelectItem value="27/05">Segunda, 27/05/2030 - Noturno (19:00 - 07:00)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <Select onValueChange={(val) => setFormData(p => ({ ...p, colleague: val }))} value={formData.colleague}>
                      <SelectTrigger className="h-14 rounded-2xl border-indigo-500/20 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md shadow-sm text-sm">
                        <SelectValue placeholder="Quem irá assumir o seu plantão?" />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border-white/40 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
                        <SelectItem value="dr_joao">Dr. João Mendes (Clínico Geral)</SelectItem>
                        <SelectItem value="dra_maria">Dra. Maria Clara (Clínico Geral)</SelectItem>
                        <SelectItem value="dr_carlos">Dr. Carlos Souza (Pediatria)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="bg-indigo-500/5 dark:bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-4 flex gap-3 text-indigo-800 dark:text-indigo-300">
                    ...
                    <p className="text-xs font-medium leading-relaxed">
                      Ao prosseguir, você confirma que o colega selecionado <strong>já aceitou</strong> cobrir este plantão. A troca só será efetivada após a aprovação da Coordenação/RH.
                    </p>
                  </div>
                </motion.div>
              )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-black/5 dark:border-white/5 shadow-inner">
                      <div className="flex-1 flex flex-col">
                        <span className="text-[10px] font-black uppercase text-red-500/80">Você cede</span>
                        <span className="font-bold text-sm">Seu Plantão</span>
                        <span className="text-xs text-muted-foreground">{formData.myShift === "25/05" ? "25/05/2030 (Diurno)" : "27/05/2030 (Noturno)"}</span>
                      </div>
                      <ArrowRight className="h-5 w-5 text-muted-foreground shrink-0 mx-4" />
                      <div className="flex-1 flex flex-col items-end text-right">
                        <span className="text-[10px] font-black uppercase text-emerald-500/80">Colega assume</span>
                        <span className="font-bold text-sm">
                          {formData.colleague === 'dr_joao' ? 'Dr. João Mendes' : formData.colleague === 'dra_maria' ? 'Dra. Maria Clara' : 'Dr. Carlos Souza'}
                        </span>
                        <span className="text-xs text-muted-foreground">Clínico Geral</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex gap-3 pt-4 border-t border-border/50">
              ...
              >
                {step === 1 ? (
                  ...
                  <>Enviar Solicitação <Send className="ml-2 h-4 w-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /></>
                )}
              </Button>
            </div>
          </div>
