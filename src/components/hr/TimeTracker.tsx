...
import { Clock, LogIn, LogOut, Coffee, ArrowRightCircle, CheckCircle2, Fingerprint } from "lucide-react";
...
export function TimeTracker() {
  ...
  const getStatusColor = (type: string) => {
    switch (type) {
      ...
      case 'pausa': return 'bg-amber-500 text-white';
      case 'retorno': return 'bg-blue-500 text-white';
      case 'saida': return 'bg-red-500 text-white';
  ...
  const getStatusIcon = (type: string) => {
    switch (type) {
      ...
      case 'pausa': return <Coffee className="h-4 w-4" />;
      case 'retorno': return <ArrowRightCircle className="h-4 w-4" />;
      case 'saida': return <LogOut className="h-4 w-4" />;
  ...
  return (
    <Card className="border border-white/70 dark:border-slate-800/50 shadow-2xl glass-card-premium text-blue-950 dark:text-slate-100 rounded-xl overflow-hidden relative transition-colors duration-500">
      ...
      <Fingerprint className="absolute right-[50%] top-8 h-56 w-56 text-blue-500/10 dark:text-slate-700/30 -mr-28 pointer-events-none" strokeWidth={1} />
          ...
          <div className="grid grid-cols-2 gap-3 pt-4">
            <Button 
              ...
              className={cn(
                ...
                canEntrada ? "bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 text-white" : "bg-white/50 dark:bg-slate-800/50 border-blue-100 dark:border-slate-700/50 text-blue-400 dark:text-slate-500"
              )}
            >
            ...
            <Button 
              ...
              className={cn(
                "h-16 rounded-2xl font-black uppercase tracking-widest text-[10px] sm:text-xs transition-all border border-transparent",
                canPausa ? "bg-amber-500 hover:bg-amber-600 shadow-lg shadow-amber-500/20 text-white" : "bg-white/50 dark:bg-slate-800/50 border-blue-100 dark:border-slate-700/50 text-blue-400 dark:text-slate-500"
              )}
            >
            ...
            <Button 
              ...
              className={cn(
                "h-16 rounded-2xl font-black uppercase tracking-widest text-[10px] sm:text-xs transition-all border border-transparent",
                canRetorno ? "bg-blue-500 hover:bg-blue-600 shadow-lg shadow-blue-500/20 text-white" : "bg-white/50 dark:bg-slate-800/50 border-blue-100 dark:border-slate-700/50 text-blue-400 dark:text-slate-500"
              )}
            >
            ...
            <Button 
              ...
              className={cn(
                "h-16 rounded-2xl font-black uppercase tracking-widest text-[10px] sm:text-xs transition-all border border-transparent",
                canSaida ? "bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/20 text-white" : "bg-white/50 dark:bg-slate-800/50 border-blue-100 dark:border-slate-700/50 text-blue-400 dark:text-slate-500"
              )}
            >
            ...
            <AnimatePresence>
              {logs.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-blue-800/50 dark:text-slate-500">Nenhum registro encontrado hoje</p>
                </motion.div>
              ) : (
                logs.map((log) => (
                  <motion.div
                  ...
                  >
                    <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center shrink-0 shadow-inner", getStatusColor(log.type))}>
