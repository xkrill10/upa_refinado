...
export function CallAnnouncer() {
  ...
  const announceCall = useCallback(
    (callName: string, roomName: string, ticket?: string) => {
      if (!isAudioEnabled || isCallPanelRoute || !window.speechSynthesis)
        return;
      ...
      setTimeout(() => {
        ...
        const isUnidentified =
          callName.toUpperCase().includes("NÃO IDENTIFICADO") ||
          callName.toUpperCase().includes("PRÉ-CADASTRO") ||
          callName.toUpperCase().includes("DESCONHECIDO");
