      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/10">
            ...
            <TableHead className="data-label py-4">Profissional Resp.</TableHead>
        ...
        <TableBody>
          {patients.map((patient) => (
            ...
            >
              <TableCell>
                <div>
                  <p className="font-bold tracking-tight text-sm">{formatWords(patient.name)}</p>
                  <p className="text-[9px] font-bold text-muted-foreground uppercase opacity-70">{new Date(patient.arrivalTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • ID: {patient.id}</p>
                </div>
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-2">
                  <div className={`h-1.5 w-1.5 rounded-full ${riskColor[patient.risk] || 'bg-slate-400'} animate-pulse`} />
                  <Badge className={`${riskColor[patient.risk] || 'bg-slate-400'} text-[9px] font-black tracking-widest uppercase py-0.5 border-0 shadow-sm w-28 justify-center`}>
                    {riskLabel[patient.risk] || patient.risk}
                  </Badge>
                </div>
              </TableCell>
              <TableCell className="text-[10px] font-black uppercase text-muted-foreground">
                {patient.sector || (patient.status === 'waiting' && !patient.triaged ? 'AGUARD. TRIAGEM' : '-')}
              </TableCell>
              <TableCell className="text-[10px] font-bold uppercase text-primary">
                {patient.responsibleProfessional || (patient.status === 'attending' ? 'Dr(a). NÃO ATRIBUÍDO' : '-')}
              </TableCell>
              <TableCell className="text-right">
                <div className="flex justify-end">
                  <span className={cn(
                    ...
                    <span className={cn("h-1.5 w-1.5 rounded-full", statusStyles[patient.status]?.dot || "bg-slate-400")} />
