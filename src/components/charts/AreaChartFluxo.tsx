    ...
    <div className={`w-full ${expanded ? 'h-[500px]' : 'h-[250px]'}`}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <XAxis dataKey="time" tick={{fontSize: expanded ? 14 : 10}} tickLine={false} axisLine={false} />
          <YAxis tick={{fontSize: expanded ? 14 : 10}} tickLine={false} axisLine={false} />
          <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', fontSize: expanded ? '16px' : '12px' }} />
          <Area type="monotone" dataKey="entradas" name="Entradas (Triagem)" stroke="#3b82f6" strokeWidth={expanded ? 4 : 3} fillOpacity={1} fill="url(#colorEntradas)" />
          <Area type="monotone" dataKey="altas" name="Altas Médicas" stroke="#10b981" strokeWidth={expanded ? 4 : 3} fillOpacity={1} fill="url(#colorAltas)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
    ...
    >
      <CardHeader className="p-4 pb-2 border-b border-white/10 dark:border-slate-800/20 bg-muted/30">
        <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
          <Activity className="h-4 w-4 text-blue-500" /> Fluxo de Atendimentos vs Altas
        </CardTitle>
      </CardHeader>
