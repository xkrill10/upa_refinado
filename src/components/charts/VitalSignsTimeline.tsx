...
export function VitalSignsTimeline({
  currentHr,
  currentSpo2,
  currentTemp,
}: VitalSignsTimelineProps) {
  ...
  const { series, categories } = useMemo(() => {
    ...
    return {
      ...
      series: [
        ...
        {
          name: "Temp (°C)",
          data: dataTemp.map((n) => parseFloat(n.toFixed(1))),
        },
      ],
    };
    ...
    yaxis: [
      {
        seriesName: "FC (BPM)",
        labels: {
          style: { colors: isDark ? "#ef4444" : "#b91c1c" },
      ...
      },
      {
        ...
        opposite: true,
        labels: {
          style: { colors: isDark ? "#3b82f6" : "#1d4ed8" },
          formatter: (val) => val.toFixed(0),
        },
      },
      {
    ...
    <Card className="glass-card-premium rounded-xl overflow-hidden mt-6">
      <CardHeader className="p-4 pb-0 border-b border-white/10 dark:border-slate-800/20 bg-muted/30">
        <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-primary">
          <Activity className="h-4 w-4" /> Evolução de Sinais Vitais (Últimas 6
          horas)
        </CardTitle>
      </CardHeader>
