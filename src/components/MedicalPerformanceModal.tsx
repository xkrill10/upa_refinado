...
export function MedicalPerformanceModal({
  patients,
  isOpen,
  onClose,
  onViewTimeline,
}: MedicalPerformanceModalProps) {
            ...
            {performanceData.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">
                  Nenhum atendimento médico registrado no momento.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {performanceData.map((doc, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="glass-card border border-slate-200 dark:border-slate-800/60 bg-white/60 dark:bg-slate-900/40 rounded-xl overflow-hidden shadow-sm transition-all"
                    >
                      {/* Header do Médico */}
                      <div
                        ...
                        onClick={() =>
                          setExpandedDoctor(isExpanded ? null : doc.doctorName)
                        }
                      >
                        <div className="flex items-center gap-4">
                        ...
                        <div className="border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 p-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {doc.patients.map((p) => (
                              <div
                                key={p.id}
                                className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 hover:border-sky-500/30 transition-colors"
                              >
                                <div>
