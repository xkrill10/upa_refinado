...
import { Search, Plus, CheckCircle2, Trash2, TestTube2, ActivitySquare, AlertCircle } from "lucide-react";
...
export function FastTrackExamsModal({ patient, onClose }: FastTrackExamsModalProps) {
  const { requestExam } = usePatientsContext();
  const [searchTerm, setSearchTerm] = useState("");
  const [customExam, setCustomExam] = useState("");
  const [selectedExams, setSelectedExams] = useState<{name: string, type: 'lab' | 'image' | 'custom'}[]>([]);
  const [activeTab, setActiveTab] = useState<'lab' | 'image'>('lab');

  // Filtragem
  const filteredExams = EXTENSIVE_EXAMS.filter(
    ex => ex.type === activeTab && ex.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleAddCustomExam = () => {
    ...
    if (selectedExams.some(e => e.name.toLowerCase() === customExam.toLowerCase())) {
      toast.error("Este exame já foi adicionado.");
      return;
    }
    setSelectedExams(prev => [...prev, { name: customExam.trim(), type: 'custom' }]);
    setCustomExam("");
    ...
    <div className="flex flex-col h-[650px] max-h-[85vh] gap-4 bg-transparent rounded-b-2xl">
      ...
      <div className="flex flex-col gap-3 shrink-0 p-4 pb-0">
        <div className="flex gap-2 p-1 bg-slate-200 dark:bg-slate-800 rounded-lg">
          <button
            className={cn("flex-1 py-2 text-sm font-bold uppercase tracking-wider rounded-md transition-all flex items-center justify-center gap-2", 
              activeTab === 'lab' ? "bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-blue-400" : "text-slate-500 hover:text-slate-700"
            )}
          ...
          <button
            className={cn("flex-1 py-2 text-sm font-bold uppercase tracking-wider rounded-md transition-all flex items-center justify-center gap-2", 
              activeTab === 'image' ? "bg-white dark:bg-slate-700 shadow-sm text-purple-600 dark:text-purple-400" : "text-slate-500 hover:text-slate-700"
            )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pb-4">
            {filteredExams.map(exam => {
              ...
              return (
                <div
                  key={exam.name}
                  onClick={() => toggleExam(exam.name, exam.type as 'lab'|'image')}
                  className={cn(
                ...
                >
                  <div className={cn("w-6 h-6 rounded-md border flex items-center justify-center mr-3 shrink-0 transition-colors",
                    ...
                    {isSelected ? <CheckCircle2 className="h-4 w-4" /> : (isLab ? <TestTube2 className="h-3.5 w-3.5" /> : <ActivitySquare className="h-3.5 w-3.5" />)}
                  </div>
                  <span className={cn("text-sm font-semibold transition-colors", 
                    isSelected 
                      ? isLab ? "text-blue-800 dark:text-blue-300" : "text-purple-800 dark:text-purple-300"
            ...
            {filteredExams.length === 0 && (
              <div className="col-span-2 text-center py-8 text-muted-foreground flex flex-col items-center gap-2">
                ...
                <p className="text-xs">Use o campo abaixo para adicionar manualmente.</p>
              </div>
            )}
          </div>
        </ScrollArea>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 space-y-1">
              <label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Adicionar Exame Manualmente (Não listado)</label>
              <Input
            ...
            <Button type="button" onClick={handleAddCustomExam} className="h-10 bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600">
          ...
          {selectedExams.length > 0 && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30">
              <div className="flex items-center justify-between mb-2 shrink-0">
                ...
                <button onClick={() => setSelectedExams([])} className="text-[10px] text-red-500 hover:underline uppercase font-bold">Limpar Tudo</button>
              </div>
              <div className="flex flex-wrap gap-2 max-h-[100px] overflow-y-auto custom-scrollbar pr-2 pb-1">
                {selectedExams.map(ex => {
                  ...
                  return (
                  <span key={ex.name} className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold shadow-sm transition-all hover:-translate-y-0.5",
                    isLab ? "bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/30 dark:border-blue-800/50 dark:text-blue-300" 
                    : isCustom ? "bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-900/30 dark:border-emerald-800/50 dark:text-emerald-300"
                    : "bg-purple-50 border-purple-200 text-purple-700 dark:bg-purple-900/30 dark:border-purple-800/50 dark:text-purple-300"
                  )}>
                    {isLab ? <TestTube2 className="h-3 w-3 opacity-70" /> : isCustom ? <Plus className="h-3 w-3 opacity-70" /> : <ActivitySquare className="h-3 w-3 opacity-70" />}
                    {ex.name}
                    <button onClick={() => toggleExam(ex.name, ex.type)} className="ml-1 opacity-50 hover:opacity-100 hover:text-red-500 transition-colors">
          ...
          <div className="flex gap-3 justify-end mt-2">
            <Button variant="outline" onClick={onClose} className="font-bold uppercase h-11 px-6">
              Cancelar
            </Button>
            <Button onClick={handleConfirm} className="font-bold uppercase tracking-wider bg-blue-600 hover:bg-blue-700 h-11 px-8 shadow-md">
