...
export function DocumentGenerator({
  ...
  const [savedAtestadoModels, setSavedAtestadoModels] = useState<
  ...
  const [savedReceitaModels, setSavedReceitaModels] = useState<
    { id: string; name: string; content: string }[]
  >([]);
        ...
        <DialogContent className="max-w-5xl w-[95vw] bg-slate-100 dark:bg-slate-900 p-0 border-0 overflow-hidden flex flex-col max-h-[90vh]">
          <DialogHeader className="p-4 bg-white dark:bg-slate-950 border-b flex flex-col xl:flex-row items-start xl:items-center justify-between shrink-0 gap-4">
              ...
              {docType === "atestado" && (
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <optgroup label="Atestados e Declarações">
                      ...
                      {!hidePediatricOptions && (
                        <option value="acompanhante">
                          Atestado para Acompanhante / Pediátrico
                        </option>
                      )}
                      <option value="fasttrack">
                        Atestado Fast Track (Sintomáticos)
                      </option>
                      <option value="isolamento">
                        Isolamento Sanitário (Infectocontagiosas)
                      </option>
                    </optgroup>
                    <optgroup label="Relatórios e Encaminhamentos">
                      <option value="encaminhamento">
                        Encaminhamento Médico (UBS/Especialista)
                      </option>
                      <option value="transferencia">
                        Relatório de Transferência (Sala Vermelha)
                      </option>
                      <option value="recusa">
                        Termo de Alta a Pedido / Recusa Médica
                      </option>
                    </optgroup>
                  ...
                  {atestadoTemplate === "custom" && (
                    <div className="flex items-center gap-2 ml-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNewAtestadoModelName(e.target.value)
                        }
                        className="h-8 text-xs font-bold border rounded-lg border-slate-300 outline-none px-2 w-32"
                      />
                      <Button
                        size="sm"
                        className="h-8 text-[10px] px-2 py-0"
                        onClick={handleSaveAtestadoModel}
                      >
                        Salvar
                      </Button>
                    </div>
                  ...
                  {(atestadoTemplate === "padrao" ||
                    ...
                    savedAtestadoModels.some(
                      (m) => m.id === atestadoTemplate,
                    )) && (
                    <>
                      <div className="flex items-center gap-2 ml-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <optgroup label="Receituários">
                      <option value="padrao">
                        Receituário Padrão (Branca)
                      </option>
                      {!hidePediatricOptions && (
                        <option value="pediatrico">
                          Receituário Pediátrico
                        </option>
                      )}
                      <option value="uso_continuo">
                        Receituário de Uso Contínuo
                      </option>
                    </optgroup>
                    <optgroup label="Retenção de Receita (2 Vias)">
                      <option value="antimicrobiano">
                  ...
                  {receitaTemplate === "custom" && (
                    <div className="flex items-center gap-2 ml-2">
                      ...
                      <Button
                        size="sm"
                        className="h-8 text-[10px] px-2 py-0"
                        onClick={handleSaveReceitaModel}
                      >
                        Salvar
                      </Button>
                    </div>
            ...
            <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto justify-end xl:mr-10">
              <Button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white gap-2 font-black uppercase"
              >
          ...
          <div className="flex-1 overflow-y-auto p-8 flex justify-center bg-slate-200 dark:bg-slate-800">
            ...
            >
              ...
              <div
                ref={printRef}
                className="w-full h-full bg-white text-black flex flex-col font-sans"
                style={{ width: "170mm", minHeight: "257mm" }}
              >
                {/* Cabeçalho */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-500 mt-1">
                        Serviço de Pronto Atendimento 24h
                      </p>
                      <p className="text-xs text-slate-400">
                        Av. Principal, 1000 - Centro - SUS
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <>
                      <div className="text-center mb-8">
                        <h3 className="text-2xl font-black uppercase text-slate-800 pb-2 border-b border-slate-200 inline-block px-12">
                          ...
                          {receitaTemplate === "pediatrico" &&
                            "Receituário Pediátrico"}
                          {receitaTemplate === "uso_continuo" &&
                            "Receituário de Uso Contínuo"}
                          {receitaTemplate === "antimicrobiano" &&
                            "Receituário de Antimicrobianos"}
                          {receitaTemplate === "controle_especial" &&
                            "Receituário de Controle Especial"}
                          {receitaTemplate === "orientacoes" &&
                            "Orientações de Alta Médica"}
                          {receitaTemplate === "custom" && "Receituário Médico"}
                          {savedReceitaModels.find(
                            (m) => m.id === receitaTemplate,
                          ) && "Receituário Médico"}
                        </h3>
                        {(receitaTemplate === "antimicrobiano" ||
                          receitaTemplate === "controle_especial") && (
                      ...
                      savedReceitaModels.find(
                        ...
                        <div className="w-full mt-8">
                          {receitaTemplate === "custom" ? (
                            <textarea
                              ...
                              onChange={(e) =>
                                setCustomReceitaText(e.target.value)
                              }
                            />
                          ) : (
                        ...
                        <div className="space-y-6">
                          {medications.length === 0 ? (
                            <p className="text-center text-slate-400 font-medium italic">
                              Nenhum medicamento prescrito para uso externo.
                            </p>
                          ) : (
                            medications.map((med, idx) => (
                              <div
                                key={idx}
                                className={`flex gap-4 p-4 hover:bg-slate-50 rounded-lg transition-colors border-l-4 ${
                                  receitaTemplate === "pediatrico"
                                    ? "border-purple-500"
                                    : receitaTemplate === "antimicrobiano"
                                      ? "border-red-500"
                              ...
                              >
                                ...
                                <div>
                                  <p className="font-black text-lg uppercase text-slate-900 mb-1">
                                    {med.medication}
                                  </p>
                                  <div className="text-sm font-medium text-slate-700 space-y-1">
                                    ...
                                    {med.recommendations && (
                                      <p className="mt-2 text-slate-600 bg-slate-50 p-2 rounded border border-slate-100">
                  ...
                  {docType === "atestado" && (
                    <>
                      ...
                      {atestadoTemplate === "padrao" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            ...
                            <strong>{patient.name}</strong>, inscrito(a) no CPF
                            sob o nº{" "}
                            <strong>{patient.cpf || "não informado"}</strong>,
                            esteve sob meus cuidados médicos nesta unidade de
                            pronto atendimento na data de{" "}
                            <strong>{currentDate}</strong>.
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            Necessita de <strong>{atestadoDays} dia(s)</strong>{" "}
                            de repouso absoluto a partir desta data, por motivo
                            de saúde
                            {printCid
                              ? diagnosis
                                ? ` (CID-10: ${diagnosis})`
                                : ` (CID-10: _____)`
                              : ""}
                            , não podendo exercer suas atividades laborais e/ou
                            acadêmicas durante este período.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "comparecimento" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            ...
                            sob o nº{" "}
                            <strong>{patient.cpf || "não informado"}</strong>,
                            esteve presente nesta Unidade de Pronto Atendimento
                            na data de <strong>{currentDate}</strong>, no
                            período das <strong>[ __:__ ] às [ __:__ ]</strong>,
                            para atendimento médico.
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            Esta declaração não justifica ausência prolongada ao
                            trabalho, limitando-se apenas às horas em que o
                            paciente esteve na unidade.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "acompanhante" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            ...
                            , inscrito(a) no CPF/RG sob o nº{" "}
                            <strong>__________________</strong>, esteve nesta
                            unidade de pronto atendimento na data de{" "}
                            <strong>{currentDate}</strong> acompanhando o(a)
                            paciente menor/incapaz{" "}
                            <strong>{patient.name}</strong>.
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            O(A) paciente necessita de{" "}
                            <strong>{atestadoDays} dia(s)</strong> de repouso
                            por motivo de saúde
                            {printCid
                              ? diagnosis
                                ? ` (CID-10: ${diagnosis})`
                                : ` (CID-10: _____)`
                              : ""}
                            , sendo indispensável o acompanhamento do
                            responsável legal durante todo este período.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "fasttrack" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            Atesto para os devidos fins, após avaliação médica
                            simplificada no fluxo Fast Track (Sintomáticos), que
                            o(a) paciente <strong>{patient.name}</strong>,
                            inscrito(a) no CPF sob o nº{" "}
                            <strong>{patient.cpf || "não informado"}</strong>,
                            compareceu a esta unidade na data de{" "}
                            <strong>{currentDate}</strong>.
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            Necessita de <strong>{atestadoDays} dia(s)</strong>{" "}
                            de repouso domiciliar a partir desta data para
                            tratamento e observação de quadro viral /
                            sintomático leve
                            {printCid
                              ? diagnosis
                                ? ` (CID-10: ${diagnosis})`
                      ...
                      {atestadoTemplate === "isolamento" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            Atesto para os devidos fins que o(a) paciente{" "}
                            <strong>{patient.name}</strong>, portador(a) do CPF{" "}
                            <strong>{patient.cpf || "não informado"}</strong>,
                            foi avaliado(a) nesta unidade com suspeita clínica
                            ou confirmação de doença infectocontagiosa
                            {printCid
                              ? diagnosis
                                ? ` (CID-10: ${diagnosis})`
                                : ` (CID-10: _____)`
                          ...
                          <div className="text-lg leading-relaxed text-red-700 font-bold mt-4 text-justify indent-8">
                            Por determinação sanitária, o paciente necessita de{" "}
                            <strong>{atestadoDays} dia(s)</strong> de isolamento
                            domiciliar rigoroso a partir de{" "}
                            <strong>{currentDate}</strong>, estando
                            expressamente impedido(a) de frequentar ambientes
                            laborais, acadêmicos ou aglomerações.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "encaminhamento" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            ...
                            <strong>{patient.name}</strong>, {patient.age} anos,
                            para avaliação e seguimento na especialidade de{" "}
                            <strong>[ ___________________________ ]</strong> na
                            rede de atenção secundária/primária.
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            <strong>Motivo / Resumo Clínico:</strong> Paciente
                            apresentou quadro de [
                            _________________________________ ] sendo realizado
                            atendimento inicial nesta UPA. Solicito avaliação
                            especializada para conduta definitiva.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "transferencia" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            O(A) paciente <strong>{patient.name}</strong>,{" "}
                            {patient.age} anos, encontra-se internado(a) nesta
                            UPA na data de <strong>{currentDate}</strong> devido
                            a quadro de [ _________________________________ ].
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            <strong>Situação Atual:</strong> Paciente
                            estabilizado, porém com indicação absoluta de
                            transferência via SAMU/CROSS para hospital de
                            referência nível terciário para avaliação com
                            cirurgia geral / neurocirurgia / UTI.
                          </div>
                        </>
                      )}
                      ...
                      {atestadoTemplate === "recusa" && (
                        <>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8">
                            Eu, <strong>{patient.name}</strong> (ou
                            responsável), inscrito no CPF{" "}
                            <strong>{patient.cpf || "não informado"}</strong>,
                            declaro ter sido amplamente informado(a) pelo corpo
                            clínico desta UPA sobre a gravidade do meu quadro
                            clínico e os riscos inerentes de interrupção do
                            tratamento (incluindo risco de morte ou sequelas
                            graves).
                          </div>
                          <div className="text-lg leading-relaxed text-slate-800 font-medium mt-4 text-justify indent-8">
                            Ainda assim, decido por minha livre e espontânea
                            vontade solicitar{" "}
                            <strong>ALTA A PEDIDO / EVASÃO</strong>, assumindo
                            inteira responsabilidade civil e criminal por esta
                            decisão e isentando a equipe médica e a instituição
                      ...
                      {atestadoTemplate === "custom" && (
                        <div className="w-full">
                          <textarea
                            ...
                            onChange={(e) =>
                              setCustomAtestadoText(e.target.value)
                            }
                          />
                        </div>
                      )}

                      {savedAtestadoModels.find(
                        (m) => m.id === atestadoTemplate,
                      ) && (
                        <div className="text-lg leading-relaxed text-slate-800 font-medium mt-8 text-justify indent-8 whitespace-pre-wrap">
                          {savedAtestadoModels
                            .find((m) => m.id === atestadoTemplate)
                            ?.content.replace(/\[DIAS\]/g, atestadoDays)
                    ...
                    <div className="space-y-8 flex-1 pb-32">
                      ...
                      <div className="space-y-6 px-4">
                        <div className="space-y-2">
                          <h4 className="text-sm font-black uppercase tracking-widest text-slate-500 border-b border-slate-200 pb-1">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="text-sm font-bold text-slate-800 uppercase bg-slate-100 p-2 rounded-lg inline-block">
                            {outcome === "alta"
                              ? "Alta Médica (Liberação / Melhora)"
                              : outcome === "encaminhamento"
                                ? "Alta com Encaminhamento (Rede Básica / Especialidade)"
                                : outcome === "alta_pedido"
                                  ? "Alta a Pedido (Termo de Responsabilidade / Evasão)"
                                  : outcome === "observacao"
                                    ? "Retenção: Sala de Medicação / Observação"
                                    : outcome === "exames"
                                      ? "Retenção: Aguardar Exames (Retorno)"
                                      : outcome === "internacao"
                                        ? "Internação (Leito UPA)"
                                        : outcome === "transferencia"
                                          ? "Transferência Hospitalar (Vaga CROSS / SAMU)"
                                          : outcome === "obito"
                                            ? "Óbito"
                                            : "Não informado no sistema"}
                          </div>
                        </div>

                        <div className="space-y-2 mt-8">
                          <h4 className="text-sm font-black uppercase tracking-widest text-slate-500 border-b border-slate-200 pb-1">
                            Prescrição Médica na Unidade
                          </h4>
                          {medications && medications.length > 0 ? (
                            <div className="space-y-3 mt-2">
                              {medications.map((med, idx) => (
                                ...
                                >
                                  <div className="font-black text-sm text-slate-400">
                                    {idx + 1}.
                                  </div>
                                  <div>
                            ...
                            <p className="text-sm text-slate-500 italic">
                              Nenhuma medicação registrada no sistema durante o
                              atendimento.
                            </p>
                          )}
                        </div>
                ...
                <div className="mt-16 pt-8 flex flex-col items-center justify-end">
                  ...
                  <div className="w-full flex items-center justify-between border border-slate-200 bg-slate-50 p-4 rounded-xl mb-12">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[8px] font-bold text-slate-400 mt-1">
                          Acesse validar.iti.gov.br para confirmar a
                          autenticidade.
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-black uppercase text-slate-900">
                        {doctorName || "Médico Não Identificado"}
                      </p>
                  ...
                  <div className="flex flex-col items-center w-80 mb-8">
                    ...
                    <p className="text-sm font-black uppercase text-slate-900 text-center">
                      {doctorName || "Assinatura do Médico"}
                    </p>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium w-full text-center border-t border-slate-100 pt-4">
                    Este documento é válido em todo o território nacional. A
                    falsificação é crime previsto em lei.
                  </p>
                </div>
              </div>
            </div>
