...
import { ArrowRightLeft, BedDouble, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export function BedTransferModal({ isOpen, onClose, patientId, fugulinSummary, onApply }: BedTransferModalProps) {
  ...
  const isIntensive = fugulinSummary?.toLowerCase().includes("intensivo") || fugulinSummary?.toLowerCase().includes("semi-intensivo");
  ...
  const handleTransfer = () => {
    ...
    const originText = currentBed ? ` ${currentBed.name} (${currentBed.room}) para o` : "";
    ...
    onApply(descText);
    onClose(false);
    toast.success(`Paciente ${currentBed ? 'transferido' : 'alocado'} com sucesso!`);
        ...
        <div className="flex-1 overflow-y-auto space-y-5 py-2 pr-2 custom-scrollbar">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 bg-slate-100/50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200/60 dark:border-slate-700/50">
              <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">Leito Atual</p>
              <div className="flex items-center gap-2">
                <BedDouble className={cn("h-5 w-5", currentBed ? "text-blue-500" : "text-slate-400")} />
                <span className="font-bold text-foreground">
                  {currentBed ? `${currentBed.name} (${currentBed.ward})` : "Sem Leito Atribuído"}
                </span>
              </div>
            </div>
            ...
            {fugulinSummary && (
              <div className="flex-1 bg-orange-500/10 p-3 rounded-xl border border-orange-500/20">
                <p className="text-[10px] font-black uppercase text-orange-600 dark:text-orange-400 mb-1 flex items-center gap-1">
                  <AlertTriangle className="h-3 w-3" /> Fator de Dependência (Fugulin)
                </p>
            ...
            <ScrollArea className="h-[250px] rounded-xl border border-border/50 bg-background/50 p-3">
              {Object.keys(bedsByWard).length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                  ...
                  <p className="text-sm font-bold">Nenhum leito disponível no momento.</p>
                </div>
              ) : (
                    ...
                    <div key={ward} className="space-y-2">
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className={cn(
                          ...
                          ward === suggestedWard ? "border-green-500 text-green-600 bg-green-500/10" : ""
                        )}>
                          Ala: {ward}
                        </Badge>
                        {ward === suggestedWard && (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {wardBeds.map(bed => (
                          ...
                          >
                            <span className="text-xs font-black truncate w-full">{bed.name}</span>
                            <span className="text-[9px] text-muted-foreground mt-0.5">{bed.room}</span>
                          </button>
                        ))}
                      </div>
                    </div>
          ...
          <div className="space-y-1">
            <Label className="text-xs font-black uppercase text-foreground/80">Motivo / Conduta (Opcional)</Label>
            <Input 
