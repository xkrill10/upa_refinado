...
export function ParklandModal({ isOpen, onClose, onApply }: ParklandModalProps) {
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <Label className="text-xs font-black uppercase text-foreground/80">Tipo de Paciente</Label>
              <Select 
                value={patientType} 
                onValueChange={(val) => {
                  setPatientType(val);
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="pediatric">Pediátrico (&lt; 14 anos)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            {/* Fator (mL) */}
            <div className="space-y-1">
              <Label className="text-xs font-black uppercase text-foreground/80">Fator (mL/kg/%SCQ)</Label>
              <Select value={factor} onValueChange={setFactor}>
                <SelectTrigger className="h-10 rounded-xl bg-slate-50/50 dark:bg-slate-900/50">
                  <SelectValue placeholder="Selecione..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="2">2 mL (ATLS - Adulto Térmica)</SelectItem>
                  <SelectItem value="3">3 mL (ATLS - Pediátrico Térmica)</SelectItem>
                  <SelectItem value="4">4 mL (ATLS - Elétrica / Parkland Clássico)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Peso */}
            <div className="space-y-1">
              <Label className="text-xs font-black uppercase text-foreground/80">Peso Estimado (kg)</Label>
              <Input 
                type="number" 
                placeholder="Ex: 70"
                value={weight}
            ...
            {/* % SCQ */}
            <div className="space-y-1">
              <Label className="text-xs font-black uppercase text-foreground/80">Superfície Queimada (% SCQ)</Label>
              <Input 
                type="number" 
                placeholder="Ex: 25"
                value={tbsa}
          ...
          {isComplete && weightNum > 0 && tbsaNum > 0 && (
            <div className="mt-6 p-4 rounded-xl bg-slate-50/70 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/50 space-y-4">
              ...
              <div className="flex flex-col gap-2 border-b border-border/50 pb-3">
                <p className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">Volume Total de Reposição (24h)</p>
                <div className="flex items-end gap-2">
                  <p className="text-3xl font-black text-orange-600 dark:text-orange-400">
                    {totalFluid.toFixed(0)} <span className="text-sm font-bold text-muted-foreground">mL (Ringer Lactato)</span>
                  </p>
                </div>
                {hasMaintenance && (
                  <div className="bg-sky-500/10 p-2 rounded-lg border border-sky-500/20 mt-1">
                    <p className="text-[10px] font-black uppercase text-sky-700 dark:text-sky-400">+ Necessidade Hídrica de Manutenção (Holliday-Segar): {maintenance24h.toFixed(0)} mL/24h</p>
                    <p className="text-[9px] text-muted-foreground mt-0.5">Na pediatria, o soro de manutenção deve ser prescrito em paralelo ao volume de reposição da queimadura.</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-background/50 p-3 rounded-lg border border-border/50">
                  <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">Metade nas primeiras 8h</p>
                  <p className="text-xl font-black text-foreground">
                    {first8Hours.toFixed(0)} <span className="text-xs font-bold text-muted-foreground">mL</span>
                  </p>
                  <p className="text-[9px] font-bold text-orange-600 mt-1">Vazão: {(first8Hours / 8).toFixed(0)} mL/h</p>
                  {hasMaintenance && (
                ...
                <div className="bg-background/50 p-3 rounded-lg border border-border/50">
                  <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">Metade nas próximas 16h</p>
                  <p className="text-xl font-black text-foreground">
                    {next16Hours.toFixed(0)} <span className="text-xs font-bold text-muted-foreground">mL</span>
                  </p>
                  <p className="text-[9px] font-bold text-orange-600 mt-1">Vazão: {(next16Hours / 16).toFixed(0)} mL/h</p>
                  {hasMaintenance && (
                    <p className="text-[9px] font-bold text-sky-600 mt-0.5">
