...
export function BedRequestModal({
  patient,
  isOpen,
  onClose,
}: BedRequestModalProps) {
  const { requestAdmission } = usePatientsContext();
  const [selectedType, setSelectedType] = useState<
    "emergency" | "observation" | null
  >(null);

  const handleSubmit = () => {
    if (selectedType) {
      const doctorName =
        localStorage.getItem("upa_stamp_name") || "Médico Assistente";
      ...
      <DialogContent className="sm:max-w-[450px] rounded-[2rem] p-0 overflow-hidden glass-card-premium border border-white/40 dark:border-white/10 shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          <p className="text-[11px] font-black uppercase tracking-widest text-muted-foreground mb-4">
            Escolha o tipo de leito necessário
          </p>
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            >
              ...
              <div>
                <h4 className="font-black uppercase tracking-tight text-red-600 dark:text-red-400">
                  Leito de Emergência
                </h4>
                <p className="text-[10px] font-bold text-muted-foreground mt-0.5">
                  Sala Vermelha / Estabilização imediata
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                <h4 className="font-black uppercase tracking-tight text-amber-600 dark:text-amber-400">
                  Leito de Observação
                </h4>
                <p className="text-[10px] font-bold text-muted-foreground mt-0.5">
                  Ala Amarela / Observação clínica
                </p>
              </div>
            </div>
          </div>
