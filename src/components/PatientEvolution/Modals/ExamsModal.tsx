...
export function ExamsModal({ patient, onClose }: ExamsModalProps) {
  ...
  const [selectedExams, setSelectedExams] = useState<
    { name: string; type: "lab" | "image" }[]
  >([]);
  const [priority, setPriority] = useState<"normal" | "urgent">("normal");
  const [activeTab, setActiveTab] = useState("lab");
  const [justification, setJustification] = useState("");
  const [lateralityDetails, setLateralityDetails] = useState<
    ...
    setSelectedExams((prev) => {
      ...
      examsToSelect.forEach((examName) => {
        if (!newExams.some((e) => e.name === examName)) {
          ...
          if (examData)
            newExams.push({ name: examData.name, type: examData.type });
        }
      });
      return newExams;
    });
  ...
  const filteredExams = COMMON_EXAMS.filter((ex) =>
    ...
    <div className="flex flex-col h-full overflow-hidden gap-2">
      <div className="flex flex-col gap-2 shrink-0 pr-1 pb-1">
        <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1 rounded-xl">
          <Button
            ...
            className={cn(
              "flex-1 h-8",
              priority === "normal" && "bg-blue-600 hover:bg-blue-700",
            )}
      ...
      >
        <TabsList className="grid w-full grid-cols-2 mb-2 bg-muted/50 rounded-xl p-1 h-10 shrink-0">
          <TabsTrigger
            value="lab"
            className="rounded-lg gap-2 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm"
          >
        ...
        <TabsContent
          value="lab"
          className="flex-1 overflow-y-auto pr-2 custom-scrollbar mt-0 outline-none"
          ref={scrollRefLab}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pb-2">
            {filteredExams
              .filter((ex) => ex.type === "lab")
              .map((ex) => {
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pb-2">
            {filteredExams
              .filter((ex) => ex.type === "image")
              .map((ex) => {
                const isSelected = selectedExams.some(
                  (e) => e.name === ex.name,
                );
                const needsDetails =
                  ex.name.includes("Extremidades") ||
                  ex.name.includes("Bacia/Pelve");
                return (
                  <div key={ex.id} className="flex flex-col gap-2">
                    <div
                      onClick={() => toggleExam(ex.name, ex.type)}
      ...
      <div className="shrink-0 space-y-2 mt-2 pt-2 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            Confirmar{" "}
            {selectedExams.length > 0 ? `(${selectedExams.length})` : ""}
          </Button>
        </div>
      </div>
    </div>
