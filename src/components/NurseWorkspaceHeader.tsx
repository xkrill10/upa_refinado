...
import { LogOut, Stethoscope, Baby, Syringe, BedDouble, PackageOpen, Plus, Minus, Check } from "lucide-react";
  ...
  const handleSendRequest = () => {
    ...
    if (formattedItems.length === 0) {
      toast.error("Nenhum item selecionado", {
        description: "Selecione ao menos um item com quantidade maior que zero."
      });
      return;
    }
          ...
          <div className="flex gap-2">
            <Button
              className={cn(
                ...
                isBlueTheme ? "bg-[#006699] hover:bg-[#005580] shadow-blue-500/10" :
                isEmeraldTheme ? "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-500/10" :
      ...
      <Dialog open={isRequestModalOpen} onOpenChange={setIsRequestModalOpen}>
        <DialogContent className="sm:max-w-[550px] p-0 overflow-hidden border border-white/20 dark:border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] rounded-3xl bg-white/80 dark:bg-slate-950/70 backdrop-blur-xl [&>button]:hidden">
          <div className={cn(
            ...
            isBlueTheme ? "bg-gradient-to-br from-[#006699]/90 to-[#004466]/90 dark:from-sky-600/50 dark:to-sky-900/50" :
            isEmeraldTheme ? "bg-gradient-to-br from-emerald-600/90 to-emerald-800/90 dark:from-emerald-600/50 dark:to-emerald-900/50" :
            ...
            <DialogDescription className="text-white/80 font-medium text-xs">
              Selecione as quantidades dos materiais necessários para a <strong className="text-white">{displayRoom}</strong>.
            </DialogDescription>
          </div>
          ...
          <div className="p-6 space-y-4 max-h-[350px] overflow-y-auto">
            {inventoryItems.map(item => {
              ...
              return (
                <div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-500/5 border border-slate-500/10 hover:border-slate-500/20 transition-all">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">
                      Estoque: {item.currentStock} {item.unit} · Setor: {item.location}
                    </p>
                  </div>
          ...
          <div className="p-6 bg-slate-500/5 border-t border-slate-500/10 flex gap-3">
            ...
            <Button
              className={cn(
                ...
                isBlueTheme ? "bg-[#006699] hover:bg-[#005580] shadow-blue-500/20" :
                isEmeraldTheme ? "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-500/20" :
