...
export function QuickConsultModal({
  ...
  >([]);
  const [savedPrescriptionModels, setSavedPrescriptionModels] = useState<
    { id: string; name: string; medications: typeof prescribedMedications }[]
  >([]);
  ...
  const handleSavePrescriptionModel = () => {
    ...
    localStorage.setItem(
      "upa_doctor_prescription_models",
  ...
  const handleRemovePrescriptionItem = (index: number) => {
    setPrescribedMedications(
      prescribedMedications.filter((_, i) => i !== index),
    );
  };
    ...
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent
          ...
          className="max-w-[1200px] w-[95vw] max-h-[95vh] p-0 overflow-hidden flex flex-col bg-slate-50 dark:bg-slate-950 border-0 shadow-2xl rounded-2xl"
        >
          <DialogHeader
            className={cn(
              "p-4 sm:p-6 text-white shrink-0 border-b border-white/20",
              btnColor,
            )}
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <DialogTitle className="text-2xl font-black uppercase tracking-tight text-white flex items-center gap-3">
                    ...
                    <Badge className="bg-white/20 hover:bg-white/30 text-white border-0 font-bold tracking-widest text-[10px]">
                      FAST-TRACK
                    </Badge>
                  </DialogTitle>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-white/60">•</span>
                    <span className="text-white/80 font-medium text-xs uppercase">
                      ID: {patient.id}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-5 space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                    <Badge
                      className={cn(
                        "px-3 py-1 font-black uppercase text-[10px]",
                        riskColors[patient.risk || "not-urgent"],
                      )}
                    >
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl text-center">
                      <p className="text-[10px] font-bold text-red-600/70 uppercase">
                        Pressão
                      </p>
                    ...
                    <div className="p-3 bg-orange-500/5 border border-orange-500/10 rounded-xl text-center">
                      <p className="text-[10px] font-bold text-orange-600/70 uppercase">
                        FC
                      </p>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                      <FileText className={cn("h-4 w-4", iconColor)} /> Evolução
                      / Anamnese
                    </Label>
                    <Textarea
                ...
                <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-blue-50/30 dark:bg-slate-900/50 shadow-sm space-y-4 mt-2">
                  <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                    <span
                      className={cn(
                        "font-extrabold uppercase tracking-wider text-[11px] flex items-center gap-1.5",
                        isPediatric ? "text-orange-600" : "text-[#006699]",
                      )}
                    >
                    ...
                    {savedPrescriptionModels.length > 0 && (
                      <Select
                        onValueChange={handleLoadPrescriptionModel}
                        value=""
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <Button
                        ...
                        className={cn(
                          "w-full h-8 text-[10px] font-black uppercase tracking-wider",
                          isPediatric
                            ? "bg-orange-500/10 hover:bg-orange-500/20 text-orange-600 border-orange-500/30"
                          ...
                          (isSavingPrescModel ? (
                            <div className="flex items-center gap-1">
                              <Input
                                ...
                                onChange={(e) =>
                                  setNewPrescModelName(e.target.value)
                                }
                              ...
                              <Button
                                ...
                                onClick={() => setIsSavingPrescModel(false)}
                              >
                                <X className="h-3 w-3" />
                              </Button>
                            </div>
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-6 text-[10px] text-emerald-600 hover:text-emerald-700 font-bold uppercase p-0"
                              onClick={() => setIsSavingPrescModel(true)}
                            >
                              + Salvar Modelo
                            </Button>
                          ))}
                      ...
                      <div className="bg-white/80 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 p-2 min-h-[120px] max-h-[160px] overflow-y-auto space-y-1.5">
                        {prescribedMedications.length === 0 ? (
                          ...
                          prescribedMedications.map((med, idx) => (
                            <div
                              key={idx}
                              className="flex flex-col bg-white dark:bg-slate-900 p-2 rounded-md border border-slate-200 dark:border-slate-800 text-xs relative group"
                            >
                              <button
                                onClick={() =>
                ...
                <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 w-full">
                  <div className="w-full xl:w-[250px] shrink-0 space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                      <ClipboardCheck className="h-3.5 w-3.5" /> Desfecho do
                      Atendimento *
                    </Label>
                    <Select value={outcome} onValueChange={setOutcome}>
                      <SelectTrigger
                        className={cn(
                          "h-12 rounded-xl font-bold border-2 focus:ring-0",
                          outcome
                            ? "border-green-500"
                      ...
                      <SelectContent className="rounded-xl font-bold">
                        <SelectGroup>
                          ...
                          <SelectItem value="encaminhamento">
                            Alta com Encaminhamento (UBS/Especialista)
                          </SelectItem>
                          <SelectItem value="alta_pedido">
                            Alta a Pedido (Recusa/Evasão)
                          </SelectItem>
                        </SelectGroup>
                        <SelectGroup>
                          <SelectLabel className="text-[10px] uppercase tracking-widest text-slate-500 mt-2">
                            Retenção na Unidade
                          </SelectLabel>
                        ...
                        <SelectGroup>
                          ...
                          <SelectItem value="transferencia">
                            Transferência Hospitalar (Regulação/SAMU)
                          </SelectItem>
                  ...
                  <div className="flex flex-wrap xl:flex-nowrap items-center justify-start xl:justify-end gap-2 mt-4 xl:mt-0 w-full xl:w-auto">
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          "https://siresp.saude.sp.gov.br/index.php",
                          "_blank",
                        )
                      }
                    ...
                    <Button
                      ...
                      className={cn(
                        "h-10 px-4 rounded-xl font-black uppercase tracking-widest text-[10px] gap-2 shadow-lg hover:-translate-y-0.5 transition-all flex-1 xl:flex-none xl:mr-1 mt-2 xl:mt-0",
                        btnColor,
                      )}
                    >
