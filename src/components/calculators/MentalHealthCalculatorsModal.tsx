...
import { Brain, Activity, ShieldAlert, Heart, Info, AlertTriangle, CheckCircle2 } from "lucide-react";
...
interface MentalHealthCalculatorsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onAppendToEvolution: (text: string) => void;
  onUpdateSummary: (text: string) => void;
}

export function MentalHealthCalculatorsModal({
  open,
  onOpenChange,
  onAppendToEvolution,
  onUpdateSummary
}: MentalHealthCalculatorsModalProps) {
  ...
  return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[800px] rounded-[2rem] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            onAppendToEvolution(descText);
                            onUpdateSummary(`RASS: ${rassVal}`);
                            onOpenChange(false);
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const descText = `- ESCALA SAD PERSONS (RISCO DE SUICÍDIO): ${score}/10 pontos (${riskClass.toUpperCase()}).\n  ` +
                              ...
                              `Conduta sugerida: ${recommendation}`;
                            onAppendToEvolution(descText);
                            onUpdateSummary(`SAD: ${score} pts (${riskClass.split(" ")[1] || "Baixo"})`);
                            onOpenChange(false);
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            onAppendToEvolution(descText);
                            onUpdateSummary(`CIWA: ${score} pts (${score > 15 ? 'Grave' : score >= 8 ? 'Mod' : 'Leve'})`);
                            onOpenChange(false);
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            onAppendToEvolution(descText);
                            onUpdateSummary(`CAGE: ${score}/4 (${score >= 2 ? 'Positivo' : 'Negativo'})`);
                            onOpenChange(false);
              ...
              return (
                <div className="space-y-3 animate-in fade-in duration-200">
                  ...
                  <div className="p-3 rounded-2xl bg-muted/30 border border-border space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button type="button" variant="outline" onClick={() => { setPhq1("0"); setPhq2("0"); setPhq3("0"); setPhq4("0"); setPhq5("0"); setPhq6("0"); setPhq7("0"); setPhq8("0"); setPhq9("0"); toast.info("Campos da calculadora limpos."); }} className="h-10 px-4 rounded-xl font-bold uppercase tracking-wider text-[10px] border-muted-foreground/20 text-muted-foreground hover:border-blue-500/40 hover:bg-blue-500/5 hover:text-blue-600 dark:hover:text-blue-400 transition-all">Limpar</Button>
                      <Button type="button" onClick={() => { const descText = `- PHQ-9 (DEPRESS\u00c3O): ${score}/27 pts (${severity.toUpperCase()}).\n  Conduta sugerida: ${recommendation}`; onAppendToEvolution(descText); onUpdateSummary(`PHQ-9: ${score} pts (${severity.split(" ")[0]})`); onOpenChange(false); toast.success("PHQ-9 integrado com sucesso!"); }} className="flex-1 h-10 rounded-xl font-bold uppercase tracking-wider text-[10px] bg-blue-600 hover:bg-blue-700 text-white">Confirmar e Aplicar ao Prontuário</Button>
                    </div>
                  </div>
                </div>
              );
              ...
              return (
                <div className="space-y-3 animate-in fade-in duration-200">
                  ...
                  <div className="p-3 rounded-2xl bg-muted/30 border border-border space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button type="button" variant="outline" onClick={() => { setGad1("0"); setGad2("0"); setGad3("0"); setGad4("0"); setGad5("0"); setGad6("0"); setGad7("0"); toast.info("Campos da calculadora limpos."); }} className="h-10 px-4 rounded-xl font-bold uppercase tracking-wider text-[10px] border-muted-foreground/20 text-muted-foreground hover:border-teal-500/40 hover:bg-teal-500/5 hover:text-teal-600 dark:hover:text-teal-400 transition-all">Limpar</Button>
                      <Button type="button" onClick={() => { const descText = `- GAD-7 (ANSIEDADE): ${score}/21 pts (${severity.toUpperCase()}).\n  Conduta sugerida: ${recommendation}`; onAppendToEvolution(descText); onUpdateSummary(`GAD-7: ${score} pts (${severity.split(" ")[1] || "Mín"})`); onOpenChange(false); toast.success("GAD-7 integrado com sucesso!"); }} className="flex-1 h-10 rounded-xl font-bold uppercase tracking-wider text-[10px] bg-teal-600 hover:bg-teal-700 text-white">Confirmar e Aplicar ao Prontuário</Button>
                    </div>
                  </div>
                </div>
              );
