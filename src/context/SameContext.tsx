...
interface SameContextData {
  ...
  auditLogs: AuditLog[];
  addDocumentRequest: (request: Omit<DocumentRequest, 'id' | 'date' | 'status'>) => void;
...
export function SameProvider({ children }: { children: ReactNode }) {
  ...
  const [digitalUploads, setDigitalUploads] = useState<DigitalUpload[]>([
    { id: "UP001", patient: "Noah Almeida", type: "Evolução Médica", size: "2.4 MB", date: "Hoje, 10:45", professional: "Dr. Ricardo Braga" },
    { id: "UP002", patient: "Helena Santos", type: "Exame de Imagem", size: "15.8 MB", date: "Hoje, 09:12", professional: "Dra. Carla Mendes" },
    { id: "UP003", patient: "João Santos", type: "Receituário", size: "1.1 MB", date: "Ontem, 16:30", professional: "Dra. Beatriz Santos" },
  ]);

  const [auditLogs, setAuditLogs] = useState<AuditLog[]>([
    { id: "LOG001", user: "Dr. Ricardo Braga", action: "Acessou Prontuário", target: "Maria Silva", time: "10:45", date: "12/05/2026", severity: "low" },
    { id: "LOG002", user: "Enf. Ana Paula", action: "Digitalizou Exame", target: "João Santos", time: "09:30", date: "12/05/2026", severity: "low" },
    { id: "LOG003", user: "Admin", action: "Alterou Localização", target: "CX-12 / ALA-A", time: "08:15", date: "12/05/2026", severity: "medium" },
    { id: "LOG004", user: "Dr. Paulo Souza", action: "Exportou Prontuário", target: "Ana Oliveira", time: "16:40", date: "11/05/2026", severity: "high" },
  ]);

  const addDocumentRequest = (request: Omit<DocumentRequest, 'id' | 'date' | 'status'>) => {
  ...
  const updateRequestStatus = (id: string, status: DocumentRequest['status']) => {
    setDocumentRequests(prev => prev.map(req => req.id === id ? { ...req, status } : req));
  };
