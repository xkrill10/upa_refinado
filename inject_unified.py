...
# 4. Inject Modals at the end (before last </motion.div>)
modals = """
      ...
      />
"""
content = content.replace("    </motion.div>", modals + "\n    </motion.div>")

# 5. Hide the giant form when activeTab === 'evolutions'.
# We can just put <EvolutionFeed feed={feed} /> when activeTab === "evolutions".
# But wait, we just want to render it somewhere. Let's find a safe spot.
# There is likely a block like {activeTab === "evolutions" && ( ... )} or something similar.
# Let's replace the whole {activeTab === "evolutions" && (...)} with the feed.
# Actually, the user's giant form is what drops down when `isFormOpen` is true.
# The content when `isFormOpen` is false is what we care about.
